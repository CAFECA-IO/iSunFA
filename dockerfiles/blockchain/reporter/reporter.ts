...
async function getIsuncoinNodeInfo() {
  return new Promise((resolve) => {
    const req = http.request('http://127.0.0.1:20024', {
      method: 'POST',
        ...
        try {
          ...
          if (result && result.result) {
            resolve({
              ...
              networkId: result.result.protocols?.eth?.network || result.result.protocols?.isuncoin?.network || 8017,
...
const collectAndReportNodeStats = async () => {
  try {
    ...
    if (!nodeInfo) {
      console.log('[Node Reporting] Node info not available (RPC not ready?), skipping report.');
      return;
    }
    ...
    const targets = ['https://isuncloud.com/api/v1/nodes', 'https://sinobee.ai/api/v1/nodes'];
      ...
      try {
        const req = https.request(target, {
          ...
          if (res.statusCode >= 200 && res.statusCode < 300) {
            console.log(`[Node Reporting] Report sent successfully to ${target}.`);
          } else {
            console.error(`[Node Reporting] Failed to send report to ${target}: ${res.statusCode} ${res.statusMessage}`);
          }
        });
        
        req.on('error', err => {
          console.error(`[Node Reporting] Network error sending to ${target}:`, err.message);
        });
