...
interface IOpenApiRecord {
    ...
    Symbol?: string;
    [key: string]: string | undefined | null | number | boolean | object | string[];
}
...
async function main() {
    const sources: IDataSource[] = [
        { name: '上市公司', marketType: 'sii', url: 'https://openapi.twse.com.tw/v1/opendata/t187ap03_L' },
        { name: '上櫃公司', marketType: 'otc', url: 'https://www.tpex.org.tw/openapi/v1/mopsfin_t187ap03_O' }
    ];
    ...
    for (const source of sources) {
        ...
        if (!contentType || !contentType.includes('application/json')) {
            ...
            throw new Error(`伺服器沒有回傳 JSON，可能被 WAF 阻擋。回傳內容開頭: ${blockedHtml.substring(0, 100)}`);
        }
