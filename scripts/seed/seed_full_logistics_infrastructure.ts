 ...
 * - PostGIS Native Support: 透過 $executeRawUnsafe 精準寫入幾何空間座標 (SRID 4326)
...
export async function seedLogisticsInfrastructure(dataDir: string) {
    console.log('🌍 [Enterprise] Starting FULL Logistics Infrastructure Seeding...');
    ...
    if (fs.existsSync(seaportsPath)) {
        ...
        for (const chunk of portChunks) {
            const values = chunk.map((f: GeoJSON.Feature) => {
                ...
                const lng = parseFloat((f.geometry as GeoJSON.Point).coordinates[0].toString());
                const lat = parseFloat((f.geometry as GeoJSON.Point).coordinates[1].toString());
                return `('${id}', '${name}', '${country}', '${size}', ${lat}, ${lng}, ST_SetSRID(ST_MakePoint(${lng}, ${lat}), 4326), NOW())`;
            }).join(',');

            if (values.length > 0) {
                const query = `
                    INSERT INTO seaports (id, name, country, size, lat, lng, geom, updated_at)
                    VALUES ${values}
    ...
    try {
        ...
        const validAirports: { id: string, iata_code: string, name: string, country: string, size: string, lat: number, lng: number }[] = [];
        for (const line of lines) {
            ...
            if (ident && !isNaN(lat) && !isNaN(lng)) {
                validAirports.push({ id: ident, iata_code: iata, name, country, size: type, lat, lng });
            }
        }
        ...
        for (const chunk of airportChunks) {
            const values = chunk.map((a: { id: string, iata_code: string, name: string, country: string, size: string, lat: number, lng: number }) => {
                ...
                return `('${id}', '${iata}', '${name}', '${country}', '${size}', ${a.lat}, ${a.lng}, ST_SetSRID(ST_MakePoint(${a.lng}, ${a.lat}), 4326), NOW())`;
            }).join(',');

            if (values.length > 0) {
                const query = `
                    INSERT INTO airports (id, iata_code, name, country, size, lat, lng, geom, updated_at)
                    VALUES ${values}
                    ON CONFLICT (id) DO UPDATE 
    ...
    console.log('🎉 [Enterprise] Full Logistics infrastructure seeding completed!');
}
