...
async function runTest() {
    ...
    console.log(`   👉 結果: ${finResult ? `✅ 成功 (已存至 ${path.basename(finPath)})` : '❌ 失敗'}\n`);
    ...
    const esgPdfResult = await downloadEsgReport(stockId, marketType, year, esgPdfPath);
    console.log(`   👉 結果: ${esgPdfResult ? `✅ 成功 (已存至 ${path.basename(esgPdfPath)})` : '❌ 失敗'}\n`);
    ...
    const esgMetricsResult = await downloadEsgMetrics(stockId, year, esgMetricsPath);
    console.log(`   👉 結果: ${esgMetricsResult ? `✅ 成功 (已存至 ${path.basename(esgMetricsPath)})` : '❌ 失敗'}\n`);
