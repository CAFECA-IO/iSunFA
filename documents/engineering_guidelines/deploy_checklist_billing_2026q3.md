...
| 同時未接受的邀請數 | 團隊 | 20 | 系統設定 `TEAM_PENDING_INVITE_LIMIT` |
| 每日寄送數（滾動 24 小時） | 團隊 | 50 | 系統設定 `TEAM_INVITE_DAILY_LIMIT` |
...
- **限流**擋單人狂點（一瞬間打很多次）
- **冷卻**擋穩定地一直寄 —— 每分鐘一封在限流眼中完全正常，永遠不會超限。**只對免費方案**（產品決定 2026-08-19）：付費團隊每加一席都在付錢，而冷卻是三道裡對他們最痛的一道——60 席的公司一次邀 60 位員工，每分鐘一封要花一小時，而那些席次的錢已經付了。兩道總量上限則**一律套用**：帳號被盜時付費團隊反而是更好的跳板（有卡、有信譽），不該完全沒有上界。訂閱過期或被取消一律視為免費方案，否則「讓訂閱過期」就成了免除冷卻的方法
- **同時未接受數**擋「一次撒出幾百封」
