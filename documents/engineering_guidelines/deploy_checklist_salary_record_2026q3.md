...
## 1.5 PR H：帳本公司設定（20260914–15）

`feature/add_company_setting` 一共動了 **4 處** schema，**全部是新增，
沒有動任何既有欄位**：

| #   | 變更                                                 | 型別／預設                | 需要回填               |
| --- | ---------------------------------------------------- | ------------------------- | ---------------------- |
| 1   | 新表 `account_book_company_profile`                  | —                         | 不需要                 |
| 2   | 新 enum `LeaveYearScheme`                            | 三個值，週年／曆年／約定  | 不需要                 |
| 3   | `salary_record.entity_name_snapshot`                 | `String?`（可空、無預設） | **不需要，而且不可以** |
| 4   | `AuditLogDataType` 加 `ACCOUNT_BOOK_COMPANY_PROFILE` | 既有 enum 加值            | 不需要                 |

另外在 `account_book` 加了一個反向關聯宣告 —— 與 §1、§1.4 同理，
那是 Prisma 層的宣告，**不產生任何 DB 欄位**。
...
- **新表是空的。** `account_book_company_profile` 與帳本 1:1，
  但既有帳本**不會**自動長出一列 —— 沒有那一列的語意就是「還沒設定過」，
  而 `accountBookCompanyProfileService.getProfile` 對 `null` 回
  `isConfigured: false`。想「順手」替每一本帳建一列空設定的話，
  請不要：那會讓每一本帳都變成「設定過但公司名稱是空字串」，
  而那兩種狀態在程式裡是分開的。
- **新欄位可空。** `entity_name_snapshot` 是 `String?`，既有的
  `salary_record` 列一律 `null`，`db push` 直接過。
- **enum 加值是相容操作**（同 §1.4.3）。PostgreSQL 的 enum 加值不影響
  既有的 `audit_log` 列，新的 `LeaveYearScheme` 則根本還沒有任何列在用。

### 1.5.2 `entity_name_snapshot` **不可以**回填，這與 §1.4.1 相反

§1.4.1 的 `base_salary_snapshot` 是「**必須**回填」，而且它的問題正是
`@default(0)` 給了一個合法但錯的值。這一欄看起來像同一類，結論卻相反：

**`null` 在這裡不是「漏填」，是「當時系統根本不知道公司抬頭」。**
這個欄位 20260914 才存在，在那之前存的每一筆紀錄，產生當下系統手上
沒有公司名稱這項資料 —— 那個事實**沒有任何腳本補得出來**。

消費端因此有一條明確的回退：`resolveEntityName(快照, 現值)` ——
快照是 `null` 就用公司設定的現值。回填成現值的話，那個回退永遠不會觸發，
而**兩者的差別會在公司改名那一天現形**：
回退版印的是「今天這家公司的名字」（誠實的近似），
回填版印的是「回填當天那家公司的名字」（一個被寫死的猜測，而且看不出是猜的）。

> ⚠️ 同理，**不要給它 `@default("")`**。空字串會讓 `??` 的回退失效
> （`""` 不是 `null`），症狀是每一張舊薪資單重印出來抬頭是一行空白。
> 這一條與 `leave_year_scheme` 的「不給 `@default`」由
> `src/__tests__/salary_schema_defaults.test.ts` 釘住。

### 1.5.3 順序做錯的症狀

這一節與 §2.3 那張表最大的差別是：**這次沒有「安靜的那一種」** ——
因為沒有回填步驟，少做一步這個選項不存在。

| 做錯什麼                       | 會不會噴錯 | 症狀                                                                                                                                                        |
| ------------------------------ | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 先部署程式、還沒 `db push`     | **會**     | 公司設定的 GET／PUT 噴 `relation "account_book_company_profile" does not exist`；儲存薪資紀錄也會噴（它會去讀公司設定）。整個薪資模組的寫入路徑掛掉，看得見 |
| `db push` 了但沒 `generate`    | **會**     | tsc 直接紅：`AuditLogDataType.ACCOUNT_BOOK_COMPANY_PROFILE` 在生成的 enum 裡不存在（實測是 2 個 error，另一個在 `audit_log.repo.ts`）                       |
| **先 `db push`、程式還是舊的** | **不會**   | 這是**安全**的中間狀態：新表是空的沒有人讀，新欄位可空舊程式不碰。可以放心先 push 再部署                                                                    |

換句話說：**這一次三種做錯方式裡，唯一不噴錯的那一種是安全的。**
與 §2.3 剛好相反 —— 那裡不噴錯的那一種是最危險的。

### 1.5.4 上線後怎麼確認它真的在動

不需要 SQL，**看得到**：

1. 隨便一本帳進 `薪資紀錄` 頁 → 應該看到一則橘色提示「這本帳還沒有公司設定」。
2. 點進 `公司設定`、填好公司名稱存檔 → 提示消失。
3. 回薪資紀錄頁勾一筆匯出 → CSV 第一列是
   `{公司名稱}　工資清冊　{期間}`，檔名是 `{公司名稱}_工資清冊_{期間}.csv`。

第 3 步沒有表頭的話，先確認第 2 步真的存進去了（設定頁重新整理，欄位還在嗎）——
20260914 有一個 bug 正是**存不進去而畫面只說「儲存失敗」**
（`optionalText` 用 `.optional()` 收不到 `null`，已修，見
`src/__tests__/salary_company_profile_contract.test.ts`）。
