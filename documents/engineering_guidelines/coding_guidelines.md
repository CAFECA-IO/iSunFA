...
3. **Repository (Data Access)**：**唯一的資料庫出入口。**
   ...
   - **禁忌**：不處理任何業務邏輯。
   - **例外 —— 跨聚合的原子寫入 (Unit of Work)**：
     當一次操作必須同時改動多張表、且「只改成功一半」會產生無法自我修復的不一致時，
     該筆交易 (`$transaction`) 連同它所保證的不變式**由 Repository 承載**，不得拆到 Service。
     理由是原子性只有 DB 給得起，而把 `$transaction` 拉到 Service 會違反更高優先的
     「只有 Repository 能碰 Prisma」。
