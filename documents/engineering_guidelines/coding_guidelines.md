...
> Info: (20260820 - Julian) 最後那一句有一個常被誤讀的邊界（M11）。
>
> `employee.repo` 的 `managesEmployee()` 曾經在 JSDoc 首行自稱
> 「**這是授權判斷**」，正對撞這裡的反例。但它算的其實是**組織圖上的一個事實**
> （target 的部門在不在 manager 那個部門的子樹裡）——「因此他可以看／可以簽」
> 才是判斷，而那只在 `assertMay*` 那幾支。同一個事實在不同動作上會導出不同答案：
> 查餘額時「管得到」就放行，核准時「管得到」還要再過一道不得自我核准。
>
> 判準因此是：**Repository 可以回答「是什麼」，不可以回答「可不可以」。**
> 方法命名與註解要讓這件事看得出來 —— 那句自稱就是它一度看不出來的證據。
...
實測結果：**全 repo 的 `$transaction` 全部已在 `src/repositories/` 底下，Service 層一處都沒有。**
其中唯一的讀取型用法是 `talk.repo.listThreadsWithCounts` 的陣列形式（分頁 + 總數），依上面的界定不受本節約束。

也就是說**這條例外描述的是既有實踐，不是新開的口子** —— 它把已經在做的事寫成有判準的規則。

> Info: (20260820 - Julian) 這裡原本寫著「**共 11 個檔案**」。那個數字 2026-08-17
> 成立，是手數的，而 2026-08-20 實測已經是 **19 個**（結論不變，數字錯了）。
>
> 一份規範最怕的不是被違反，是它自己描述的現況已經不是現況 —— 讀到「共 11 個」
> 的人會以為自己看的是一份維護中的文件。因此**數字從本節移除**，改由
> `src/__tests__/transaction_layering.test.ts` 把不變式本身釘住：
> 掃描根是整個 `src`（不只 `src/services`，因為 route 與 lib 違反的是同一條規則），
> 違反時它會**列出檔名**，而不是要求誰再數一次。
>
> 同一支測試另外釘住「排除掉的檔案確實只有 `.test.ts`」—— 否則「測試檔不算」
> 這個放寬本身會變成後門（把服務放進 `__tests__` 就能繞過）。

> ToDo: (20260817 - Luphia) 尚未逐一核對那些檔案是否都滿足上面三條判準（尤其是
