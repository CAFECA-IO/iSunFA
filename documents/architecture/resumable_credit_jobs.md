...
| 意圖 | 沒有任務 | 已完成 | 已取消 | 別人正在跑 |
|---|---|---|---|---|
| `RESUME` | 404 | 400（收起按鈕） | 400（收起按鈕，錯誤碼另立 `TW000033`） | **409（留著按鈕，請他等）** |
| `START` | 放行 | 放行（覆寫舊書籤） | 放行（覆寫舊書籤） | **409** |

`START` 也要擋：兩個分頁各自從第一份開始匯入同一個聊天室的話，兩份帳都要付。

已取消與已完成分成兩個錯誤碼（review #6726 高-1）：兩者的下一步不同——已完成是「沒有東西可做了」，已取消是「你自己說不做的」，後者顯示成前者會讓使用者以為系統把它跑完了。

**客戶端要逐字執行這張表**（review #6726 阻-1）：換許可的 catch 只對「不是判決」的失敗（網路斷、伺服器自己壞掉）放行；`TW_JOB_CANCELLED`／`TW_JOB_ALREADY_COMPLETED`／403 一律不跑、各說各的話，終局判決並讓畫面改口（`refreshImportJob`）。判斷收斂在 `resolveJobClaimDenial`（純函式）。高-1 曾只修伺服器那一半——BroadcastChannel 不可用、或舊分頁開在另一台裝置上時，客戶端吞掉判決就是照跑照扣。
