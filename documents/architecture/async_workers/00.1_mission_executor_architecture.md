...
1. **任務層級分流 (`skillRegistry` 與 Two-Turn RAG)**：程式會檢查 `subTaskConfig.type` 是否為預先寫死的 TypeScript Skill（例如 `EsgParsingSkill`）。這些 Skill 會在內部執行兩回合檢索 (Turn 1: 擷取特徵 -> Turn 2: 本地向量檢索後由 AI 選擇)。
...
為了貫徹系統的「零誤差企業級數值架構」，`MissionExecutor` 的職責邊界有嚴格的型別管制：

- **乾淨的 JSON 輸出**：Executor 僅負責將 AI 或規則引擎的結果序列化為乾淨的 JSON 格式，並寫入本地 `result.md`。
- **邊界隔離**：Executor 絕不會引入 Prisma，也不負責操作 `BigInt`、`Prisma.Decimal` 等資料庫底層型別。這些高風險的型別鑄造（Type Casting）與防護，全數保留給負責寫庫的 `MissionRecorder` 統一執行。這種設計完美遵守了單一職責原則（SRP），防止資料庫耦合洩漏到邊緣 AI 運算節點。
