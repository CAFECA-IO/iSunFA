...
1. **業務邏輯攔截 (`VendorRegistry`)**：最優先攔截，針對傳票解析，若判定為「中華電信」等已知廠商特例，走決定論查表，100% 杜絕 LLM 算數學。
2. **任務層級分流 (`skillRegistry`)**：若未被攔截，程式會檢查 `subTaskConfig.type` 是否為預先寫死的 TypeScript Skill。
