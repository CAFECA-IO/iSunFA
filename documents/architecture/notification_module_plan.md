...
| D43 | 通知的去處以**型別**為鍵，而它實際取決於**類別**：`NOTIFICATION_LINK_PATH[ANALYSIS_*]` 一律指向 `/analysis?tab=history`，但該頁的預設查詢是 `type IN [...CATEGORIES]`，而 `CERTIFICATE_ANALYSIS`、`AI_CONSULTING`、`TRANSPORTATION_CARBON_FOOTPRINT`、`JOURNAL_CORRECTION` 四種都不在其中 —— 使用者點「憑證分析失敗」會落在一個結構上放不下那筆紀錄的頁面，而頁面正常載入、其他分析都在，看起來像資料消失（比 D12 的死連結更難查）。**文案層認得這四個類別（五語系字典都有名字），導航層不認得**；成功與失敗兩條路都中 | 分兩步，皆已完成：① 去處解析搬進 `notificationHrefOf(type, payload)`，讀 `payload.analysisType` 查 `ANALYSIS_LINK_PATH_BY_CATEGORY`（只列那四種例外，其餘 11 種仍走型別層那一格）；`resolvePathTokens` 逐段代入 `:token`，**任一個代不進去整條回 `null`** —— 半條路徑（`/user/account_book/undefined/journal`）與字面 `:accountBookId` 都是「看起來有反應」的錯誤去處。② `accountBookId` 進 payload：recorder 原有的三層 fallback 抽成 `resolveAccountBookId`，完成通知、成功路徑的失敗通知、放棄路徑的失敗通知三處共用同一個答案；取不到就不寫這個鍵（不是寫 null —— payload 是永久資料，恆為 null 的欄位會被讀成「這筆沒有帳本」）。發現於 `qa_recorder_injection.ts --scenario sync-failed` |
| D44 | **已確認、未實作** —— 批量分析的通知洗版與粒度不對稱：`MissionIssuer` 對憑證分析**每個檔案發一個任務**，而完成通知的 dedupeKey 是 `analysis-completed:<analysisId>`、失敗是 `analysis-failed:<orderId>` —— 一次上傳 20 張憑證會發 **20 則**完成、卻只發 **1 則**失敗。`NOTIFICATION_HISTORY_LIMIT` 是 10，所以一次批量上傳就把面板整個洗掉，其他通知全被推到第二頁。粒度不對稱同時解釋了驗收清單 B 段那個洞：失敗通知的粒度是訂單，它沒有能力表達「其中 3 筆需人工確認」 | 三個修法各有代價（訂單級摘要有靜默風險、查詢層聚合有效能風險、就地更新計數引入可變通知），**需要先做一個決定** —— 分析與傾向見 [`notification_batch_granularity.md`](notification_batch_granularity.md)。發現於驗收的反向盤點（「目前已接線的功能有哪些」） |
| D45 | 面板底部「查看全部通知」的條件寫成 `list.completed.length > 0 && list.completed.length > 0` —— **同一個判斷兩次**，一個沒寫完的想法留下的痕跡。效果是**只有待辦（例如一封待接受的邀請）而沒有完成通知的人，完全找不到 `/user/notifications`**。而 D20 加這個連結的理由正是「只在被截斷時才出現的話，這個頁面就只有通知夠多的人發現得了」—— 綁在 `completed.length` 上是同一個錯誤的弱化版本：門檻從「夠多」降到「至少一則」，但仍然是門檻 | 條件只保留 `list`（那不是門檻，是「清單還沒讀到」）；補掃描測試「底部的查看全部連結不綁在完成通知的數量上」。發現於把人工驗收項目改寫成測試時（`notification_manual_verification.md` §8） |

**D43 值得記的是「兩張表各自都對，合起來錯了」**：`NOTIFICATION_TYPE`（4 種）與 `ANALYSIS_CATEGORY`（15 種）是兩個不同粒度的分類，而去處查表以前者為鍵。對邀請與錢包升級是對的 —— 它們沒有類別可言；對分析類則是把 15 種壓成 2 格，壓掉的那 13 分之 4 正好是**不在 `/analysis` 那一頁上的**。

而它躲掉的方式很典型：**文案層認得那四個類別**（五語系字典都有名字，通知會正確顯示「憑證分析已完成」），只有導航層不認得。同一個 payload 餵給兩個消費者，一個答得出、一個答不出，而畫面上兩者都「正常」。

可推廣的判準：**一張查表的鍵，要問「它是不是真的決定了答案」**。這裡真正決定去處的是類別，型別只是它的上位分類 —— 以上位分類為鍵時，下位的差異會安靜地被折疊掉。
