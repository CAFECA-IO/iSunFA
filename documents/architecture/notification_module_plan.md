...
| D43 | **未修** —— 通知的去處以**型別**為鍵，而它實際取決於**類別**：`NOTIFICATION_LINK_PATH[ANALYSIS_*]` 一律指向 `/analysis?tab=history`，但該頁的預設查詢是 `type IN [...CATEGORIES]`，而 `CERTIFICATE_ANALYSIS`、`AI_CONSULTING`、`TRANSPORTATION_CARBON_FOOTPRINT`、`JOURNAL_CORRECTION` 四種都不在其中 —— 使用者點「憑證分析失敗」會落在一個結構上放不下那筆紀錄的頁面，而頁面正常載入、其他分析都在，看起來像資料消失（比 D12 的死連結更難查）。**文案層認得這四個類別（五語系字典都有名字），導航層不認得**；成功與失敗兩條路都中 | 修法、影響範圍與驗收判準見 [`notification_link_destination_fix.md`](notification_link_destination_fix.md)。分兩步：先讓去處解析吃 `payload.analysisType`（缺 payload 的兩種先填 `null`，退回 D12 的不可點形狀），再補 `accountBookId` 進 payload。發現於 `qa_recorder_injection.ts --scenario sync-failed` |
