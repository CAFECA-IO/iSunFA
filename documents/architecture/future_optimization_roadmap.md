...
### 4. 財務逆向工程的「三大指標」局限性 (Mock Data Limitations)
*   **盲點**：目前的 `financial_reverse_engineer.ts` 是一個高度簡化的財務模擬器，只逆向萃取了「營業收入」、「營業費用」與「折舊」三個數字。
*   **風險**：系統並未模擬期初現金、存貨、應收帳款、銀行借款等龐大科目。若我們貿然在 `cross_validator.ts` 中比對「總資產」或「淨利」，會發現模擬出來的總資產僅剩數十億（全為銷貨收現），而 Golden Data 是 5 兆，導致測試 100% 失敗。這會讓不知情的開發者誤以為核心引擎崩潰。
*   **解法**：我們的終極目標是「CPA 認證」。因此，內部勾稽測試（確保引擎不當機）與 AI 分類抽憑（Revenue/OpEx）是現階段的絕對重心。至於『總資產對齊真實財報』，我們將它正式列為下一階段 (Phase 6) 的架構升級目標，未來將透過導入完整的『期初餘額 (Opening Balances)』系統來徹底解決資產配平問題。
*   **近期行動 (Action Item)**：在下一階段，我們將深化 `cross_validator.ts`，加入**「內部三表勾稽測試 (Internal Articulation)」**。我們不比對外部絕對數字，而是嚴格驗證：(1) 資產=負債+權益 是否恆等 (2) 損益表淨利是否成功結轉至資產負債表與現金流量表 (3) CFS期末現金是否等於BS現金。這將成為證明報表引擎運算邏輯 100% 完美的終極防線。
