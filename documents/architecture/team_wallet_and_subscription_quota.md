...
**展延閘門（阻擋-1，產品裁定 2026-08-21）：當期剩餘 30 天內才能購買延長／換方案**（`SUBSCRIPTION_EXTENSION_WINDOW_DAYS`，`TW_SUBSCRIPTION_EXTENSION_TOO_EARLY`）。展延語意維持原案（新期自當期屆滿日累加、`planId` 立即換新），但沒有閘門時它對「換方案」是一個漏洞：年繳團隊版第 1 天買月繳企業版 → 剩餘 364 天全部免費升級再加一個月，約四折。閘門把免費升級的剩餘天數壓到最多 30 天，也讓訂閱跨距任何時候不超過「一期 + 30 天」——席次補收的 2 倍上限與新分母因此天然有界。付款頁的揭露文案分流：窗內講「自該日累加（升級立即適用）」，窗外講「剩餘尚多，請於到期前 30 天內再延長」。
