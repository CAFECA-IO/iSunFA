...
- **免費版人數上限**（`FREE_PLAN_MAX_MEMBERS` 系統設定，預設 5）：額度逐成員化後，付費方案以「席次 × 單價」自然封頂，免費版的席次單價是 0——人數上限就是它的封頂。方案頁的免費版卡片已標示該數字（由 server 端讀設定後傳入，與記憶保留天數同一套作法），服務條款 §3.1「以方案頁標示為準」因此成立；調整設定即同步標示與擋門，不需改條文。
