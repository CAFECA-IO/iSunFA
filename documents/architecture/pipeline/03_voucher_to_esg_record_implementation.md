...
### 🚨 2.1 嚴重違反職能分離：逼迫 AI 算數學 (Forcing AI into Arithmetic)
這是目前 codebase 內最大的架構地雷。大型語言模型 (LLM) 不具備穩定的浮點數運算能力。碳排係數通常包含多位小數（如 `0.054495`），讓 AI 去執行 `123.45 加侖 * 0.054495`，極其容易產生錯誤的 `emissions`。這種「數學幻覺」一旦進入資料庫，將面臨極其嚴重的合規風險，Big 4 查帳時會立刻亮起紅燈。
