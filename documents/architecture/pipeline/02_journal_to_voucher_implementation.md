...
這讓 AI 承擔了「取得歷史匯率」與「浮點數乘法」的雙重責任。AI 算數學（匯率幻覺）絕對會產生截斷誤差，在 CPA 審計標準下，一元之差即代表內控失敗。Roadmap V2 已明確規定「外幣匯率與後端運算解耦」。
...
3. **將匯率與數學運算抽回系統層 (Backend Math Offloading)**：
   - 剝奪 AI 算匯率的權力，AI 僅被允許萃取「原始幣別 (如 USD)」與「原始金額 (如 100.50)」。
   - 後端透過專屬的匯率微服務 (Exchange Rate API) 取得精確的結匯日匯率，並使用精確的 `Prisma.Decimal` 與 `BigInt` 模組算出本位幣，從根源消滅數學幻覺。
