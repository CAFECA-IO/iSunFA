...
| 時機         | 做什麼                                                                                                                                                                                                         |
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 現在         | 把「加購點數」從承諾裡拿掉：本文 §2 / §5.1 的敘述（已補警語），以及 `carbon_chatbot.import_paused_chapters` 那句「補上點數後可以從這裡接著匯入」（五語系，見 §13.3）。今天只有「等視窗重置」與「升級方案」兩條 |
...
也就是說 §12.3 記為「待決定」的深連結，現在不是優化，而是**這則通知能不能
兌現的前提**。一則說「回去按 X」的通知，如果落地的地方沒有 X，它就不是通知，
是一道謎題。落點：

- `/user/carbon_chatbot` 支援 `?session=<id>`，並在 `&openImport=1` 時
  到站直接把預覽卡打開；
- `NOTIFICATION_LINK_PATH` 改成帶 token 的樣板，代入走 D43 已經做好的
  `resolvePathTokens`（任何 token 代不出來就整條回 null，不會產生半條路徑）；
- 文案跟著縮短成「可以繼續了」，不再指名任何按鈕 ——
  **通知不該教學路徑，它該把人放在能動手的地方。**

修改計劃（三組問題合成一份）見 `resumable_job_resume_landing_and_copy.md`。
