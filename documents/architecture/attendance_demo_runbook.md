# 🎯 簽到系統 Demo 執行手冊 (Demo Runbook)

> **Author**: Julian　**Last updated**: 2026-08-14
> **演示形式**：**全程手機**（兩支）。P2 靠實地走出圍欄，不靠任何座標偽造工具。
> **相關**: `attendance_demo_plan.md`（做了什麼、為什麼）｜`attendance_demo_mock_data.md`（資料長什麼樣）
> **本文件**：**怎麼跑、會出什麼錯、出錯怎麼辦**
>
> **演示當天只需要帶這一份。** §6 是逐步操作，§7 是故障對照表 —— 那兩節設計成可以站著看。

### 一句話的心法

**技術風險幾乎全部集中在「定位」與「網路」，而它們都無法在台上修。**
因此重量放在**事前校準**（§2）與**備援分級**（§8），不是台上的臨場反應。

### ⚠️ 兩件會讓整場演不成、而且只能事前處理的事

1. **手機連得到 demo 站台**（計畫書 §10.1）。`NEXT_PUBLIC_APP_URL` 若還是
   `https://isunfa.localhost`，手機解析不到 —— 而定位、Google 登入、Passkey
   三者都要求安全來源。要嘛隧道、要嘛區網 IP + 憑證，**T−5 就要定案**。
2. **演示日期綁在 seed 的常數上**。全文以 **2026-08-13（週四）14:00** 為演示當日，
   而 `seed_attendance_demo.ts` 是寫死日期的（`DEMO_DATE` 等六個常數）。
   **改期就要一起平移**，否則現場頁的「今日」是空的。清單見計畫書檔頭。

---

## 🗓️ 1. 時程與分工

| 時點             | 工作                                                                          | 完成判準                                                            |
| ---------------- | ----------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| **T−7**          | Google OAuth 用戶端申請、憑證寫入 `/admin/settings` 並簽章（§3.2）            | 打卡頁登入卡上**看得到** Google 按鈕，且設定頁該列來源標示為 **DB** |
| **T−7**          | 確認上台兩位的公司 Google 帳號，寫入 `DEMO_EMAIL_EMP005` / `_EMP006`          | 兩個 email 已交給工程，且與 Google 帳號完全相同                     |
| **T−5**          | **手機連得到 demo 站台**（隧道或區網 + 憑證，計畫書 §10.1）                   | 兩支手機都能開啟且網址列是 https                                    |
| **T−5**          | **場勘 + 座標校準**（§2）；**實地走一次 P2**，定出圍欄半徑與精度門檻（§10.2） | `DEMO_SITE_A_LAT` / `_LNG` 已寫入；走出去確實被拒                   |
| **T−5**          | **手機畫面投影實測**（§10.5）                                                 | 會場看得到手機畫面                                                  |
| **T−5**          | 圍欄不重疊驗證                                                                | seed 斷言通過                                                       |
| **T−3**          | seed 全量跑一次，**人工核對四張畫面**（§4）                                   | §4 全綠                                                             |
| **T−3**          | **錄製完整流程螢幕錄影**（Plan C）                                            | 影片檔已存**本機**                                                  |
| **T−1**          | 完整彩排，**用當天的裝置與網路**                                              | §5 全綠                                                             |
| **T−0 早上**     | 重跑 seed、確認日期參數                                                       | §4 全綠                                                             |
| **T−0 前 30 分** | 現場測定位、確認距離顯示                                                      | 顯示「可打卡」                                                      |
| **T+1**          | 清理（§9）                                                                    | `Deprecated` 已處理或已建 issue                                     |

> **T−5 的場勘不可省。** 它是唯一能在事前發現「這個場地根本定位不到」的機會，而那件事在台上無解。

| 角色     | 職責                                                                                        |
| -------- | ------------------------------------------------------------------------------------------- |
| **主講** | EMP005 張文彬（工地主任）。敘事、第一位打卡、**走出圍欄示範 P2**、投影程式碼                |
| **助手** | EMP006 李冠廷（工地工程師）。第二位打卡、**全程盯著自己的螢幕確認畫面狀態**、必要時執行備援 |

> **助手最重要的任務不是打卡，是在主講切換畫面之前先看過一次。**
> 若那一頁不正常，用一句話讓主講改變順序，而不是讓四十個人一起看著空白畫面。

---

## 🛰️ 2. 座標校準（最重要的前置）

### 2.1 為什麼不能用地圖標註值

不是精度問題，是**系統性偏移**：地圖標的是建物中心，演示時人站在某層樓的某個房間；
室內定位主要靠 Wi-Fi 輔助，**同一台裝置在同一位置會穩定偏向某個方向**，不是隨機誤差。
兩者疊加可以吃掉 500 公尺圍欄的大半餘裕。

### 2.2 校準程序

**必須用演示當天要用的那台裝置、那個瀏覽器、站在演示要站的位置。**
不同裝置的定位來源不同（有無 GPS 晶片、Wi-Fi 資料庫差異），**別台裝置量的值不能用**。
手機地圖 App 顯示的座標也不能用 —— 那經過廠商修正，與 `navigator.geolocation` 不是同一個值。

| 步  | 動作                                                                     |
| --- | ------------------------------------------------------------------------ |
| 1   | 在演示位置開啟打卡頁，**等 30 秒讓定位穩定**（剛開頁的第一筆通常最不準） |
| 2   | 連續取樣 **5 次**，每次間隔 10 秒                                        |
| 3   | **取中位數**，不是平均 —— GPS 偶爾噴離群值，平均會被拉走，中位數不會     |
| 4   | 記下 5 筆的精度，**最差那個 × 2** 就是半徑下限參考                       |
| 5   | **走到會場／工區各角落各量一次**，取最遠一點到中位數中心的距離           |
| 6   | 半徑 = 步驟 5 + 步驟 4 + 100 公尺餘裕，四捨五入到 50 的倍數              |
| 7   | 三個值都寫進 `.env`：`DEMO_SITE_A_LAT` / `_LNG` / `_RADIUS`（半徑未設定時保底 500）|
| 8   | 執行圍欄不重疊驗證（展示資料 §2.2），特別檢查與板橋本部的距離            |

> **步驟 6 算出來超過 500 就用算出來的值，不要硬套 500。**
> 500 是起始值不是必須遵守的數字。**唯一不能發生的事是主角站在現場打不了卡。**

**取值方式**：專用校準頁未實作。實務做法是開瀏覽器 console 讀
`navigator.geolocation.getCurrentPosition(p => console.log(p.coords))`，
或直接看打卡頁顯示的「定位精度約 XX 公尺」與「距工區 XX 公尺」判斷偏移。

> **合併前必做（不是演示當天）**：`maplibre-gl` 由 v6 降回 v5 是**全站相依性變更**，
> 受影響的不只簽到 —— 物流路線圖與碳足跡計算機也用同一套。三處都要人工開一次確認底圖、
> 圖層與 `fitBounds` 正常（自動化測試只擋得住版本宣告，擋不住實際渲染）。

### 2.3 校準當天順便確認三件事

1. **會場有無 Wi-Fi、要不要密碼** —— 只靠 4G 時室內定位精度會明顯變差
2. **HTTPS 是否正常** —— `navigator.geolocation` 在非 HTTPS（localhost 除外）根本不會啟動，症狀是「一直定位中」
3. **能否連到 `accounts.google.com` 與 `www.googleapis.com`** —— 後者是 JWKS 驗簽用的，
   擋掉會讓登入在**最後一步**失敗（前面的授權全都會成功，所以看起來像帳號問題）

---

## ⚙️ 3. 環境設定與「壞掉時長什麼樣」

**右欄比設定本身重要** —— 症狀能反推回原因，才有可能在 30 秒內判斷是哪裡出問題。

**設定分兩處**：Google 憑證在**資料庫**（`/admin/settings`，§3.2），其餘在 `.env`（§3.3 說明為什麼搬不了）。

| 設定                                               | 位置   | 沒設定的症狀                                                                                                                                                                                               |
| -------------------------------------------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GOOGLE_OAUTH_CLIENT_ID` / `_CLIENT_SECRET`        | **DB** | 打卡頁的登入卡顯示**「Google 登入尚未設定」**（閘門自己查過 provider，不會只留一塊空白）                                                                                                                   |
| Google Console 的 redirect URI                     | Google | 授權後跳轉失敗，Google 端顯示 `redirect_uri_mismatch`                                                                                                                                                      |
| `DEMO_EMAIL_EMP005` / `_EMP006` 與實際登入帳號不符 | `.env` | **登入成功，但每一頁都顯示「尚未對應到員工檔」**（§3.1）                                                                                                                                                   |
| `HR_PII_KEY_V1` / `HR_PII_BLIND_INDEX_PEPPER`      | `.env` | **seed 直接中止**；若跳過 seed，打卡回 500（`HrPiiKeyError`）                                                                                                                                              |
| `NEXT_PUBLIC_APP_URL`（HTTPS）                     | `.env` | 打卡頁**一直停在「定位中」**，且沒有任何錯誤訊息                                                                                                                                                           |
| `DEMO_SITE_A_LAT` / `_LNG`                         | `.env` | seed 中止（刻意設計）                                                                                                                                                                                      |
| `DEMO_SITE_A_RADIUS`                               | `.env` | 不中止，**靜默保底 500 公尺** —— 若演示要走出圍欄示範被拒，500 公尺走不出去。這是唯一一個「沒設定也不會報錯、但演示會做不成」的參數 |
| `NEXT_PUBLIC_MAPTILER_KEY`                         | `.env` | **打卡頁與現場頁**的地圖各換成一行說明，**其餘功能不受影響**（刻意設計，見 §7）。金鑰**錯的或被網域限制擋住**也是同一條路徑 —— 前端會先問一次 style.json，401/403 就直接降級，不會留下一張沒有街道的空白圖 |

### 3.1 登入身分與員工檔的綁定

**Google 首登會自動綁定，不需要任何人工步驟。**
`AttendanceIdentityService.linkOnFirstLogin` 拿 `UserIdentity` 裡**已驗證的信箱**
比對 `Employee.email`，命中即寫入 `Employee.userId`。之後每一次都直接走那個外鍵，
不再碰信箱 —— 公司信箱可以變更，而打卡歷史不該跟著飄移。

因此 T−7 唯一要做的是：**確認 `DEMO_EMAIL_EMP005` / `_EMP006` 與上台者實際登入的
Google 帳號完全相同**（含大小寫）。

#### 三件會讓自動綁定失效的事

1. **用個人 Gmail 登入** —— `Employee.personalEmailCipher` 是密文、DB 端查不了，
   永遠對不上。這是 ADR 018 §7 的必然結果，不是 bug
2. **同帳本內兩筆員工檔的信箱只差大小寫** —— 系統擋住不猜（`CF_EMPLOYEE_EMAIL_AMBIGUOUS`），
   任選一筆綁定就是讓某人以另一個人的身分打卡
3. **該員工已經綁在別的 `User` 上** —— `CF_EMPLOYEE_ALREADY_LINKED`。
   `oauth.service` **絕不用 email 自動合併既有帳號**，所以同一個人的 passkey 帳號與
   Google 帳號是兩個不同的 `User`

#### 救援工具（現場對不上時）

`scripts/seed/link_employee_user.ts` 仍然保留，它是唯一能在 30 秒內手動綁定或解綁的方式：

```bash
# 預演（唯讀，不寫入任何東西）
npx tsx scripts/seed/link_employee_user.ts

# 指名綁定：address 取自登入後 localStorage 的 user_address
npx tsx scripts/seed/link_employee_user.ts --employee-no=EMP005 --address=0x… --commit

# 解綁（彩排後換人、或第 3 種情況的處置）
npx tsx scripts/seed/link_employee_user.ts --unlink --employee-no=EMP005 --commit
```

> **`seed_attendance_demo.ts` 會清空並重建員工檔，`Employee.userId` 一併歸零。**
> 走 Google 自動綁定的話這不是問題（下次登入會重新綁上），
> 但如果你用過 `--commit` 手動綁定，重跑 seed 之後要再綁一次。

### 3.2 把 Google 憑證寫進系統設定（`/admin/settings`）

**這是唯一的路徑。** `.env` 也讀得到這兩個值，但那是「DB 從沒被用過」時的 fallback ——
一旦 DB 有了設定，env 就再也不會被看一眼（讀取優先序：**DB 已驗簽 > env > 程式碼保底值**，ADR 017）。
把憑證同時放兩處只會製造「改了 env 卻沒有生效」這種最難查的問題。

**前置條件**：SUPER_ADMIN 的 **passkey 實體裝置**在手上。託管帳號簽不了 ——
`custodial_signing.service.ts` 對 `ADMIN_ACTION` 用途的 challenge 是硬拒絕的。

**步驟**

1. 以 SUPER_ADMIN 登入 → `/admin/settings`
2. **等頁面把現有值全部載出來再動手**（理由見下方陷阱 1）
3. 在 `THIRD_PARTY_LOGIN` 群組填入 `GOOGLE_OAUTH_CLIENT_ID` 與 `GOOGLE_OAUTH_CLIENT_SECRET`
4. 按「簽章並儲存」→ 通過 passkey 驗證
5. **驗收**：該列的來源標示變成 **DB**；打卡頁登出後看得到 Google 按鈕

#### 三個會咬人的地方

| #   | 陷阱                                         | 後果                                                                                                                                                                                                     |
| --- | -------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **這是全量取代，不是差異更新**               | 簽的是「整組設定的最終狀態」的 digest。2026-08-10 發生過一次真實事故：**一張還沒載入完的空白設定頁，把既有的 Google OAuth 與 `LLM_MODEL` 整組覆蓋掉**。`system_setting_write_guard.test.ts` 就是為它寫的 |
| 2   | **半套的寫入比不寫更糟**                     | DB 有資料但驗簽不過 → 快照 `UNTRUSTED` → `get()` **直接 throw 且不退回 env**（刻意的防回滾設計）。症狀是登入卡顯示「無法確認可用的登入方式」，而且此時 `.env` 裡放什麼都救不了                           |
| 3   | **只放在 `.env` 的 secret 不在簽章覆蓋範圍** | 設定頁會對這種欄位顯示「env-only」警告。它能用，但沒有完整性保證                                                                                                                                         |

> **不要在演示前一晚第一次嘗試這條路。** 陷阱 2 的失敗狀態會讓登入完全不可用，
> 而唯一的復原方式是把 DB 的設定重新簽對 —— 那需要 passkey 與時間。

### 3.3 這些留在 `.env`，而且搬不了

| 設定                                                        | 為什麼不能進 DB                                                                                                       |
| ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| `NEXT_PUBLIC_*`（`APP_URL`、`MAPTILER_KEY`）                | **build-time 內聯**進前端 bundle，執行期才讀的 DB 來不及。**改完一定要重啟 dev server**                               |
| `HR_PII_KEY_V1` / `HR_PII_BLIND_INDEX_PEPPER`               | 它們保護的是 **DB 欄位內容**。放進被自己保護的東西裡面，等於沒有保護（ADR 018 §5，與 `SECRET_VAULT_MASTER_KEY` 同理） |
| `DEMO_SITE_A_LAT` / `_LNG` / `_RADIUS`、`DEMO_EMAIL_EMP005` / `_EMP006` | 是 **seed 腳本的輸入參數**，不是 `SystemSettingKey`。它們在 seed 執行的那一刻就用完了。五個都只以**註解**記載於 `.env.example` —— 寫成鍵值會讓沒設它們的既有部署掉進「尚未初始化」（`env_example_contract.test.ts` 擋著） |

**需要對外連線**：`accounts.google.com`、`oauth2.googleapis.com`、`www.googleapis.com`（JWKS 驗簽）、
MapTiler（底圖，掛掉不影響任何功能）。
**備援：手機熱點** —— 最便宜也最有效的一招，但要事先確認熱點也連得上 demo 站台。

### 三個最難診斷的症狀

| 症狀                                 | 為什麼難                               | 判別方式                                                                                                                             |
| ------------------------------------ | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| **打卡回 500**                       | 錯誤訊息不會說是金鑰問題               | 看 server log 有沒有 `HrPiiKeyError`。**無法在台上修**                                                                               |
| **一直「定位中」**                   | 沒有錯誤、沒有逾時提示                 | 先看網址列是不是 `https://`；再看有沒有出現權限詢問                                                                                  |
| **登入到一半失敗**                   | 看起來像帳號問題，其實是網路           | 會場擋 `www.googleapis.com` 時，前面的授權都會成功，只有最後驗簽失敗。切**手機熱點**重來                                             |
| **地圖只剩一片底色、連圍欄圈也沒有** | 看起來像「還在載入」，沒有任何錯誤訊息 | 若 `<Marker>`（星星與藍點）看得到、其餘都不見，那是 **maplibre-gl 被升到 v6**（計畫書 §5 第 7 條）。`npm ls maplibre-gl` 必須是 5.x |
| **改了程式碼但畫面完全沒變**         | 沒有錯誤，容易誤判成功能沒做出來       | 比對 `.next/BUILD_ID` 與原始檔的 mtime；**重啟 dev server**（計畫書 §5 第 20 條）                                                    |

---

## ✅ 4. 資料就緒檢查（T−3 與 T−0 各做一次）

跑完 seed 後**用眼睛看四張畫面**，不要只看 log 說成功。

**打卡頁**（以 EMP005 登入）

- [ ] 顯示的姓名與工號是**張文彬 / EMP005**（這種錯誤只有看畫面才會發現）
- [ ] **地圖有畫出來**，圍欄圈與自己的藍點都看得到（沒畫出來 → 檢查 `NEXT_PUBLIC_MAPTILER_KEY`；不影響打卡，但 P2 少一半衝擊）
- [ ] 顯示今日班別「工地日班 07:30–17:00」
- [ ] 顯示「距 大漢溪橋梁改建工程 工區 XX 公尺　可打卡」，且**距離 < 100 公尺**（大於就代表校準有問題）

**現場頁**

- [ ] **四個工地都出現在地圖上**，圓圈彼此不重疊
- [ ] 大漢溪工區「在班 0」；全處「未到工 2」（就是上台兩位）、「今日請假 2」（王雅琪、鄭俊翔）
- [ ] 台北港工區「在班 2 ／ **未打下班卡 1**（許家豪）」
- [ ] 本部在班 4
- [ ] 名單上**沒有任何座標欄位**

**排班月曆**

- [ ] 許家豪那一列是夜間施工班，與其他人顏色不同
- [ ] 8/7 現場人員整欄是停工（借用的 `HOLIDAY` 色）
- [ ] 點任一格，編輯面板出得來

**出勤總覽**（逐格對照展示資料 §7.3）

- [ ] 8/5 李冠廷 🔴（遲到 45 ＋工時不足 15，**點開要看得到兩筆**，格子右上角有小點）
- [ ] 8/6 鄭俊翔 🟠（早退 20 分，**只有一筆**）｜8/6 周欣怡 🟡（工時不足 90，**不遲到不早退**）
- [ ] 8/7 現場人員整欄停工｜8/8 張文彬與李冠廷 ⬜（假日搶修，**無異常**）
- [ ] 8/11 王雅琪 ⚫（曠職）
- [ ] **8/12 林淑芬 🔴 47 分 與 周欣怡 ✅ 並排可見**（P4 靠這一組）
- [ ] 8/12 許家豪 🔵、賴世昌 🔵
- [ ] 8/13 整欄為 `·`（今日未結束）

> **這一節是唯一能在事前發現「演示到一半沒有例子可指」的機會。**
> 一片綠色的方格圖什麼也證明不了，而那件事在台上才發現就來不及了。

---

## 🎭 5. 彩排（T−1，用當天的裝置與網路）

完整跑一次 §6 每一步，**不要跳過任何一步**。

| #   | 項目                                                         | 常見問題                                                   |
| --- | ------------------------------------------------------------ | ---------------------------------------------------------- |
| 1   | 從**登出狀態**開始                                           | 開發時瀏覽器有 session，正式演示沒有 —— 沒彩排過會卡在登入 |
| 2   | 兩台裝置分別登入不同的 Google 帳號                           | 同一瀏覽器登兩個 Google 帳號會互相踢掉                     |
| 3   | 現場頁在第二人打卡後**確實更新**（15 秒輪詢）                | 要在台上等 15 秒，先知道就不會慌                           |
| 4   | **實地走出圍欄一次**，確認幾步之後會被拒絕、走回來又可以打卡 | 半徑與精度門檻要用**當天那兩支手機**實測                   |
| 5   | **走回圍欄內後畫面確實恢復「可打卡」**                       | 定位有延遲，要知道大概等幾秒                               |
| 6   | 要投影的程式碼**已開好在另一個視窗**                         | 台上現找檔案會斷掉節奏                                     |
| 7   | 全程計時                                                     | 目標 9 分鐘；超過 12 分鐘要砍掉第二段的某幾步              |

**彩排後把螢幕錄影存下來** —— 這就是 Plan C。

---

## 🎬 6. 演示當天操作手順

> 台詞見計畫書 §6。本節只列**操作**與**前置確認**。

**演示前 30 分鐘**：① 打卡頁確認「可打卡」且距離 < 100 m　② 現場頁確認未到工 2、今日請假 2
③ 出勤總覽確認 8/12 紅綠對照看得到　④ 程式碼視窗開好捲到判定區塊
⑤ **登出，停在 `/hr_management/attendance`** —— 該頁未登入時直接顯示登入卡，不必先繞到登入頁

> ### P2 改用「真的走出去」（全程手機演示，見計畫書 §10.2）
>
> 手動座標輸入框已移除，而手機沒有 DevTools 感應器面板。
> 因此 demo 工區的圍欄半徑改小到 **60–80 公尺**，主講走出去 20–30 步即被拒絕，約 30 秒。
>
> **這比偽造座標更有說服力** —— 觀眾看到的是一個人真的走出範圍，而不是一個輸入框。
>
> 台詞要誠實：「這個工區我們登記的範圍是 60 公尺，實際案場會依工區大小量測」。
> **不要說「系統的圍欄是 60 公尺」** —— 那會讓觀眾以為 500 公尺那個數字是假的。
>
> ⚠️ 半徑改小之後精度門檻相對變嚴，`DEMO_MAX_ACCURACY_METERS` 要一起調降（計畫書 §10.2）。
> **它是 `src/constants/attendance.ts` 的常數，不是環境變數** —— 改它要動程式碼並重新 build，
> 不能像半徑那樣在 T−5 現場改 `.env` 就算數。**這一項必須在 T−5 校準後、彩排前完成。**

### 第一段

| 步  | 操作                                                                                           | 前置確認                                                      |
| --- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| 1   | **在打卡頁上**點「使用 Google 登入」→ 選帳號 → **自動回到打卡頁**                              | 按鈕存在（換成黃底警告就是 OAuth 未設定）；帳號**不是**管理員 |
| 2   | 停在打卡頁，唸出班別與距離                                                                     | 顯示「可打卡」                                                |
| 3   | 按「上班打卡」→ **確認對話框**按確定 → **自動跳到現場狀態頁**                                  | 頁首的秒錶有在跳（那是伺服器時間，與紀錄一致）                |
| 4   | （已在現場頁）指三個數字                                                                       | 自己已從未到工變成在班                                        |
| 5   | **邊講邊往外走** → 指出地圖上藍點跨出圈外、距離數字變大 → 按打卡 → 403                         | 已實地走過一次，知道要走幾步                                  |
| 6   | 切現場頁，指出名單**沒有變**                                                                   | —                                                             |
| 7   | 請助手打卡 → 等 15 秒 → 名單多一人                                                             | 助手已登入完成                                                |
| 7b  | 按「匯出點名單」，打開 CSV 指出時間戳與產出者                                                  | —                                                             |
| 7c  | 指台北港工區的 🟡 許家豪                                                                       | —                                                             |
| 7d  | **指「今日請假 2」** → 點 EMP008 鄭俊翔 → 發起銷假徵詢，填「颱風後搶修，第一工區缺兩名工程師」 | 自己是 DEP-001 主管（一般員工看不到這個入口）                 |
| 7e  | **切到助手的裝置** → 徵詢卡出現 → 按「同意銷假」                                               | 切回瀏覽器會立刻重取，不必等滿 15 秒                          |
| 7f  | 回現場頁：EMP008 從「今日請假」移到「未到工」                                                  | 15 秒輪詢                                                     |
| —   | **走回圍欄內**                                                                                 | ← 後面的步驟都要在圈內才正常                                  |

### 第二段

| 步  | 操作                                                    |
| --- | ------------------------------------------------------- |
| 8   | 排班月曆：指許家豪的夜班列與 8/7 停工欄                 |
| 9   | 出勤總覽：指 8/12 林淑芬 🔴 47 分 與 周欣怡 ✅          |
| 10  | **切到程式碼視窗**，指出沒有 `if (shiftType)`           |
| 11  | 回總覽，點開周欣怡 8/6 → 工時不足 90 分                 |
| 12  | 指王雅琪 8/11 曠職                                      |
| 12b | 指許家豪 8/12 漏打下班卡，**切回現場頁**指出他此刻是 🟡 |
| 13  | 回總覽，指 8/13 整欄的 `·`                              |
| 14  | 指 8/7 停工欄、8/8 假日搶修的無異常格                   |
| 15  | 收尾：補登單在正式版                                    |

---

## 🚨 7. 現場故障對照表

> **左欄是你在台上看到的，右欄是 30 秒內能做的。** 「無法現場修」直接跳 §8 備援。

| 症狀                                              | 最可能原因                                                                                | 現場處置                                                                                                                                                                     |
| ------------------------------------------------- | ----------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 登入卡顯示**「Google 登入尚未設定」**             | `/admin/settings` 裡沒有這兩個值，或整組設定被覆蓋掉了（§3.2 陷阱 1）                     | ❌ 需要 SUPER_ADMIN passkey 才修得了 → **Plan B**                                                                                                                            |
| 登入卡顯示**「無法確認可用的登入方式」**          | 設定快照 `UNTRUSTED` —— 簽章驗不過，而**此時 env 的 fallback 也被關掉了**（§3.2 陷阱 2）  | 看伺服器記錄有無 `IS_SETTING_STATE_UNTRUSTED`。❌ 現場修不了 → **Plan B**                                                                                                    |
| Google 按鈕是**灰的**，上面一行琥珀色字           | 目前的網址不是 `NEXT_PUBLIC_APP_URL` —— OAuth 只能在那個 origin 上完成                    | 改用手冊指定的正式網址重開                                                                                                                                                   |
| 授權後 `redirect_uri_mismatch`                    | Google Console 的 redirect URI 不符                                                       | ❌ 無法現場修 → **Plan B**                                                                                                                                                   |
| **登入後沒有回到打卡頁，跳到 `/admin/dashboard`** | 該帳號的 `Role` 是 ADMIN／SUPER_ADMIN —— callback 對管理員會忽略 `returnTo`               | 手動輸入網址回打卡頁；**演示帳號不要用管理員**                                                                                                                               |
| 登入到最後一步失敗                                | 會場擋 `www.googleapis.com`（JWKS 驗簽）                                                  | 切**手機熱點**重來                                                                                                                                                           |
| **登入成功但顯示「尚未對應到員工檔」**            | 登入的信箱 ≠ `Employee.email`                                                             | 有 DB 存取就用 `link_employee_user.ts --employee-no=… --address=… --commit`（約 30 秒）；否則換另一位上台者 → **Plan B**                                                     |
| **打卡回 403「距離過遠」但人在現場**              | 校準值錯 / 裝置定位偏移 / 半徑調太小                                                      | ① 走到窗邊重試　② 走近圓心幾步　③ → Plan C                                                                                                                                   |
| **打卡回「定位精度不足，請重試」**                | 室內收訊差，精度 > 200 公尺                                                               | 走到窗邊或戶外重試。**這不是拒絕他到班，是證據品質不足** —— 可以順口講這個區別                                                                                               |
| **一直停在「定位中」**                            | 非 HTTPS / 權限未授權                                                                     | 看網址列鎖頭；重新整理後在權限詢問按「允許」                                                                                                                                 |
| **打卡回 500**                                    | `HR_PII_KEY_V1` 未設定                                                                    | ❌ 無法現場修 → **Plan C**                                                                                                                                                   |
| **現場頁是空的**                                  | 今日打卡未 seed / seed 跑在錯的帳本                                                       | 若有 DB 存取可重跑 seed（約 30 秒）；否則直接講第二段                                                                                                                        |
| **名單沒有即時更新**                              | 15 秒輪詢還沒到                                                                           | 等一下；或手動重新整理                                                                                                                                                       |
| **看板頂端出現「顯示的是上一次成功取得的資料」**  | 網路抖動，輪詢失敗                                                                        | 畫面上是舊資料但**有標示**。等下一輪；持續失敗就切熱點                                                                                                                       |
| **未打下班卡沒有出現**                            | 許家豪的夜班打卡未 seed                                                                   | 跳過步驟 7c 與 12b，其餘照跑                                                                                                                                                 |
| **出勤總覽一片綠色**                              | 歷史異常未 seed                                                                           | ❌ 第二段大部分演不了 → Plan C                                                                                                                                               |
| **地圖沒畫出來（兩頁皆是）**                      | MapTiler 金鑰錯誤／被網域限制／連不到（瀏覽器 console 會有 `MapTiler style unavailable`） | **不必處理** —— 兩頁的地圖區塊都會自動換成一行說明，打卡、人數、名單、匯出全部照常。P2 改用文字演（「距工區 3.2 公里」），少一點衝擊但主張不變。**距離才是主張，地圖是佐證** |
| **觀眾拿自己手機想試打卡**                        | 該帳號沒有對應的員工檔                                                                    | ✅ **這是好事，見下**                                                                                                                                                        |

### 7.1 把最常見的「故障」變成賣點

觀眾很容易當場拿出手機想試。他們會註冊、登入成功，然後看到「尚未對應到員工檔」。

**不要道歉，把它講成設計：**

> 「對，這正是實名制的意思 —— 不在名冊上的人打不了這個工地的卡。工地上有誰，是事先建檔決定的，不是誰下載了 App 就能報到。」

這一句話同時回答了工程機關最關心的「承包商能不能亂報人頭」，而它是免費的。

### 7.2 台上的三條紀律

1. **不要在演示中重跑 seed** —— 會清掉現場剛打的那兩筆卡，而那兩筆是整場最有說服力的資料
2. **不要現場改圍欄半徑** —— 改大了 P2 就演不了（走到哪都算在工地內），而且要重新解釋一次
3. **不要用沒彩排過的裝置** —— 定位偏移是裝置相關的，換一台等於校準作廢

---

## 🛟 8. 備援分級

**Plan C 必須在 T−3 就準備好。** 它是最便宜的保險，而需要它的時候一定來不及做。

| 方案       | 觸發條件       | 內容                                                       | 事前準備                                                                                                                                       |
| ---------- | -------------- | ---------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| **Plan A** | 一切正常       | 完整現場演示                                               | —                                                                                                                                              |
| **Plan B** | 登入相關故障   | **事先登入好、放著不關的兩個分頁**，跳過登入直接從打卡開始 | 演示前 30 分鐘登入好兩台裝置，**不要登出、不要讓裝置休眠**。登入卡上的 Passkey 備援**救不了 EMP005／EMP006**（見下），所以這仍是唯一的登入備援 |
| **Plan C** | 定位或後端故障 | 播放 T−1 彩排的**螢幕錄影**，主講在旁同步解說              | **T−3 錄好，存本機硬碟** —— 需要它的時候通常就是網路壞掉的時候                                                                                 |
| **Plan D** | 完全無網路     | 四張畫面的**截圖**，講概念不演操作                         | 與 Plan C 一起準備                                                                                                                             |

> ### 登入卡上的 Passkey 備援，救不了 EMP005／EMP006
>
> Passkey 是唯一**不需要外部網路**的登入路徑，所以它留在登入卡下方 —— 但它對這兩位無效。
>
> `Employee.userId` 只能指向一個 `User`，而 `oauth.service` **絕不用 email 自動合併既有帳號**：
> 同一個人的 passkey 帳號與 Google 帳號是兩個不同的 `User`。
> 若事先註冊 passkey 並手動綁定，Google 首登就會撞 `CF_EMPLOYEE_ALREADY_LINKED`；
> 若留給 Google 自動綁，passkey 帳號登入後就是「尚未對應到員工檔」。**兩者互斥。**
>
> 真的想要一條不靠 Google 的活路，做法是拿一位**備用員工**（例如 EMP007）註冊 passkey 並手動綁定，
> Google 掛掉時改用那個身分演打卡 —— 敘事要換一個角色，但打卡、現場名單、銷假徵詢全部照常。
> 要不要做，建議 T−1 彩排確認會場網路之後再決定。

---

## 🧹 9. 演示後

**立即**

- [ ] 記錄現場提出的問題 —— 尤其**線形工程圍欄**、**停工日**、**外勤與補登**這三題（計畫書都有對應段落）

**T+1**

- [ ] 處理剩下的 `Deprecated:` 標記：`DEMO_ACCOUNT_BOOK_ID`、CSV 欄位標題（手動座標路徑與 `DEMO_GEOFENCE_RADIUS_METERS` 已移除）
- [ ] 若要延續為正式開發，**第一個 sprint 是權限矩陣**（計畫書 §7.3 第 1 順位）—— demo 的**讀取**沒有範圍控制，登入者看得到全帳本名單與所有人的判定結果（**寫入**已限部門主管，但那只是收窄，不是矩陣）
- [ ] demo 資料庫**不得**匯入任何真實員工個資；若已有真人 email，評估是否清除
- [ ] 若 demo 資料庫可能留作測試環境，**PII 金鑰需重新產生並納入正式的密鑰管理**

**值得記下來的兩件事**

1. **校準值與實際半徑** —— 這是正式版「實地量測」的第一筆資料，不要弄丟
2. **哪一步觀眾有反應、哪一步沒有** —— 下一場的取捨依據

---

## 📋 附錄：演示當天單頁檢查表

```
□ 打卡頁：顯示「可打卡」，距離 < 100 m
□ 打卡頁：地圖有畫出來，藍點在橘色圈內
□ 打卡頁：班別顯示「工地日班 07:30–17:00」
□ 現場頁：四個工地都有人、圓圈不重疊
□ 現場頁：全處「未到工 2」（就是上台兩位）、「今日請假 2」
□ 現場頁：沒有任何「徵詢中」標記（那一筆要現場產生）
□ 現場頁：台北港工區「未打下班卡 1」
□ 排班頁：許家豪的夜班列看得出來
□ 總覽頁：8/12 那一欄有紅有綠（P4 對照組）
□ 總覽頁：8/11 王雅琪為曠職
□ 總覽頁：8/13 整欄為「·」
□ 兩支手機：精確位置已開、低耗電模式已關、螢幕不自動鎖定
□ 兩支手機：用的是 Safari／Chrome 本身，不是 App 內建瀏覽器
□ 手機連得到 demo 網址，且網址列是 https（§10.1）
□ 投影：手機畫面已實測投得出去（§10.5）
□ 程式碼視窗：已開好，捲到判定區塊
□ 兩台裝置：都已登出，且停在打卡頁（看得到 Google 按鈕，且不是灰的）
□ 上台的兩個 Google 帳號**都不是**管理員（否則登入後會跳去 /admin/dashboard）
□ 兩台裝置：網址列是 `NEXT_PUBLIC_APP_URL` 指定的那一個
□ Plan B：兩個已登入的分頁備著
□ Plan C：彩排影片在本機打得開
□ 計時器：準備好
```

---

> **相關文件**：`attendance_demo_plan.md`｜`attendance_demo_mock_data.md`｜`time_attendance_module_plan.md`（母文件 v1.3）
