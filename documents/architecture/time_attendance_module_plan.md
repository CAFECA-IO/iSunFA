...
| G6 | 限流 bucket `ATTENDANCE_PUNCH` | 腳本連續打卡 | ❌ `RATE_LIMIT`（429） |
...
1. **匯出必寫 `AuditLog`**（`EMPLOYEE_PII` / `READ`，`dataId` 填操作者的 `Employee.id`）—— 它一次讀出全帳本的在班名單，是本模組批次讀取量最大的動作
...
> **失敗的打卡也要計入限流。** 這是「拒絕」帶來的新攻擊面：既然圍欄判定在伺服器端，攻擊者唯一的手段就是**用不同座標反覆嘗試直到猜中圍欄位置**。若只對成功的打卡限流，這條路完全暢通。每日 40 次的上限對正常使用者綽綽有餘（含失敗重試），對枚舉攻擊則遠遠不夠。

現場人數、名單與地圖沿用既有 `READ` bucket（120/min），足以支撐 30 秒輪詢。
