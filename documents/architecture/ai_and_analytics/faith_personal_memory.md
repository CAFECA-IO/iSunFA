...
> **Author**: Luphia
> **Version**: 1.0 (Draft)
> **Status**: Implemented (minimal) — 任務短期記憶與長期記憶最小方案於 2026-08-17 上線；語意去重（§4.2）與後台觀測介面（§8）未實作
...
|---|---|---|---|
| 0 | 記憶分兩層：**任務短期記憶**（同一任務內記得前後文，各方案皆有）與**長期記憶＋回饋學習**（付費訂閱專屬） | 兩層都要實作：短期記憶＝同一任務內的對話上下文（**不落地儲存**，不受 90 天條款拘束）；長期記憶＝本規範 §3 起的儲存對象 | §2 事實 1、§3–§7 |
| 1 | 每位成員擁有**獨立**的費思代理人，不與其他成員或客戶共用 | 長期記憶以 `(userId, teamId)` 為鍵隔離；Repo 層強制帶鍵，無「查全部」入口；組 prompt 時只帶本人記憶 | §3、§6 |
...
1. **上界是硬的，且與預扣同源**。截斷後的字元數由 `buildShortTermHistory` 一併回傳，估算與注入用的必須是同一份——分開算就會出現「估的是 A、送的是 B」，而 hold 一旦小於實耗，`settleSpend` 的「只退不補」前提就破了（§2 事實 3 對長期記憶提出的同一個修正，短期記憶同樣適用）。以 4,000 字元計，每則訊息最多多扣約 2 點。
   ...
   **修訂（2026-08-18，第三輪 D）**：`totalChars` 回的是 **`renderShortTermHistory()` 輸出的長度**，不是各則訊息內容長度之和。兩者相差兩行標頭與每輪的 `User: ` / `Assistant: ` 前綴（十輪滿載約 180 字元）——先前回的是後者，也就是「估的是 A、送的是 B」本身。`FAITH_PROMPT_OVERHEAD_TOKENS = 600` 大概吸收得掉，但那是巧合，不是設計。內容長度仍是**截斷預算**（`FAITH_HISTORY_MAX_CHARS`），注入長度才是**預扣依據**，兩者刻意分開。
...
### 2.2 長期記憶最小方案的實作（2026-08-17）

條款 §3.7 與隱私政策 §5 承諾的三件事——**存在、依方案提供、90 天後刪除**——現在都成立了。實作與本規範的差異記於此：

| 規範 | 最小方案 | 說明 |
|---|---|---|
| §4.2 語意相似度以外的去重 | ✅ 正規化字面雜湊 | 規範本來就排除語意比對（不決定論），此處一致 |
| §4.2 上限與 LRU 淘汰 | ✅ 50 條，淘汰 `updatedAt` 最舊者 | |
| §4.2 禁記數值型事實 | ✅ **含任何數字即丟棄** | 以確定性規則攔截，不靠 prompt 自律 |
| §5 注入預算與預扣加計 | ✅ 1,200 字元硬上界，計入 `estimateFaithHoldCredits` | 與短期記憶同一個理由，見 §2.1 |
| §6.2 欄位級加密 | ✅ 沿用 `key_vault` 的 `FAITH_MEMORY` 子金鑰 | 獨立子金鑰，不與簽章私鑰共用信任邊界 |
| §6.2「密文與列綁定」 | ✅ 2026-08-18 補上 GCM 的 AAD = `faith-memory:{userId}:{teamId}` | 先前只綁 purpose，authTag 擋得住竄改但擋不住**跨列複製**——把 A 的四個欄位複製到 B 的列，B 就解出 A 的偏好。AAD 為 `sealSecret` 的**選填**參數，既有的託管金鑰與系統設定密文不受影響 |
| §6.3 方案 Gate（讀寫兩側） | ✅ fail-closed | 查無訂閱 / 非 ACTIVE / 已過期一律當免費版 |
| §7.1 期限的寫入與清除 | ⚠️ **改以對帳而非掛事件** | 見下 |
...
| **P2** | **任務短期記憶（多輪上下文）** + 長期記憶萃取管線 + 注入 + **預扣估算修正** | 同一任務內多輪對話記得前後文（不落地儲存）；萃取為封閉列舉、含金額樣式的項目被攔下；`actual ≤ held` 不變式在含記憶時仍成立（單測） |
