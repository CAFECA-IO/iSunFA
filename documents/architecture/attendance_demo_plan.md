...
> **v2.2 變更摘要**（相對 v2.1）：**實作過程中回頭修正計畫書的九處**
>
...
> 9. **§8.3 排班月曆的簡稱、編輯方式與唯讀模式（W10）**：見該節的 v2.2 實作補充。
...
> **v2.2 實作補充（W10）**
>
> 1. **格子上的簡稱是算出來的，不是切出來的。** 資料裡沒有「一個字的班別代號」
>    這一欄 —— 只有 `code`（SITE-DAY）與 `name`（工地日班）。取名稱首字看起來可行，
>    直到帳本裡同時有「工地日班」與「工程師彈性班」：兩個都是「工」。
>    因此 `buildShiftLabels` 取**能讓全體互不相同的最短前綴**（一字 → 兩字 → 退回 `code`），
>    回傳的標籤集合**必定**互不相同。碰撞的症狀是「排班表看起來完全正常、
>    只是有兩欄的意思是反的」—— demo 的四個班別不會發生，客戶的八個一定會。
>    整體升級而不是只加長碰撞的那幾個：一張表上一字與兩字並存，讀的人會以為長度有意義。
> 2. **編輯改成方格圖下方的面板，不是下拉選單**（原文寫下拉）。兩個理由：
>    這張表會橫向捲動，而浮動選單在捲動容器裡的定位是一整類難查的錯誤，
>    不值得在演示當天冒；而且面板放得下班別全名與時間窗（「工地日班　07:30–17:00」），
>    28 像素的下拉只放得下簡稱 —— 投影出來要看得懂。**一樣不做拖拉。**
> 3. **無排班與休假是兩種顏色。** 前者是「這一天還沒有人排過」，後者是
>    「明確排了不用上班」。同色的話，「這個月還有幾天沒排」永遠算不出來。
> 4. **星期標頭不是裝飾。** 演示的方式是指著某一列說「這是輪班」，
>    那一秒沒有時間讓人數格子數到星期六。星期以 UTC 解析 ——
>    用本地時區解析在 UTC 以西的機器上整排會錯一格。
> 5. **部門篩選在前端做**，即使 A7 支援 `departmentId`。選項來自回傳的資料，
>    把篩選推到伺服器就會**選了某個部門之後其他選項全部消失**，再也切不回去。
> 6. **不做假的唯讀開關。** 原文的「唯讀模式給一般員工」需要權限控制，而 demo 沒有
>    （§12.3 第 1 順位）。一個擋不住任何請求的鎖頭圖示，在被問到
>    「權限怎麼控」的時候比誠實地說沒有更糟。
...
| W10 | 前端排班月曆：方格圖（唯一簡稱 + 星期標頭）+ 單格編輯面板 | P4 上半可演 | 1.0 d |
