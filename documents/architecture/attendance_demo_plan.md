...
> **v2.2 變更摘要**（相對 v2.1）：**實作過程中回頭修正計畫書的八處**
>
...
> 4. **§8.4 出勤總覽補了圖示、多重異常標記與「工時不足」統計欄（W11）**：見該節。
> 5. **§8.2「未到工」的門檻由「窗起」改為「核心起 + 遲到寬限」，且不分地點（W7）**：見該節。
> 6. **§7.2 錯誤碼 `NF000019` 改配給 `NF_WORK_LOCATION_UNKNOWN`**：見該節。
> 8. **§7.2 補 `NF_EMPLOYEE`／`NF_SHIFT_PATTERN` 兩個跨帳本檢查用的錯誤碼（W5）**：
>    `EmployeeShiftDay` 的 `employeeId` 與 `shiftPatternId` 在資料庫層
>    **沒有任何跨帳本約束** —— 少了這兩個檢查，帶一個別家帳本的 id 就寫得進去。
> 7. **母文件 §D5「標記上的數字＝在班人數」改為「在班 + 未打下班卡」（W9）**：
...
> **v2.2 實作補充（W5）**
>
> 1. **A8 的請求本體是可辨識聯集，不是「`dayType` 加一個選填的 `shiftPatternId`」。**
>    ADR 019 的判準在這裡直接適用：能讓非法狀態不可表示，就不要退而求其次
>    讓它可被拒絕。寫成聯集之後，「上班日沒帶班別」與「休假日卻帶了班別」
>    **連解析都過不了**，下游不需要再寫一次 if。
> 2. **非上班日要求 `shiftPatternId` 明確為 `null`，不接受省略。**
>    把上班日改成休假時舊班別必須被清掉，而 `undefined` 在 Prisma 的
>    `update` 裡的意思是「不要動這個欄位」—— 省略會留下一個掛著班次的休假日。
> 3. **`employeeId` 與 `shiftPatternId` 都要確認屬於本帳本。**
>    `EmployeeShiftDay` 在資料庫層對這兩者都沒有跨帳本約束，
>    光靠 id 查得到就寫下去等於租戶隔離破了一個洞 ——
>    而症狀要等到有人發現自己的班表出現沒看過的班次才會浮現。
> 4. **非法狀態擋在三層，各自服務不同的呼叫者**：TypeScript 的
>    `IDaySchedule`（編譯期）、zod 的聯集（API 呼叫者）、
>    `assertSchedulableDay`（種子腳本、資料遷移、排班表 Excel 匯入）。
>    第三層不是重複 —— 匯入正是一次寫入上千筆、最可能把兩個欄位配錯的地方。
> 5. **A7 與 A9 刻意是兩支端點**：前者是判定的**輸入**（人排的），
>    後者是**輸出**（系統算的）。排班畫面必須能在判定之外獨立存在 ——
>    下個月的班表現在就排得出來，那時還沒有任何打卡可判。
>
> **⚠️ 一個 demo 不補、但正式版必補的洞**：A8 沒有留下任何異動軌跡，
> 而它會**改寫歷史判定** —— 判定是即時算的，把 8/12 從上班日改成休假，
> 那一天的曠職就當場消失，沒有任何地方記得誰在什麼時候改的、原本是什麼。
> 出勤紀錄本身是 append-only（打卡改不了），但它的**比較基準**可以被無聲地改掉。
> 見 §12.3。
...
| `NF_EMPLOYEE` | `NF000020` | `NOT_FOUND` | A8 指定的員工不存在或不屬於本帳本（W5）|
| `NF_SHIFT_PATTERN` | `NF000021` | `NOT_FOUND` | A8 指定的班別不存在或不屬於本帳本（W5）|
| `NF_EMPLOYEE_FOR_USER` | `NF000017` | `NOT_FOUND` | Google 帳號對不到員工檔（§2.1） |
| `NF_WORK_LOCATION` | `NF000018` | `NOT_FOUND` | — |
| `NF_WORK_LOCATION_UNKNOWN` | `NF000019` | `NOT_FOUND` | A4／A10 指定的地點不存在或不屬於本帳本（W7）|
...
> **v2.2 實作補充（W9，前端）**
>
> 1. **圍欄圓圈用真實座標的多邊形，不用 maplibre 的 circle 圖層。**
>    後者的半徑單位是**螢幕像素**，縮放時涵蓋的實際範圍會一直改變 ——
>    而這個圓要回答的正是「我站在這裡打不打得到卡」。測試直接以
>    `calculateDistanceKm`（伺服器判定圍欄用的同一支）驗證每個頂點都真的
>    離圓心 500 公尺，兩邊各用一套地球半徑就會被擋下來。
> 2. **地圖標記上的數字含 `STALE`**（見上方變更摘要第 7 點）。
>    分項留給地點卡片，地圖只回答「這裡要找幾個人」。
> 3. **輪詢不清空畫面。** 這面板子會被開著一整天，每 15 秒換成轉圈圈
>    等於有八分之一的時間不能看。只有第一次載入顯示載入中；
>    更新失敗時**保留舊資料並標示它是舊的** —— 一次網路抖動就清空看板，
>    會讓人以為工地上真的沒有人。
> 4. **分頁看不見時停止輪詢，切回來立刻更新。** 回到這個分頁的人要看的是
>    「現在」，不是十四秒前。同一時間只有一輪請求在飛，否則網路慢的時候
>    畫面會在新舊資料之間跳。
> 5. **沒有 MapTiler 金鑰時只換掉地圖，不擋整頁。** 地圖是配角 ——
>    人數、名單、匯出都不需要它。會場連不上底圖時，紅色錯誤框會讓整頁
>    看起來壞掉，而其實壞掉的只有底圖。
> 6. **名單把未打下班卡的排最前面。** 依工號排比較整齊，但那份整齊會讓
>    最該被看到的三個人散落在四十個人中間。
> 7. **匯出檔名以伺服器回的 `Content-Disposition` 為準**，前端不自己組 ——
>    否則會出現「下載下來的檔名與稽核紀錄裡的對不起來」。

> **v2.2 實作補充（W7）**
>
> 1. **「未到工」的門檻改為「核心起 + 遲到寬限」，不是原文寫的「窗起」。**
>    彈性班的窗起是 07:00、核心起是 10:00 —— 以窗起為門檻，每天七點過後
>    所有彈性班同仁都會被列成未到工，而那是他們正當的彈性。
>    **一個每天早上都在報警的數字，不會有人看第二次。** 取「系統開始判遲到的
>    那一刻」才是這個數字的意思。固定班的窗＝核心，因此這個改動只影響彈性班。
> 2. **「未到工」不分地點，只有全帳本一個總數與名單。** 這些人沒有打卡，
>    系統手上沒有任何座標可以把他們歸給某個工區，而 `EmployeeShiftDay`
>    也沒有地點欄位 —— 硬要分配就是捏造。
>    **ToDo（正式版）**：排班帶上預定工區（或給員工一個預設工地），
>    屆時未到工才分得了地點，而「這個工區今天少了三個人」比
>    「全處少了三個人」對工地主任有用得多。
> 3. **沒有人的地點也要列出來、人數印 0。** 只列有人的地點，會讓
>    「這個工區今天沒有人」與「這個工區不存在」在畫面上長得一模一樣。
> 4. **現場狀態一定要撈昨天。** 夜間施工班的打卡 `workDate` 是昨天，
>    只看今天的話凌晨的看板會顯示零人。判定規則是**「最近一個有打卡的
>    工作日說了算」** —— 而不是「找第一個未關閉的段落」，否則昨天忘記
>    打下班卡、今天已正常上下班的人會被永久釘在現場名單上。
> 5. **A4（名單）不寫 `AuditLog`，A10（匯出）寫。** 前者是每 15 秒輪詢的看板，
>    每次留痕會把 `AuditLog` 沖爆 —— 那正是 `AuditLogAction.READ` 的註解
>    點名要避免的事。後者是刻意把名單帶走的動作，次數以個位數計。
> 6. **CSV 以 BOM 開頭、CRLF 換行、欄位跳脫。** 這份檔案的第一個讀者
>    幾乎必然是用 Excel 打開它的人；沒有 BOM，中文姓名全是亂碼。
>    工區名稱是人打的，一個沒跳脫的逗號會讓整份名單的欄位往後錯一格，
>    而在事故現場沒有人會發現。
...
> **v2.2 實作補充（W11）**
>
> 1. **顏色不是唯一的訊號，每一格另有圖示。** 這裡的主角正是紅與綠，
>    而紅綠是最常見的一型色覺辨識障礙 —— 純靠顏色，這張表對那些人等於空白，
>    而它在工地上是要拿來對出工的。
> 2. **一格只有一個顏色，一天卻可以有多種異常**（遲到 + 工時不足）。
>    因此有一份嚴重度順序決定誰代表這一天（曠職 > 漏打卡 > 遲到／早退 > 工時不足：
>    曠職是最大的問題，漏打卡代表工時無從計算，工時不足通常只是前兩者的結果），
>    而被蓋掉的那些以格子右上角的小點示意。**沒有那個點，
>    使用者沒有理由去點開它，而看不到的資訊等於不存在。**
> 3. **統計欄多一欄「工時不足」**（原表四欄）。彈性工時的主張正是靠它成立 ——
>    P4／P5 要講的「既不遲到也不早退，但總時數不夠」在原本四欄裡沒有位置。
>    上下班漏打卡合併成一欄（引擎判定表 #6 #7 互斥，相加不會重複計算），
>    哪一端由明細交代。
> 4. **沒有「位置異常跳動」這一欄，也不在篩選選單裡。** G5 未實作，
>    開一欄填 0 等於在報表上宣稱「查過了、沒有」；放進選單則會讓
>    選了永遠沒結果被讀成「沒有這種情況」。
> 5. **明細一定印出判定階段**（尚未開始／進行中／已結束）。
>    使用者看到空白格的第一個問題是「為什麼」，而答案有兩個且意思相反：
>    「這天還沒過完」與「這天沒有排班」。少了這一行，演示現場最容易被問到的那一格
>    正好是唯一答不出來的。
> 6. **只有換月份才重新打 API**：A9 一次回整月 × 全體（demo 規模 372 格），
>    篩選與排序都在前端完成。
...
| W5 | 班表：`attendance_schedule.service.ts` + 可辨識聯集 + 跨帳本檢查 + A6 / A7 / A8 | 排班可讀可改 | 0.75 d |
| W6 | 判定 API：即時評估組月曆矩陣 + A9（含 `phase` 與區間上限） | 出勤結果正確 | 0.5 d |
| W7 | 現場狀態：由 punch 推導 + `STALE`（正式語意）+ 未到工 + A3 / A4 / A10（CSV + AuditLog）| 名單正確、窗迄後轉黃 | 0.5 d |
| W8 | 前端打卡頁（四種定位狀態 + 今日班別 + 示範橫幅） | P1 / P2 可演 | 1.0 d |
| W9 | 前端現場頁：三數字看板 + maplibre 真尺寸圍欄 + 名單 + CSV 匯出 | P3 可演 | 1.0 d |
| W10 | 前端排班月曆（唯讀 + 單格下拉編輯） | P4 上半可演 | 1.0 d |
| W11 | 前端出勤總覽：方格圖（含圖示與多重異常標記）+ 明細 + 篩選 | P4 下半 / P5 可演 | 1.0 d |
...
| **3.5** | **排班異動軌跡（W5 實作時發現，母文件沒有這一節）** | 判定是即時算的，因此**改排班就是改歷史判定**：把 8/12 從上班日改成休假，那一天的曠職當場消失，而沒有任何地方記得誰改的、原本是什麼。打卡本身是 append-only，但它的**比較基準**目前可以被無聲地改掉 —— 對出工查核而言那是一個真正的洞。兩條做法：排班改成 append-only 加生效版本（貴，但能重建任一時點的班表），或新增排班類別的 `AuditLogDataType`（便宜，足以回答「誰改的」）|
...
| 8 | 限流、名單匯出的 `EMERGENCY_ROSTER` 權限 | 上線前必備，但不阻擋開發。**點名匯出的 `AuditLog` 已在 W7 實作**（一位被列出的員工一筆），缺的是權限與限流 |
