...
> **目標**: 演示「Google 登入 → GPS 打卡 → 現場人數與名單」與「班表 → 自動比對 → 出勤異常」兩段完整故事。

> **v2.0 變更摘要**（相對 v1.1）
>
> 依需求加入**排班、彈性工時、遲到早退曠職判定**。這不是加三個功能，是讓 demo 從「一段」變成「兩段」——
> 因此 §1 重寫為兩段式敘事、§11 演示腳本重排、時程由 5 天調整為 **10.5 天**（單人序列估算；W4 可與 W2/W3 併行，兩人分頭做前後端可壓到約 7 天）。
>
> 三個因為擴充而**變好**的地方：
...
這兩段的目標聽眾本來就不同：第一段講給營運與職安，第二段講給 HR 與管理層。

### 第一段：「我到了嗎」（約 4 分鐘）— 即時、有畫面、講現場

| # | 主張 | 演示方式 |
|---|---|---|
| **P1** | 員工用既有的公司 Google 帳號就能簽到 | 點「使用 Google 登入」→ 打卡畫面顯示自己的姓名與工號 |
| **P2** | **人不在現場就打不了卡** —— 圍欄是到班的定義，不是事後標記 | 辦公室打卡成功；切換遠處座標 → **403「距台北總部 3.2 公里」** |
| **P3** | 系統隨時答得出「現在有幾個人在現場、分別是誰」 | 現場頁：地圖 + 人數 + 名單；第二人打卡 → 名單即時多一人 |

### 第二段：「我準時嗎」（約 5 分鐘）— 制度、規則、講管理
...
| ❌ **正式環境過寬** | 「在對面咖啡廳也算到班」，會讓母文件 §D6「圍欄即到班定義」的立場失效 |
...
- **「應到未打卡」現在算得出來了**：有了班表，`dayType = WORK` 且已過窗起卻無打卡的人數。v1.1 只能顯示 `—`
- 15 秒輪詢
...
新增 `scripts/seed/seed_attendance_demo.ts`（不動既有 `seed.ts`）：

| 產出 | 內容 |
|---|---|
| `AccountBook` | 1 本，id 以 `demo-book-` 前綴（比照 CLAUDE.md §8 的 `e2e-book-` 慣例） |
| `Department` / `JobTitle` | 各 3 筆（產品部、工程部、門市部） |
| `Employee` | **5 筆，`email` 必須是實際上台者的公司 Google 帳號**；`phoneCipher` 等必填 PII 走 `encryptPii` |
| `WorkLocation` | **1 筆：demo 場地實測座標，`radiusMeters = 500`** |
| `ShiftPattern` | **4 筆**（§5.2 表格） |
| `EmployeeShiftDay` | **本月 1 日～今日 + 未來 7 日**，每人每日一筆：內勤展開朝九晚六、林美玲彈性班、陳大文早/晚輪班、週末例假與休息日、1 天國定假日 |
| `AttendancePunch` | **過去 10 個工作日的歷史打卡**，刻意佈置成涵蓋判定表的每一種情形（見下） |

### 9.1 歷史打卡要刻意佈置出這些情形

沒有這些資料，出勤總覽頁會是一片綠色 —— **而一片綠色什麼也證明不了**。

| 情形 | 佈置方式 |
|---|---|
| 遲到 | 王小明某日 09:31 打卡（朝九晚六，寬限 5 分） |
| 早退 | 某人 17:20 下班（core 18:00） |
| 曠職 | 陳大文某個排了早班的日子完全無打卡 |
| 漏打下班卡 | 某人只有 `CLOCK_IN` |
| 漏打上班卡 | 某人只有 `CLOCK_OUT` |
| **彈性班工時不足** | 林美玲 10:00 進 16:00 出 —— **不遲到、不早退，但 360 < 480** |
| **彈性班晚到但正常** | 林美玲某日 09:47 打卡 → 正常（P4 對照組） |
| 假日出勤 | 某人在 `REST_DAY` 打卡 → 不算異常 |
| 跨日班 | 陳大文晚班 22:00 進、次日 06:00 出，`workDate` 歸屬前一日 |

> **今天的資料留空，由現場真人打卡產生。** 這是 demo 最有說服力的一段：歷史是 seed 的，但剛剛那一筆是真的。

### 9.2 兩個必須配合演示時間的安排

1. **`WorkLocation` 座標必須現場實測** —— 不要用 Google Maps 抓的建物中心點，演示當天站的位置與地圖標註可能差上百公尺，那個誤差會吃掉 500 公尺的一部分。做法：到現場用 `navigator.geolocation` 讀一次寫進 seed。
2. **至少一位員工要排「窗迄落在演示時段」的班** —— §3.2 的 `STALE` 演示靠它。演示排 14:00 就給陳大文排早班（06:00–14:00），並 seed 一筆當天 06:02 的 `CLOCK_IN` 而無 `CLOCK_OUT`。

---

## 🗓️ 10. 工作分解（估 10.5 個工作天 / 1 人）
...
| W12 | Seed（含 §9.1 九種情形）+ 環境設定 + **實地座標量測** + 完整彩排 | §10.1 全綠 | 0.75 d |
...
- [ ] **今天的排班已 seed**，且上台者的班別是預期的那一個
- [ ] **有一位員工排了「窗迄落在演示時段」的班**（§9.2 第 2 項），`STALE` 才演得出來
- [ ] 出勤總覽頁的歷史資料涵蓋 §9.1 全部九種情形（**上台前自己先看一遍那張方格圖**）
...
| 2 | 打卡頁顯示「今日班別 朝九晚六」+「距台北總部 32 公尺 ✅」 | P1 | 「在你按下去之前，系統就先告訴你打不打得成、以及你今天幾點該到」 |
| 3 | 按「上班打卡」→ 成功 | — | — |
| 4 | 切現場頁 → 地圖標記 + 名單有自己 | **P3** | 「這個數字現在是可信的，下一步說明為什麼」 |
| 5 | **切示範模式，輸入 3 公里外座標 → 403「距台北總部 3.2 公里」** | **P2** | **「人不在現場，不是『到班但有疑慮』，是到班沒有發生。系統不記錄一件沒發生的事」** |
| 6 | 回現場頁，指出名單沒變 | P2→P3 | 「這就是第 4 步那個數字為什麼可信 —— 圍欄外的打卡從來沒進過資料庫」 |
| 7 | 請第二人打卡 → 名單即時多一人 | P3 | 「火災疏散時，這張名單是要拿來對人頭的」 |
...
| 步 | 動作 | 主張 | 台詞重點 |
|---|---|---|---|
| 8 | 打開排班月曆，指陳大文那一列的早/晚交替與劃休 | P4 | 「門市與產線的輪班劃休。點一格就能調班」 |
| 9 | **打開出勤總覽，指出王小明 09:47 遲到 47 分、林美玲同樣 09:47 正常** | **P4** | **「同一個時間、兩種結果。因為一個是固定班、一個是彈性班」** |
| 10 | **投出 `attendance_rules.ts`，指出沒有 `if (shiftType)`** | **P4** | **「資料庫裡沒有『班別類型』這個欄位。固定班就是彈性窗收縮到跟核心時間一樣的彈性班 —— 所以判定引擎一條分支都不需要。要加第三種制度，改的是資料不是程式碼」** |
| 11 | 點開林美玲另一天：10:00 進 16:00 出 → **不遲到、不早退，但工時不足 120 分** | P5 | 「彈性工時真正要管的不是幾點到，是總時數夠不夠」 |
| 12 | 指出陳大文的曠職與漏打下班卡；**切回現場頁指出他此刻是 🟡** | P5 | **「同一件事的兩個面向：現在這張名單告訴保全『我不確定他還在不在』，明天這張報表告訴 HR『這天要補登』」** |
| 13 | 指著今天那一格的空白 | P5 | **「他今天還沒打卡，但系統沒有判他曠職 —— 現在才下午三點。一套會在早上十點判人曠職的系統，產出的每個數字都不能信」** |
| 14 | 指著某人的「無排班」灰格 | P5 | 「沒有班表就沒有比較基準，系統不會發明一個不存在的出勤義務」 |
| 15 | 收尾 | — | 「打卡有異常時，員工線上填補登單、主管確認後系統自動重算 —— 這部分在正式版」 |

> **第 10 步是整場 demo 對技術聽眾價值最高的一分鐘**，也是唯一需要投影程式碼的地方。
> **第 12、13 步是對 HR 聽眾價值最高的兩分鐘** —— 它們證明系統知道自己不知道什麼，而那是考勤系統最容易失守、也最難事後補救的地方。
...
| 2 | `ShiftAssignmentRule` 週期性指派 | 沒有它，HR 要為每個內勤員工逐日建檔：22 天 × 50 人 = 1,100 筆（§4.4） |
| 3 | 補打卡申請單 | 圍欄外一律拒絕之後，它是**外勤到班事實的唯一入口**（母文件 §D9） |
| 4 | `AttendanceDailyResult` 落地 + `engineVersion` + 判定 Worker | 效能與可回溯。**在資料量還小的時候補，是最便宜的時機** |
| 5 | `AttendancePresence` 快取 + 勾稽對帳 | 同上 |
| 6 | `AuditLog` 寫入、限流、緊急點名匯出 | 上線前必備，但不阻擋開發 |
...
src/app/api/v1/user/account_book/[account_book_id]/hr/attendance/**  （新增 × 9 端點）
...
scripts/seed/seed_attendance_demo.ts                    （新增：含 §9.1 九種情形）
