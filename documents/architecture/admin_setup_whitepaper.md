...
- **AI 處理管線**：`GEMINI_API_KEY` 與 `MODEL` (預設為 `gemini-2.5-pro`)，這些是支撐「憑證萃取」與「混合決策管線」的心臟。
  > **正式保管位置已是資料庫（2026-08-12 補註）**：這兩項屬於 `SYSTEM_SETTING_KEYS`，精靈簽章後會**連鍵值一起從 `.env.setup` 移除**（`setup.system_setting.service` 的 `STAGED_KEYS`），之後由 `/admin/settings` 管理。`.env.setup` 只是遷移前的暫存區。
  > 唯一的例外是**無主資料庫權限的節點**（`MissionExecutor`）—— 它們只能讀部署環境裡的那份，因此 `/admin/settings` 的輪替與撤銷對它們無效，詳見 [已知缺陷](../engineering_guidelines/known_issues/executor_settings_isolation.md)。
