...
| 缺口                                                    | 追蹤   | 性質                                              |
| ------------------------------------------------------- | ------ | ------------------------------------------------- |
| ~~薪資單缺雇主名稱~~ **案由不成立，已關閉**（20260914） | #6795  | 見下方追記 —— §14-1 是四款且全是金額              |
...
**20260914 追記：第一項的案由不成立，#6795 已關閉。**

那一列原本寫「施行細則 §14-1 六項必載只有五項」。核對條文之後，§14-1 是
**四款，全部是金額**：一、勞雇雙方議定之工資總額；二、工資各項目之給付金額；
三、依法令規定或勞雇雙方約定，得扣除項目之金額；四、實際發給之金額。

雇主名稱、勞工姓名、工資給付期間都不在裡面，而薪資單四款都有。
工資清冊那一邊（本法 §23 II）也查了：「發放工資、工資各項目計算方式明細、
工資總額等事項」，同樣全是金額 —— **兩邊都不要求雇主名稱**。

同一段錯誤敘述另有兩份（`pay_slip_meta.ts`、`salary_pay_slip_delivery_plan.md`），
已一併刪除。它在被發現之前生出過一輪工作，經過記在
`salary_company_profile_plan.md`。

（這一則追記本身就是上面那段話的例子：**沒有讀者的 ToDo 與沒有這個 ToDo
是同一件事** —— 而一則**錯的**追蹤項比沒有更糟，因為它會指揮人去做事。）
