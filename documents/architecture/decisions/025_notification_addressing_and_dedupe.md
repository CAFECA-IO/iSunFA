...
**活算的代價是「查得到」變成一個要設計的問題。** 邀請有兩條投遞路徑（錢包位址、email），而查詢一開始只認位址 —— email 邀請的 `inviteeAddress` 是 NULL，於是那半邊的受邀者在鈴鐺與團隊頁上都看不到任何東西（D19）。入庫的話這個問題不存在：寫入時就決定好收件人是誰。

修法是讓查詢同時吃 `inviteeAddress` 與 `inviteeEmailKey`（`inviteeEmail` 的正規化投影，與 `pendingKey` 同一套 `canonicalizeEmailForKey`）。兩個約束：**只採信已驗證的信箱**（未驗證的 email 是使用者宣稱的字串，採信它等於宣稱一個信箱就能讀到寄給該信箱的邀請內容），且**查詢與唯一鍵必須用同一套正規化**（否則 `pendingKey` 認定 `alice+x@` 與 `alice@` 是同一個人，通知卻認定不是）。

以 passkey 註冊、從未綁定第三方登入的人**沒有任何 email**（`User` 沒有那個欄位），所以寄到信箱的邀請對他而言只存在於那封信裡。那是能力的上限，不是缺陷。
