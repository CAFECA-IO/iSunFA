...
Demo 期間的假別是一個寫死的 Prisma enum（`ANNUAL PERSONAL SICK OFFICIAL MARRIAGE BEREAVEMENT OTHER`），而它上方的註解已經自我否定了這個設計：一張只有七列且不會被編輯的表，比 enum 貴而不多任何一分保證。

那個判斷在 Demo 的前提下是對的。正式版的前提相反：需求明文要求「可自訂給假規則」。而在 enum 的世界裡，租戶要新增「志工假」或把特休改成曆年制，路徑是**改 schema、跑 migration、發一版**。那不是設定，那是發版。

### 1.1 但「整個變成資料表」會撞上另一條鐵律

CLAUDE.md §3 第 4 條禁止把需要條件判斷的字串 hardcode 比對。若假別全部變成資料表的一列，規則引擎就必須寫出
...
| 年度日數、年資級距表、最小單位數值、給薪比例、證明門檻 | 欄位 / 子表 | 純數字，修法改的是值不是邏輯 |
...
這條規矩靠 `leave_policy_no_code_branching.test.ts` 釘住，不是靠自律：以 TypeScript 的 AST 掃引擎與編排兩層的假勤檔案，出現 `policy.code ===` / `!==` / `==` / `!=` / `switch (policy.code)` 或代號字串常值即失敗。**這是規則引擎與設定資料之間唯一的一道牆，沒有它，D1 只是一個好意。**

不掃 `src/repositories/`：那一層本來就要用 code 把內建假別**查出來**。查詢與分支是兩件事 —— 查詢是「把那一列拿出來」，分支是「因為它是那一列，所以規則不一樣」。

### 2.2 §38 的級距表是資料

勞基法 §38 I 的 3/7/10/14/15/+1 至 30 日進 `LeaveAccrualTier` 子表，不進程式碼。理由：**它會修法**。2016 年那次修法改的就是這張表。修法時該改的是六列資料，不是一個 switch 的六個 case。

「10 年以上每年加給 1 日，加至 30 日為止」用 `incrementDaysPerYear` 與 `maxDays` 兩個欄位表達 —— 它是一條規則不是 20 個特例。

---

## 🎯 3. 決策：給假週期兩制並存，且曆年制必須通過「不低於週年制」的護欄

需求要求「到職日給假或曆年制給假」二選一。⚠️ 授權依據為勞基法施行細則 §24，**條號待核對**（計畫書 §3.2）。
...
實務上這件事很容易發生：曆年制首年按比例給假，而比例的分母、起算點、捨入方向各家算法不同，隨手一個「不足一日者捨去」就會少給。
...
**這條護欄一寫出來就擋下了計畫書 §6.3 那條比例公式** —— 一個 3/1 到職的人，週年制在 9/1 拿到法定的 3 日，曆年制按剩餘天數占比只給 1.1 日。它擋下的不是一組壞資料，是一條寫錯的規則（計畫書 §17 缺口 9）。曆年制的實務作法是「把未來的年資額度**提前**給」，不是「把當期的法定額度按比例砍掉」，公式須改為下界形式。⚠️ 待法務確認函釋依據。

護欄的性質與財務的 `A = L + E`、ESG 的質量守恆相同：它是一條不可違反的下界，違反代表設定或輸入有錯，不是一個需要人工判斷的警示。依 CLAUDE.md §6 在 Service 開頭凍結。
...
需求寫「最小請假單位（半小時或半天）」。直覺的模型是一個 `minimumUnitMinutes: 30 | 240` 欄位，而它是錯的。
...
`minimumUnitMinutes` 沒有這個結構。在 `HALF_WORKDAY` 之下它不是「另一個說法」，它只是**沒有被讀**。它無法與 `unitBasis` 矛盾，因為它不宣稱任何與 `unitBasis` 同類的事實。拆成兩張表不會消除任何非法狀態，只會讓假別清單要查兩張表再合併。

**維持單表**，由 `assertLeavePolicyUnit` 擋在 repository（唯一 DB 閘口）：
...
| 代價 | 說明 | 為何接受 |
|---|---|---|
| seed 成為正確性的一部分 | 內建假別不再由編譯器保證存在 | 以 `isSystemDefined` 標記 + 禁止刪除 + `leave_seed_integrity.test.ts` 驗證每個帳本都有完整的內建假別 |
| 多一次 join | 假單查詢要 join `LeavePolicy` | 在這個資料量下，不值得為省一次 join 換一個新的矛盾來源 |
| i18n 回退 | 自訂假別沒有 i18n key | 對照表改為 `code → key`，查無則顯示 `LeavePolicy.name`（租戶自己填的，本來就是他要的語言） |
...
1. **`enum LeaveType` 降為 seed 資料**：Prisma enum 與 `src/constants/leave.ts` 的 `LeaveType` 移除，7 個值擴充為 `LEAVE_POLICY_CODE` 的 13 個初始列。**沒有相容期** —— enum 一從 schema 移除，所有引用點就同時編譯失敗，留著它等於同時存在兩套假別來源。
2. **`EMPLOYEE_SCHEDULED_LEAVE_TYPES` 刪除**，改讀 `LeavePolicy.recallable`。
3. **⚠️ 待法務複核**（計畫書 §3.2）：施行細則 §24 的條號、特休比例給假「不得低於週年制」的函釋字號、§38 I ⑥「每一年加給一日」自滿 10 年當年或次年起算。
