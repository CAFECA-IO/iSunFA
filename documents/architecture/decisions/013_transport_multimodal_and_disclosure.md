...
| 9 | 方案代碼貫穿 PDF / CSV / 檔名 | 匯出物之間無交叉索引 | long format 下同路線多方案數列,需可辨識歸屬 |
...
**問題**:long format 讓 CSV 一條路線出現多列(如第 1 條路線 6 列 = 海運 3 段 + 空運 3 段),但列與列之間**沒有可辨識的歸屬標記**——旁人看不出前 3 列是同一條路線的同一個方案;PDF 與 CSV 之間也沒有共同索引,無法回答「這份 PDF 對應 CSV 的哪幾列」。

**決策**:引入**方案代碼** `R{路線兩位}-{模式碼}`(`R01-SEA` / `R01-AIR` / `R01-SLA` / `R02-LAND` / `R01-CUS`),三處一致:

1. **CSV 首欄** `Code`——同方案各段共用同一代碼,換方案即換代碼,群組一眼可辨
2. **PDF 標頭**——徽章與標題並列顯示代碼 + 運輸模式名稱
3. **PDF 檔名**——以代碼開頭(`R01-SEA_台北市-Shanghai_sea_multimodal.pdf`)

另加**匯出批次識別碼** `YYYYMMDD-HHmm`(CSV 檔頭 + PDF 頁尾),使跨批次的同名代碼(兩次匯出都有 R01-SEA)仍可區分。

**可讀性取捨**:欄名一律去除單位與贅字(`Plan Total CO2e (kg)` → `Plan CO2e`、`Estimated?` → `Est.`),單位集中於檔頭 `# Units` 行;檔頭亦由單行 600 字改為多行短註解(一行一件事、不含逗號)。代價是「單看某一欄名無法得知單位」,以檔頭必然同在同一份檔案作為補償——欄名是每天要掃視的東西,揭露文字只需讀一次。

**取捨**:代碼以「路線序號」而非資料庫 id 為基礎——序號在單一匯出批次內穩定且對人類友善;跨批次的唯一性由批次識別碼補足。若未來需要全域永久唯一(如對外送審),應改用 analysis id 或 UUID,屆時代碼格式需一併調整。
