...
| **成員移除** | **沖銷**分配餘額（2026-08-18 修訂）：分配歸零但**不回池**。收回在合約層面做不到（只有 `burnAndUnlock(uint256)`，燒的是 `msg.sender` 自己的餘額），點數已在成員的鏈上錢包裡；加回池會讓同一筆價值存在兩份，團隊得以再鑄一次。分錄記為負的 `ADJUST`（不是 `REVOKE`——後者在守恆恆等式裡被排除，記錯會讓下一輪勾稽誤判違反守恆而凍結錢包）。舊的離鏈餘額走同一條路：分不出哪些已鑄上鏈，而少算池餘額不會多發點數給任何人 |
