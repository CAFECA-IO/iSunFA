...
| **成員移除** | **沖銷**分配餘額（2026-08-18 修訂）：分配歸零但**不回池**。收回做不到：點數已在成員的鏈上錢包裡，而**移出那個錢包必須有持有人簽章**，被移除的成員不會去簽（2026-08-18 更正：原本這裡寫「合約層面做不到」，那個因果是錯的——扣款以持有人簽章就做得到，見 `ensurePersonalCreditCharge`；補一個平台可呼叫的 `burn(address, uint256)` 與條款 §3.3 的簽章承諾相反，不是解法）；加回池會讓同一筆價值存在兩份，團隊得以再鑄一次。分錄記為負的 `ADJUST`（不是 `REVOKE`——後者在守恆恆等式裡被排除，記錯會讓下一輪勾稽誤判違反守恆而凍結錢包）。舊的離鏈餘額走同一條路：分不出哪些已鑄上鏈，而少算池餘額不會多發點數給任何人 |
| **守恆恆等式** | 原式 `購入 + 調整 − 消耗 + 退還 = 池餘額 + Σ 分配餘額` 的右側第二項對新資料恆為 0；分配後的點數改由鏈上餘額承載，離鏈帳本只保證「池 + 已鑄出量」對得起來。⚠️ **2026-08-18 修正**：這一格只寫到「右側第二項恆為 0」，沒有跟著改**左側**——於是每成功分配一次，右側就少一個 amount 而左側不動，勾稽判為違反並**凍結錢包**。按一次「分配」就足以觸發，而 `wallet_audit.test.ts` 的 fixture 同時給了 `ALLOCATE 50` 與「分配餘額 47」（真實資料裡不可能的狀態），所以全綠。修法：`allocate()` 在同一個交易裡另寫一筆負的 `ADJUST`（鍵為 `allocate-offchain-exit:{原鍵}`），把「價值離開離鏈帳本」記在左側——與遷移腳本、成員移除沖銷**已經在用**的作法一致。存量差額以 `scripts/repair_wallet_conservation.ts` 一次性補平（見 [凍結處理程序](../../engineering_guidelines/wallet_conservation_freeze_runbook.md)） |
