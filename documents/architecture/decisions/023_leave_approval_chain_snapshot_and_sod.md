...
前三條的**規矩**沿用出勤模組 §D9 對補登單立下的定義，第四條為本模組新增。

> **注意**：§D9 只是「點名了這兩個錯誤碼應該存在」——補登單從未實作，
> 因此它們在本模組動手時仍不存在。第一版誤以為可以直接沿用，
> 結果是 `new AppError(undefined)`。教訓寫在計畫書 §11 的更正欄。

| # | 規則 | 錯誤碼 |
|---|---|---|
| 1 | 不得自我核准 | `FO_SELF_APPROVAL_FORBIDDEN`（`FO000014`，**本模組新增**） |
| 2 | 非當前簽核節點不得代簽 | `FO_NOT_AUTHORIZED_REVIEWER`（`FO000015`，**本模組新增**） |
...
3. **`HR` 節點的角色來源待定**：目前沒有帳本層級的 HR 角色定義。里程碑 3 前必須決定。
   ...
   >
   > **決定（2026-08-18）**：兩個子問題一起收 —— **指派表**
   > （`EmployeeHrFunctionAssignment`），一人可同時具備多個職能，撤銷寫
   > `revokedAt` 而不刪列。
   >
   > 決定的依據是第二個子問題：HR 職能等於「看得到全體個資 + 簽得掉別人的假」，
   > 而勞動檢查與資安複核問的不是「現在誰是人資」，是**「四月那張單被簽核時，
   > 誰有這個權限」**。一個會被就地覆寫的 enum 陣列答不出那個問題，
   > 而 `AuditLog` 只記「誰改過誰」、不記舊值。
   >
   > enum 只有兩個值 —— `HR_ADMIN` 與 `TIMEKEEPER` —— 因為今天只有兩個消費者。
   > 憑空多列 `PAYROLL` / `AUDITOR` 會讓它看起來像一條生效中的規則。
   >
   > 三個接點同時收斂：`hrEmployeeIds`（本節）、簽核規則設定（L32 的
   > `assertMayConfigure`）、排班寫入閘（改為「HR 職能跨部門 or 部門主管在自己
   > 範圍內」，取代原本只問「有沒有管任何部門」的粗判斷）。
   >
   > ⚠️ **尚未決定：誰有權指派職能。** 不能讓 `HR_ADMIN` 指派 `HR_ADMIN`
   > （自我擴權），也不能空著（第一個人永遠拿不到）。因此目前**沒有指派 API**，
   > 只有 seed 與 repository 方法 —— 一個沒有授權判斷的指派端點，
   > 比沒有端點危險得多。傾向由帳本所屬 team 的 `OWNER` / `ADMIN` 指派。
   >
