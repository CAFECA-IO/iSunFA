...
`WorkDayType.REGULAR_OFF`（例假）依 §40 原則上**不得使人工作**，僅限天災、事變或突發事件，且須於 24 小時內通報主管機關並事後補假。

本 ADR **不處理**通報與補假義務（⚠️ §40 未查證）。因此在補齊之前，例假日的加班申請一律**擋下**並回 `FO_OVERTIME_ON_REGULAR_OFF`（403），提示須由 HR 依 §40 程序處理。
