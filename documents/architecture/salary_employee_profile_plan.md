...
| `basic_info_form.tsx` | 到職／離職的勾選與日期改為**唯讀顯示**（值來自 `hireDate`），或維持可改但標示「只影響這次試算」。**這一格要再確認**，見 §9 |
...
2. **`basic_info_form` 的到職／離職欄位要不要變成唯讀？（需要產品決策）**
   有了 `hireDate` 之後，計算機上那兩個欄位有兩種語意：「改這次試算」或「改這個人的到職日」。
   維持可改最不打擾，但使用者會以為改了員工檔；改唯讀最誠實，但「這個月才發現到職日打錯」
   就得繞去員工列表。建議**可改 + 一行說明「只影響這次試算，要改員工資料請到員工列表」**，
   並在 §4.2 的差異對話框裡把它一起列出來 —— 那樣兩種語意都保留，且不會靜默。
