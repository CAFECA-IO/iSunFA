...
>
> **但寄信量需要自己的界線**：人數不再是煞車，而每一封 email 邀請都是真的寄出去的信。因此同一輪新增三層（2026-08-19）——依操作者的限流（10/分、100/日）、團隊層的同時未接受邀請數（預設 20）、團隊層的每日寄送數（滾動 24 小時，預設 50）。三層分工不同：限流擋單人狂點、同時數擋「一次撒出幾百封」、每日數擋「撤回再邀」的迴圈。實作見 `assertInviteVolumeWithinLimits`。
