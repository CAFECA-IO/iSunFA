...
免費版團隊：不計席次費，跳過步驟 2–3，**人數不限**（2026-08-19 產品拍板）。

> 原本這裡受「免費版人數上限」約束，理由是「否則免費版可無限拉人共用同一份免費額度」。而真正的問題是那句話當時**不成立**——2026-08-14 起免費版的額度也是逐成員各一份，所以拉人不是「共用同一份」，是**每個人各得一份**（20 人 ＝ 每週 800 點、月費零），上限因此成為必要。
>
> 2026-08-19 把免費方案的額度改回**全隊共用一份**（設計書 §5.4.2 的 2026-08-19 修訂）之後，「無限拉人共用同一份免費額度」變成**現在的設計**而不是要防的事：加人不再產生任何額度。上限與它的兩道防線（邀請端、接受端）因此一併移除。付費方案完全不變——人數仍由「席次 × 單價」自然封頂。

### 3.5 收費前必須先揭露金額（2026-08-18，使用者回報）

上面那個順序有一個**沒有寫出來的前提**：使用者在按下「邀請」時，已經知道會被收多少錢。實際上他不知道。

回報的原話是：**「我在邀請時完全不知道會被加收多少錢」**。當時的行為是——邀請對話框只有位址／信箱與角色，按下送出後系統立刻以 merchant-initiated 交易刷訂閱那張卡（持卡人是 OWNER，而發起者可能是 ADMIN），事前沒有任何金額揭露；成功後的提示也只在「用了空席」時說一句話，連金額都不顯示。步驟 1 那句「簽章也授權這筆扣款」因此站不住腳：使用者簽的是一個他不知道金額的授權。

**規則**：任何會產生扣款的加人動作，都必須在送出前顯示

1. 將收取的**金額**；
2. **為什麼是這個數字**：要收費的席次數、本期剩餘天數（比例計價的依據）；
3. 不收費時也要說明原因（免費方案／沿用已付費的空席／期末零頭），否則使用者無法分辨「這次免費」與「系統忘記收」；
4. 現在**不能**加人時的原因（沒有可扣款的卡／單價缺失／當期補收已達上限／免費版人數已滿），並在揭露不到金額時**不允許送出**。

**實作**：`quoteSeatAddition()`（唯讀）＋ `GET /api/v1/user/team/{teamId}/seat_quote?seats=N`。

關鍵是**同一支實作**：`chargeSeatAddition()` 的所有判斷都改成呼叫 `quoteSeatAddition()`，自己只負責執行（擋下的原因翻成 ApiError、要收費的建單與扣款）。另寫一份試算等於讓「顯示的金額」與「實扣的金額」是兩支程式，而它們分岔的那天，畫面會很有說服力地報一個錯的價格。`seat_quote_contract.test.ts` 因此同時呼叫兩支並比對金額，另有一條掃原始碼確認扣款端沒有第二份比例計價。

前端在對話框開啟時試算一次，把金額寫在送出按鈕上（「確認並支付 TWD 420」）；試算失敗時**不允許送出**，改為顯示「重新試算」——「試算掛了但照樣可以邀請」等於回到沒有揭露就扣款的原點。

**揭露必須與扣款綁在一起（2026-08-19 補）**：只有「送出前顯示金額」還不夠。試算是在對話框開啟時算的，而送出隔著填表與一次 FIDO2 簽章——試算的兩個輸入在那段時間都會變：

| 變的是什麼 | 後果 |
|---|---|
| 席次佔用（另一位管理者同時邀了人） | 顯示「使用已付費的空席，不會再收費」→ 實際被刷一席的比例費用 |
| 計費週期滾動（8/31 23:58 試算、00:02 送出） | 顯示「本期即將結束、不收費」→ 新週期的比例計價回近乎全額 |

因此送出時要把**畫面上顯示過的金額**（`expectedAmount`）一起送回，服務端在建單與扣款**之前**比對一次，不符即 `TW000025` 擋下並要求重新試算——不照新價扣款。值為 0 也要比對：「顯示不收費、實際收費」是這個缺陷最糟的形態。

`expectedAmount` 由兩支邀請端點的 validator **要求必填**：「沒有先試算過就不能送出」不能只活在前端按鈕的 `disabled` 條件裡，那一行被刪掉就精準退回原本的行為。

成功之後也要把**實際扣款的金額**顯示出來（`seatCharge.amount`）。先前只顯示「使用了空席」，於是「實際扣了多少」在整個流程裡從頭到尾沒有出現過，分岔只會在下期帳單被發現。
