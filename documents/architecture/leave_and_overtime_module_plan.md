...
> **#1 與 #2 的順序是被修正過的（2026-08-19，review B7）**。原本 `isEmergency` 排在第一列「任意日別」，於是申請人只要在送出的 payload 裡自填一個布林值，就同時跳到加倍級距**並繞過例假日的閘門** —— 而系統裡沒有任何地方記載那次報備。
>
> 兩者都以天災事變為前提，但**程序不同、法律效果不同**：§32 IV 是「通知工會；無工會者報當地主管機關**備查**」，§40 是「報當地主管機關**核備**」。備查是報請存查，核備須經主管機關認可。拿 §32 IV 的認定去放行 §40 的情形，是用一份不對的文件當通行證。
>
> **#1 的誠實揭露**：例假日（`REGULAR_OFF`）依 §40 原則上不得使人工作，僅限天災、事變或突發事件，且須報當地主管機關**核備**並事後補假休息。⚠️ §40 的核備程序（文號、核備日、事後補假日期）未查證亦未實作，因此例假日**一律擋下**、沒有旁路 —— 放行會讓一個違法的排班在系統裡看起來像一筆正常的加班（ADR 024 §4.5）。
...
**現況**：`evaluateOvertimeLimits()` 硬擋，且**不看 `isEmergency`** —— 天災事變一樣受 12/46/54/138 小時的限制。
...
**仍未做**：報備的 24 小時時限只記錄不擋（逾期是另一個違章，擋下只會逼出一個把時點往前填的動作），逾期的統計要進 L28。下面第 2 項的書表欄位仍未核對 —— 它決定 `emergencyReportUrl` 指向的東西該長什麼樣。
...
1. §32 IV 原文：延長工時的法定事由、報備時限、受理對象（工會／無工會者為當地主管機關）。
2. 該報備的法定書表格式與必填欄位。**欄位靠回憶重建是不可接受的** —— 一份欄位錯的報備書比沒有更糟，它會讓人以為已經報備了。（目前 `emergencyReportUrl` 只收一個連結，形式不拘，正是因為這一項未定：要求一個固定格式的文號會逼出一堆填 `N/A` 的紀錄。）
...
| T21 | `leave_i18n_keys.test.ts`                | 五語系（en/ja/ko/zh_cn/zh_tw）key 齊備；比照 `i18n_keys.test.ts`                                                                                       | ❌ 未寫。里程碑 2。`i18n_keys.test.ts` 已掃全 `src`，實質涵蓋大部分                                            |
...
| 7   | `src/i18n/locales/{5 語系}/hr_management.ts` | `type_*`（7 個）→ `policy_*`（13 個）；新增 `day_type_suspended` 與其縮寫、`leave_on_leave`                                        | `i18n_keys.test.ts`           |
