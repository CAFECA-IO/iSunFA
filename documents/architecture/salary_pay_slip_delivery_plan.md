...
7. **前端的「隱藏金額」是視覺遮擋，不是權限（20260908 記錄）。**
   ...
   **附帶解掉的是 §2.1 的張力。** 軟刪除之後資料庫層的 DELETE 不再發生，
   ...
   Info: (20260914 - Julian) 這裡原本掛著一則「雇主名稱仍然缺席，這一格
   才是法定必載」的 ToDo。**核對條文之後那句話是錯的，已刪除。**

   §14-1 實際上是**四款，全部是金額**：一、勞雇雙方議定之工資總額；
   二、工資各項目之給付金額；三、依法令規定或勞雇雙方約定，得扣除項目之金額；
   四、實際發給之金額。雇主名稱、勞工姓名、工資給付期間**都不在裡面**，
   而薪資單四款都有。#6795 以案由不成立關閉。

   工資清冊那一邊也查了（本法 §23 II）：「發放工資、工資各項目計算方式明細、
   工資總額等事項」，同樣全是金額。**明細與清冊兩邊都不要求雇主名稱。**

   注意上面 §「不做批次匯出」那張清冊／明細對照表是**對的** ——
   它正確地把 §23 II 與 §23 I ＋細則 §14-1 分開。錯的只有這一則 ToDo 的
   六款列舉，而同一份文件裡兩個版本並存了五天沒有人發現。

   到職日與投保狀態同樣不是法定必載 —— 它們是客戶自己的需求
   （勞檢實務上會問）。這句話原本就對，只是接在錯的前提後面。

   **但核對過程撞到一個真的清冊缺口**：§38 第五項要求把「每年特別休假之期日」
   與「未休之日數所發給之工資數額」記載於工資清冊，並每年定期以書面通知勞工。
   目前三缺一（只有金額），而年度通知整個功能不存在。
   細節與分階段見 `salary_company_profile_plan.md` §2.1。
