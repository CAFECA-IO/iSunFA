...
| 10.4 同步寄送的時間成本 | 未觸發。Next 的 serverless 逾時尚未實測，冷啟動 4.6s + SMTP 往返仍可能超過 10 秒 |
| 10.5 批次寄送 | 不在範圍，未做 |
| 10.6 已收到分頁 | 維持假資料，`pay_slip.ts` 與頁面註解都寫明了，並有測試釘住「它還是假的」 |

### 11.5 尚未完成

- **`salary_delivery_repo.e2e.test.ts` 從未執行**（開發機沒有資料庫）。它是唯一
  驗得到「員工改信箱之後既有的列不跟著變」與 `lastSentAt` 真的算得出來的東西，
  連同 §8 必跑 mutation 的第 3 條（`recipientEmail` 改成 join）一起留給 CI。
- **`npx prisma db push` 尚未執行**，schema 還沒套用到任何資料庫。
- **寄信服務在開發機上未設定**，因此整條路徑沒有做過端到端的實際寄送。
  SMTP 的五個變數與取得方式記在 `.env.example` 的 PART 7（**只能寫成註解，
  不能寫成鍵值** —— `validateEnvDetailed()` 會把鍵值全當成必要鍵）。
