...
| **4. VOUCHER_LINES**| `voucher.ts` 內的分錄 JSON Array 範例與會計恆等式指令 | ~250 chars | ~180 Tokens |
| **5. ESG** | `esg.ts` 內極度肥大的 13 欄位碳盤查 JSON 結構與嵌套物件 | ~1,150 chars | ~900 Tokens |
| **Total (單張憑證)**| | **~2,030 chars** | **~1,510 Tokens** |

*(註：Gemini 針對繁體中文的 Tokenizer 切分較細，平均約 1 Token = 1.2~1.5 中文字元。)*
...
* **每月節省的 Input Tokens**: 10,000 * 1,510 = **15,100,000 Tokens (約 1,500 萬 Tokens)**
* **推論速度提升 (TTFB)**: AI 不再需要花費注意力權重 (Attention Mechanism) 去閱讀、理解與 mapping 高達兩千字的 JSON 範例，而是直接由底層 API 強制輸出結構。這不僅降低了 API 收費，還能顯著**降低每張憑證的解析延遲 (Latency)**。
