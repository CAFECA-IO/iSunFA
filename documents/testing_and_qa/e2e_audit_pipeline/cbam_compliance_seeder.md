...
此模組透過對抗式 AI (Adversarial Generator) 動態生成極度擬真的供應鏈碳流動數據，為前端 CBAM 申報模組與盡職調查 (DDP) 提供深度的壓力測試資料庫。此外，最新版本已支援**跨年度動態參數 (year)**，可用於模擬與推估未來的財務與排碳數據。
...
- **機制**：將上述所有產品碳排、重量、HS Code (海關稅則號別) 打包，生成可用於歐盟 CBAM 申報系統的擬真報單。這是驗證「申報量」與「底層盤查量」勾稽正確性的防線。

### 5. `cbam_generator.ts` (最終勾稽核心)
- **用途**：CBAM 的最終稽核與財務-物理雙向勾稽報告產生器。
- **機制**：它**不是**單純產生假資料，而是讀取 E2E 管線中 `chronological_reverse_engineer.ts` 所產生的財務傳票 (`simulated_vouchers.json`)，透過分析「會計科目」與「供應商」，**反推回物理量的消耗**。例如：從台電的電費傳票反推用電度數 (kWh) 並計算 Scope 2 碳排；從鋼材供應商的進貨傳票反推鋼材重量，並透過 BOM 表抓取的碳排因子 (Emission Factor) 計算前驅物碳排。這證明了系統在「財務金流」與「物理碳排量」上的絕對守恆 (Single Source of Truth) 與防漂綠能力。
