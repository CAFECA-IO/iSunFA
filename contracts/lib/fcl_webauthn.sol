...
import {Base64Url} from "./base64_url.sol";
...
library FCL_WebAuthn {
    ...
    function WebAuthn_format(
        ...
        // Let the caller check if User Presence (0x01) or User Verification (0x04) are set
        {
            ...
            string memory challengeEncoded = Base64Url.encode(
            ...
            if (
                keccak256(abi.encodePacked(bytes(challengeEncoded))) != moreData
            ) {
    ...
    function checkSignature(
        ...
        bytes32 message;
        // Info: (20260412 - Luphia) Verify User Presence
        if (
            (authenticatorData[32] & authenticatorDataFlagMask) !=
            authenticatorDataFlagMask
        ) revert InvalidAuthenticatorData();
        ...
        for (uint256 i = 0; i < challengeExtracted.length; i++) {
            challengeExtracted[i] = clientData[clientChallengeDataOffset + i];
        }

        if (
            keccak256(abi.encodePacked(challengeEncoded)) !=
            keccak256(challengeExtracted)
        ) revert InvalidClientData();

        bytes memory verifyData = abi.encodePacked(
            authenticatorData,
            sha256(clientData)
        );
    ...
    function checkSignature_prec(
        ...
        // Info: (20260412 - Luphia) Let the caller check if User Presence (0x01) or User Verification (0x04) are set
        ...
        bool result = FCL_ecdsa.ecdsa_precomputed_verify(
            message,
            rs,
            dataPointer
        );
    ...
    // Info: (20260412 - Luphia) beware that this implementation will not be compliant with EOF
    function checkSignature_hackmem(
        ...
        bool result = FCL_Elliptic_ZZ.ecdsa_precomputed_hackmem(
            message,
            rs,
            dataPointer
        );
