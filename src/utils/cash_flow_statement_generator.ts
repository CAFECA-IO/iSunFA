...
import { ICashFlowStatement, ICashFlowStatementItem } from "@/interfaces/cash_flow_statement";
...
export function generateCashFlowStatement(
  ...
  if (depreciationAndAmortization !== 0) {
      finalOperatingItems.push({ name: '折舊及攤銷費用(加回)', amount: depreciationAndAmortization });
  }
  finalOperatingItems.push(...mapToArray(operatingItems));

  const totalOperating = finalOperatingItems.reduce((acc, curr) => acc + curr.amount, 0);
  const finalInvestingItems = mapToArray(investingItems);
  const totalInvesting = finalInvestingItems.reduce((acc, curr) => acc + curr.amount, 0);
  const finalFinancingItems = mapToArray(financingItems);
  const totalFinancing = finalFinancingItems.reduce((acc, curr) => acc + curr.amount, 0);
  ...
  const operatingCashFlowRatio = safeDivide(totalOperating, currentLiabilitiesTotal) * 100;
  ...
  return {
    ...
    metrics: {
      ...
      cashFlowAdequacyRatio: safeDivide(totalOperating, capitalExpenditure + Math.abs(totalFinancing)) * 100, // Info: (20260330 - Julian) 簡化版
      cashReinvestmentRatio: safeDivide(totalOperating - calculateDividends(finalFinancingItems), 1000000) * 100 // Info: (20260330 - Julian) 簡化版 (需要總資產，這裡無法直接取得)
    }
  };
}

function calculateDividends(items: ICashFlowStatementItem[]) {
    return items.filter(i => i.name.includes('股利')).reduce((acc, curr) => acc + Math.abs(curr.amount), 0);
}
