 ...
 */
/**
 * Info: (20260821 - Emily) 三段動詞修訂(ch1-intro、ch3-3、ch1-5),偏離原稿的僅有三處。
 *
 ...
 *   依據 IFRS S2 規定     → 參照 IFRS S2 所定之…        (第二輪 review;
 *     「依據」進了合規動詞清單。曾評估「讓源頭的尺比紙面窄」並否決:
 *     guidance 教模型寫「依據」,模型照寫,**紙面**判準就紅,而紙面分不出
 *     那句是誰教的 —— 源頭例外救不了紙面,只有改動詞自洽。實測見 PR。)
 *
...
export const CARBON_REPORT_GUIDANCE_IFRS: Readonly<Record<string, string>> = {
  ...
  "ch1-5":
    "參照 IFRS S2 所定之組織邊界方法，說明組織邊界如何與財務合併報表對齊（採用控制權法或股權比例法），並明確交代若有投資聯屬公司或合資企業時的範疇界定。",
  "ch1-6":
