...
export const BASELINE_TIERS: ReadonlyArray<{
  ...
  },
  {
    ...
    keys: [
      ...
      "對帳逐字引用列_排除",
      /**
       * Info: (20260820 - Emily) 紙上「範疇」出現幾次,以及其中哪些是**系統印的分類標籤**。
       *
       * 承重的判準是「有分類標籤就要有對照說明」;這兩個鍵記的是判準的取樣範圍。
       * 08-20 run C:字數 6、分類標籤 0(節標題 2 + 客戶原文 2 + 近似映射揭露 2)。
       * 兩個數字並列才看得出「收窄了多少」—— 只留判準結果的話,
       * 下一次有人把判準改寬改窄都不會有人發現。
       */
      "紙上範疇字數",
      "紙上範疇分類標籤",
