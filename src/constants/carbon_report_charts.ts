...
export enum CarbonChartTemplateEnum {
  ...
  // Info: (20260720 - Emily) #53 碳流量桑基圖(mermaid sankey-beta):憑證 → 排放源 → Scope;
  // Info: (20260720 - Emily) 總流入=總流出即質量守恆的視覺化(#22 同一哲學)
  EMISSION_SANKEY = "EMISSION_SANKEY",
}

// Info: (20260720 - Emily) 桑基圖憑證節點上限:超過即略過憑證層(排放源→Scope),避免不可讀的毛線團
export const CARBON_SANKEY_MAX_EVIDENCE_NODES = 30;

// Info: (20260721 - Emily) UAT:排放總量匯總段(3.6)草稿生成時自動附掛碳流量桑基圖 —
// Info: (20260721 - Emily) mermaid 原始碼落在 Markdown 輸入區,PDF 預覽區同步渲染
