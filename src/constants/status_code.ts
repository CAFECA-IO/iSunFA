...
enum ErrorMessage {
  ...
  OCR_PROCESS_STATUS_IS_NOT_IN_PROGRESS = `${ErrorCode.OCR_PROCESS_STATUS_IS_NOT_IN_PROGRESS}: OCR process status is not in progress`,
  UNSUPPORTED_EXPORT_TYPE = `${ErrorCode.UNSUPPORTED_EXPORT_TYPE}: Unsupported export type`,
  INVALID_EXPORT_TYPE = `${ErrorCode.INVALID_EXPORT_TYPE}: Invalid export type for handleAssetExport`,
  INVALID_FILE_TYPE = `${ErrorCode.INVALID_FILE_TYPE}: Invalid file type`,
  INVALID_COMPANY_ID = `${ErrorCode.INVALID_COMPANY_ID}: Invalid companyId`,
  UNSUPPORTED_FILE_TYPE = `${ErrorCode.UNSUPPORTED_FILE_TYPE}: Unsupported file type`,
  INVALID_TIMESTAMP = `${ErrorCode.INVALID_TIMESTAMP}: Invalid timestamp`,
  INVALID_TIMEZONE_FORMAT = `${ErrorCode.INVALID_TIMEZONE_FORMAT}: Invalid timezone format, should be +HHMM or -HHMM`,
  UNBALANCED_DEBIT_CREDIT = `${ErrorCode.UNBALANCED_DEBIT_CREDIT}: Debit Amount Must Equal Credit Amount`,
  UNAUTHORIZED_ACCESS = `${ErrorCode.UNAUTHORIZED_ACCESS}: Unauthorized access`,
  RESOURCE_NOT_FOUND = `${ErrorCode.RESOURCE_NOT_FOUND}: Resource not found`,
  FORBIDDEN = `${ErrorCode.FORBIDDEN}: Forbidden`,
  AICH_SUCCESSFUL_RETURN_BUT_RESULT_IS_NULL = `${ErrorCode.AICH_SUCCESSFUL_RETURN_BUT_RESULT_IS_NULL}: AICH successful return but result is null`,
  AICH_API_NOT_FOUND = `${ErrorCode.AICH_API_NOT_FOUND}: AICH API not found`,
  API_NOT_DEFINED = `${ErrorCode.API_NOT_DEFINED}: API is not defined`,
  METHOD_NOT_ALLOWED = `${ErrorCode.METHOD_NOT_ALLOWED}: Method not allowed`,
  CONFLICT = `${ErrorCode.CONFLICT}: Conflict`,
  INVITATION_CONFLICT = `${ErrorCode.INVITATION_CONFLICT}: Invitation has been used`,
  DUPLICATE_COMPANY = `${ErrorCode.DUPLICATE_COMPANY}: Duplicate company`,
  DUPLICATE_COMPANY_KYC_DONE = `${ErrorCode.DUPLICATE_COMPANY_KYC_DONE}: Duplicate company KYC done`,
  DUPLICATE_ROLE = `${ErrorCode.DUPLICATE_ROLE}: Duplicate role`,
  INVALID_INPUT_PARAMETER = `${ErrorCode.INVALID_INPUT_PARAMETER}: Invalid input parameter`,
  INVALID_INPUT_FORM_DATA_IMAGE = `${ErrorCode.INVALID_INPUT_FORM_DATA_IMAGE}: Invalid input form data image`,
  INVALID_INPUT_INVOICE_BODY_TO_VOUCHER = `${ErrorCode.INVALID_INPUT_INVOICE_BODY_TO_VOUCHER}: Invalid input invoice body to voucher`,
  INVALID_INPUT_VOUCHER_BODY_TO_JOURNAL = `${ErrorCode.INVALID_INPUT_VOUCHER_BODY_TO_JOURNAL}: Invalid input voucher body to journal`,
  INVALID_ENUM_VALUE = `${ErrorCode.INVALID_ENUM_VALUE}: Invalid enum value`,
  INVALID_INPUT_TYPE = `${ErrorCode.INVALID_INPUT_TYPE}: Invalid input type`,
  INVALID_VOUCHER_AMOUNT = `${ErrorCode.INVALID_VOUCHER_AMOUNT}: Voucher amount is not greater or equal to Payment amount, Debit and Credit should be equal and greater than Price * (1 + Tax Rate) + Fee`,
  INVALID_OUTPUT_DATA = `${ErrorCode.INVALID_OUTPUT_DATA}: Invalid output data`,
  INVALID_CONTENT_TYPE = `${ErrorCode.INVALID_CONTENT_TYPE}: Invalid content type`,
  INVALID_FILE_FORMAT = `${ErrorCode.INVALID_FILE_FORMAT}: Invalid file format`,
  INTERNAL_SERVICE_ERROR = `${ErrorCode.INTERNAL_SERVICE_ERROR}: Internal service error`,
  IMAGE_UPLOAD_FAILED_ERROR = `${ErrorCode.IMAGE_UPLOAD_FAILED_ERROR}: Image upload failed`,
  DATABASE_CREATE_FAILED_ERROR = `${ErrorCode.DATABASE_CREATE_FAILED_ERROR}: Database create failed`,
  DATABASE_READ_FAILED_ERROR = `${ErrorCode.DATABASE_READ_FAILED_ERROR}: Database read failed`,
  DATABASE_UPDATE_FAILED_ERROR = `${ErrorCode.DATABASE_UPDATE_FAILED_ERROR}: Database update failed`,
  PARSE_JSON_FAILED_ERROR = `${ErrorCode.PARSE_JSON_FAILED_ERROR}: Parse JSON failed`,
  BAD_GATEWAY = `${ErrorCode.BAD_GATEWAY}: Bad gateway`,
  INTERNAL_SERVICE_ERROR_AICH_FAILED = `${ErrorCode.INTERNAL_SERVICE_ERROR_AICH_FAILED}: Bad gateway connect AICH failed`,
  INTERNAL_SERVICE_ERROR_DATA_FROM_AICH_IS_INVALID_TYPE = `${ErrorCode.INTERNAL_SERVICE_ERROR_DATA_FROM_AICH_IS_INVALID_TYPE}: Bad gateway data from AICH is invalid type`,
  INTERNAL_SERVICE_ERROR_PRISMA_ERROR = `${ErrorCode.INTERNAL_SERVICE_ERROR_PRISMA_ERROR}: Prisma related error`,
  GATEWAY_TIMEOUT = `${ErrorCode.GATEWAY_TIMEOUT}: Gateway timeout`,
  MISSING_ERROR_FROM_BACKEND_API = `${ErrorCode.MISSING_ERROR_FROM_BACKEND_API}: Missing error from backend API`,
  DUPLICATE_COUNTERPARTY_NAME = `${ErrorCode.DUPLICATE_COUNTERPARTY_NAME}: Duplicate counterparty name`,
  DUPLICATE_COUNTERPARTY_TAX_ID = `${ErrorCode.DUPLICATE_COUNTERPARTY_TAX_ID}: Duplicate counterparty tax ID`,
  USER_NOT_IN_TEAM = `${ErrorCode.USER_NOT_IN_TEAM}: User not in team`,
  OWNER_IS_UNABLE_TO_LEAVE = `${ErrorCode.OWNER_IS_UNABLE_TO_LEAVE}: Owner is unable to leave`,
  TEAM_NOT_FOUND = `${ErrorCode.TEAM_NOT_FOUND}: Team not found`,
  EXCEED_PLAN_LIMIT = `${ErrorCode.EXCEED_PLAN_LIMIT}: Exceed free plan limit`,
  ACCOUNT_BOOK_NOT_FOUND = `${ErrorCode.ACCOUNT_BOOK_NOT_FOUND}: Account book not found`,
  TRANSFER_RECORD_NOT_FOUND = `${ErrorCode.TRANSFER_RECORD_NOT_FOUND}: Transfer record not found`,
  TRANSFER_RECORD_IS_COMPLETED = `${ErrorCode.TRANSFER_RECORD_IS_COMPLETED}: Transfer record is completed`,
  TRANSFER_RECORD_IS_CANCELLED = `${ErrorCode.TRANSFER_RECORD_IS_CANCELLED}: Transfer record is cancelled`,
  TRANSFER_RECORD_IS_DECLINED = `${ErrorCode.TRANSFER_RECORD_IS_DECLINED}: Transfer record is declined`,
  TRANSFER_RECORD_IS_FAILED = `${ErrorCode.TRANSFER_RECORD_IS_FAILED}: Transfer record is failed`,
}
...
export const STATUS_CODE = {
  ...
  [STATUS_MESSAGE.USER_NOT_IN_TEAM]: ErrorCode.USER_NOT_IN_TEAM,
  [STATUS_MESSAGE.OWNER_IS_UNABLE_TO_LEAVE]: ErrorCode.OWNER_IS_UNABLE_TO_LEAVE,
  [STATUS_MESSAGE.TEAM_NOT_FOUND]: ErrorCode.RESOURCE_NOT_FOUND,
  [STATUS_MESSAGE.EXCEED_PLAN_LIMIT]: ErrorCode.FORBIDDEN,
  [STATUS_MESSAGE.ACCOUNT_BOOK_NOT_FOUND]: ErrorCode.RESOURCE_NOT_FOUND,
  [STATUS_MESSAGE.TRANSFER_RECORD_NOT_FOUND]: ErrorCode.RESOURCE_NOT_FOUND,
  [STATUS_MESSAGE.TRANSFER_RECORD_IS_COMPLETED]: ErrorCode.CONFLICT,
  [STATUS_MESSAGE.TRANSFER_RECORD_IS_CANCELLED]: ErrorCode.CONFLICT,
  [STATUS_MESSAGE.TRANSFER_RECORD_IS_DECLINED]: ErrorCode.CONFLICT,
  [STATUS_MESSAGE.TRANSFER_RECORD_IS_FAILED]: ErrorCode.CONFLICT,
};
