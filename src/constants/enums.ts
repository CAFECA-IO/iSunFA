...
export enum MeasurementUnit {
  ...
  // Info: (20260525 - Tzuhan) --- 財務花費 (Spend-based) ---
  TWD = "TWD",
  USD = "USD",
...
export const UnitDimensionMap: Record<MeasurementUnit, PhysicalDimension> = {
  ...
  [MeasurementUnit.TWD]: PhysicalDimension.MONEY,
  [MeasurementUnit.USD]: PhysicalDimension.MONEY,
...
export const verifyDimensionalConsistency = (
  docUnit: MeasurementUnit,
  coefUnit: MeasurementUnit,
): boolean => {
  const docDim = UnitDimensionMap[docUnit];
  const coefDim = UnitDimensionMap[coefUnit];
