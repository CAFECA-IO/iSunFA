...
/**
 * Info: (20260805 - Tzuhan) 原文用來表示「本類別沒有細分項」的標記。
 *
 * 出現在開放類別(類別六)的子項欄。實測第三種版面把它獨立成一列:
 *
 * ```
 * | 類別六 | | NA | |      ← 類別小計;解析器的開放類別特例已從這裡產出資料列
 * | - | NA | | |           ← 同一筆資訊的重複,原文以 - 表示無細分項
 * ```
 *
 * 前一行已經記過了,這一行沒有新資料 —— 但它「有數值卻認不出主體」,
 * 於是被記成未解析列,而未解析列有獨立的否決權:
 * **勾稽 16 項全數通過,卻因為三列 `| - | NA | | |` 整張表不予入帳,桑基圖因此不畫。**
 *
 * 判準收緊到明表:非空儲存格恰好是「一個本標記 + 一個可判讀數值」才算。
 * 不用排除法 —— 「有數值但認不出子代碼」本來就該記成未解析,那道網不能放寬。
 */
export const TABLE38_NO_SUBITEM_TOKEN = "-";
