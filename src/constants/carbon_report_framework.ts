 ...
 *
 * Info: (20260824 - Luphia) **冒號原本漏了兩種寬度**(PR review)。
 *
 * 上一段自己列出了 `：→:` 這條轉換,清單卻只有逗號/分號/句號 ——
 * 於是冒號成為唯一擋不住的子句邊界,而它在報告裡比逗號更常出現
 * (「查證結論:」「揭露架構:」這類標籤位置)。
 *
 * 實測 #54 上線後外殼要印的那張封面 —— 允許的〔對齊聲明 + 免責句〕
 * 加一行報告本來就有的查證聲明:
 *
 *     本報告已通過第三方查證:
 ...
 *
 * 條 2、條 3 都正確滿足(`alignmentDeclared` 與 `disclaimerPresent` 皆真),
 * 條 4 卻命中 `通過第三方查證:本報告依IFRS` —— 一份完全合規的報告過不了驗收。
 *
 * 踩得到的原因是 `squeezeForMatch` 連換行一起壓掉:相鄰兩行變成一個連續
 * 字串,而擋跨句的唯一機制就是這個排除類。與上一段修掉的逗號跨句是
 * **同一個洞的同一種形狀**,只是漏了一個字元。紅測試見
 * `carbon_framework_claims.test.ts` 的冒號那兩條。
 *
 * ## 仍然漏得掉的(誠實寫出)
 *
