 ...
 *
 * 比對的是 `squeezeForMatch` 壓過的文字（NFKC + 去空白），所以樣式裡不留空白。
 *
 * ## 為什麼是 regex 而不是字串清單
 *
 * 1. **子字串比對看不到否定。** 「本報告尚不符合 IFRS S1」壓過之後含 `符合IFRS`，
 *    會被字串清單誤判成合規宣告 —— 而那句話的意思正好相反。
 *    否定回顧 `(?<![不未尚])` 把它排除。
 * 2. **重疊項會重複報。** 原本同時列 `符合IFRS` 與 `符合IFRSS1`，命中時兩條都中，
 *    讀起來像找到兩個違規。後者是前者的子集，直接刪掉。
 *
 * ⚠ **這份清單就是判準的界**：清單外的說法漏得掉 ——
 * 「已達成 IFRS S1 要求」「與 IFRS S2 一致」都不在清單裡，因為它們沒有合規動詞。
 * 刻意不做語意判斷（那會變成一個抓不準又擋不住的判準）。
 * 清單只能變長：每次真的在紙上看到新說法就加一條，並在這裡記下它出自哪一趟。
 */
export const COMPLIANCE_CLAIM_PATTERNS: ReadonlyArray<RegExp> = [
  // Info: (20260821 - Emily) 「符合」系列。否定回顧排除「不符合」「未符合」「尚符合」
  /(?<![不未尚])符合IFRS/,
  // Info: (20260821 - Emily) 「遵循」「依循」系列
  /(?<![不未尚])遵循IFRS/,
  /(?<![不未尚])依循IFRS/,
  // Info: (20260821 - Emily) 查證語境:「通過 IFRS S1 查證」
  /(?<![不未尚])通過IFRS/,
];
