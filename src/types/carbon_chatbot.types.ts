...
export interface ICarbonInventoryState {
   ...
   * 匯入第二份**帶不同年度**的報告就長出來了。匯入成功入帳時以當時的 state.year 存快照;
   *
   * Info: (20260831 - Emily) ⚠ **目前真實路徑走不到那個條件**(PR #6725 review R1)。
   * 原文寫「同房匯入第二份年度報告就長出來了」,而 `state.year` 是房間層級、
   * write-once(`carbon_inventory.ts` 的 `state.year ?? extraction.year`),
   * 匯入路徑也不帶年度進來 —— 所以同一間房匯兩份報告會拿到**同一個年度值**,
   * 這個 Record 永遠只有一個鍵,規則 3 的跨年度換鍋也永遠不觸發。
   * 年度必須來自**那份報告**(解析封面/章節,或匯入預覽卡讓使用者確認),
   * 而那是獨立的一張票(`issue_drafts/open/69`)。在它做完之前,
   * 這個欄位的能力只在單元測試手寫兩個年度時成立 —— 據實寫在這裡,不留假宣稱。
