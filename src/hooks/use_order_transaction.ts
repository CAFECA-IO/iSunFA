...
export const useOrderTransaction = () => {
  ...
  const executeOrderTransaction = async (
    ...
    onPaymentSuccess: (authData: { orderId: string; transactionHash: string } & AuthenticationJSON) => Promise<void>
  ) => {
    ...
    try {
      ...
      const transferAuth: AuthenticationJSON = await fido2ClientService.startLogin({
