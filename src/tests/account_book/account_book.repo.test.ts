...
describe('帳本建立功能測試', () => {
  ...
  // Info: (20250603 - Shirley) 模擬 Prisma 回傳值
  ...
  beforeEach(() => {
    ...
    // Info: (20250603 - Shirley) 模擬 transaction 函數
  ...
  describe('有效資料建立帳本成功', () => {
    ...
    it('應該在未提供圖片 ID 時生成預設圖片', async () => {
      // Info: (20250603 - Shirley) 準備測試數據 - 移除 fileId
  ...
  describe('稅務 ID 格式驗證', () => {
    it('應該接受有效的稅務 ID 格式', async () => {
      // Info: (20250603 - Shirley) 準備測試數據 - 有效稅務 ID
      const dataWithValidTaxId = {
        ...mockAccountBookData,
        taxId: '53212539', // Info: (20250603 - Shirley) 有效統編
      };
  ...
  describe('權限驗證', () => {
    it('應該檢查用戶是否有權限創建帳本', async () => {
      // Info: (20250603 - Shirley) 模擬用戶不是團隊成員
      mockPrisma.teamMember.findFirst = jest.fn().mockResolvedValue(null);

      // Info: (20250603 - Shirley) 模擬 convertTeamRoleCanDo 返回沒有權限
      mockConvertTeamRoleCanDo.mockReturnValue({
      ...
      // Info: (20250603 - Shirley) 驗證團隊成員查詢
  ...
  describe('訂閱方案限制檢查', () => {
    it('應該檢查團隊是否達到帳本數量限制', async () => {
      // Info: (20250603 - Shirley) 模擬達到帳本數量限制
  ...
  describe('重複帳本名稱處理', () => {
    it('應該在發現重複的稅務 ID 時拋出錯誤', async () => {
      // Info: (20250603 - Shirley) 模擬已存在相同稅務 ID 的帳本
  ...
  describe('資料庫約束驗證', () => {
    it('應該處理資料庫操作失敗的情況', async () => {
      // Info: (20250603 - Shirley) 模擬資料庫操作失敗
  ...
  describe('建立後預設設定初始化', () => {
    it('應該正確初始化會計設定', async () => {
      ...
      // Info: (20250603 - Shirley) 驗證會計設定初始化
      expect(mockPrisma.accountingSetting.create).toHaveBeenCalledWith({
        data: expect.objectContaining({
          companyId: mockCreatedAccountBook.id,
          salesTaxTaxable: true,
  ...
  describe('地址處理', () => {
    ...
    it('應該處理缺少地址欄位的情況', async () => {
      // Info: (20250603 - Shirley) 準備測試數據 - 移除地址欄位
