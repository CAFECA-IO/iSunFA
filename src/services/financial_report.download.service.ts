...
export async function downloadFinancialReport(
  ...
  try {
    ...
    const htmlText = Buffer.from(await searchRes.arrayBuffer()).toString(
      "latin1",
    );
    ...
    const filenameMatch = htmlText.match(
      /([a-zA-Z0-9_]*F04[a-zA-Z0-9_]*\.pdf)/i,
    );

    if (!filenameMatch || !filenameMatch[1]) {
      console.warn(
        `⚠️ 找不到 ${stockId} (${year}) 的「股東會年報(F04)」(可能尚未上傳或非此格式)`,
      );
      return false;
    }
    ...
    if (!realUrlMatch) {
      console.error(
        `❌ 找不到過渡連結。預覽 HTML: ${step2Text.replace(/\s+/g, " ").substring(0, 200)}`,
      );
      return false;
    }
