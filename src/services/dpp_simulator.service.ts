...
export class DppSimulatorService {
  ...
  public async generateDownloadZip(
    stockId: string,
    year: string,
    skuId: string,
    cwd: string,
    ...
    try {
      const baseOutputsDir = path.join(cwd, "data", stockId, year, "outputs");
      if (!fs.existsSync(baseOutputsDir)) {
        throw new Error("Company data not found. Please generate first.");
      }
      ...
      if (
        ...
        throw new Error("Original mock sources missing.");
      }
      ...
      if (fs.existsSync(originalSpecsPath)) {
        const specsData = JSON.parse(
          fs.readFileSync(originalSpecsPath, "utf-8"),
        );
        fs.writeFileSync(tmpSpecsPath, JSON.stringify(specsData, null, 2));
      }

      const { convertJsonToCsv } =
        await import("@/scripts/e2e_seeder/dpp/convert_json_to_csv");
      const { convertSpecsToCsv } =
        await import("@/scripts/e2e_seeder/dpp/convert_specs_to_csv");

      await convertJsonToCsv(stockId, year, { baseDirOverride: tmpDir });
      ...
      return {
        buffer: zipBuffer,
        filename: `DPP_Simulation_${stockId}_${year}_${skuId}.zip`,
      };
    } finally {
