...
// Info: (20260721 - Emily) UAT:UNMAPPED_UNIT 移除 — precomputed 紀錄不重算,金額基準(TWD)等
// Info: (20260721 - Emily) 非物理單位僅作顯示,原樣保留即可,不構成跳過理由
export enum CarbonEsgSkipReasonEnum {
  // Info: (20260720 - Emily) AI 分析未完成(PENDING/PROCESSING/FAILED)的紀錄不可入報告
  NOT_COMPLETED = "NOT_COMPLETED",
  // Info: (20260720 - Emily) 範疇無法決定性判定(無 GHG 子分類、無活動類型映射、scope 缺漏或為
  // Info: (20260720 - Emily) 裸 SCOPE_3):15 類不可代猜
...
/**
 * Info: (20260720 - Emily) 範疇裁決(決定性優先序):
...
export class CarbonEsgLinkService {
  ...
  async listBookActivities(
    ...
    records.forEach((record) => {
      ...
      // Info: (20260721 - Emily) 單位:可正規化者取 MeasurementUnit(利於分組/守恆對齊);
      // Info: (20260721 - Emily) 金額基準等非物理單位原樣保留 — precomputed 不重算,單位僅顯示用
