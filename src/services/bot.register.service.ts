...
export class RegisterBotService {
  ...
  public async registerAndLogin(
    ...
    const code = await publicClient.getBytecode({
      address: scwAddress as `0x${string}`,
    });
    const isDeployed = code && code !== "0x";

    if (!isDeployed) {
      console.log(
        `[BotService] Deploying SCW for bot '${username}' at ${scwAddress}...`,
      );
      ...
      const authJson = this.createMockAuthenticationJSON(
        challengeBase64,
        credentialID,
        privKeyBytes,
      );
      ...
      const result = await bundlerService.sendUserOpAsync(
        finalUserOp,
        CONTRACT_ADDRESSES.ENTRY_POINT,
      );
      console.log(
        `[BotService] SCW Deployment submitted! Tx: ${result.transactionHash}. Waiting for chain confirmation...`,
      );
    ...
    const loginAuthJson = this.createMockAuthenticationJSON(
      loginChallenge,
      credentialID,
      privKeyBytes,
    );
    const loginResult = await webAuthnService.loginWithCredential(
      challengeTokenAndData.token,
      loginAuthJson,
    );
   ...
   */
  public createMockAuthenticationJSON(
    ...
    const clientDataJSONBase64 =
      Buffer.from(clientDataJSONStr).toString("base64url");
