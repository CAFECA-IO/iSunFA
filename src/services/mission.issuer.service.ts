...
export class MissionIssuerService {
  async processNext() {
    ...
    try {
      ...
      let baseRewardBigInt = parseEther(Math.max(0, computedCost / 2).toString());
      ...
      if (baseRewardBigInt < minRewardWei) {
        console.log(`[MissionIssuer] Reward ${formatEther(baseRewardBigInt)} ICP is below minReward ${formatEther(minRewardWei)} ICP, padding it...`);
...
## Verifications
1. Check if the output follows the expected analysis structure for category: ${(missionData as { category?: string }).category}.
...
`;
      ...
      await prisma.order.update({
        ...
        data: {
          ...
          mission: taskId // Info: (20260420 - Luphia) storing the Mission NFT TaskID as string
        }
      });

      console.log(`[MissionIssuer] Order ${order.id} transitioned to EXECUTING. NFT Task ID: ${taskId}`);
      return taskId;
