// Info: (20260720 - Tzuhan) 質量守恆勾稽護欄(#6520 / issue 22):純決定性檢核,無 LLM、無 DB、無狀態
// Info: (20260720 - Tzuhan) 等式:期初庫存 + 本期採購 = 本期投入(消耗) + 期末庫存(± LOSS_RATIO_TOLERANCE)
// Info: (20260720 - Tzuhan) 哲學:violation 不擋資料入帳(資料仍是事實),但凍結報告數據段落並讓費思追問;
// Info: (20260720 - Tzuhan) 非庫存類(電力/運輸)改跑合理性區間,超界僅警示(可能是集團級數據,不武斷凍結)
// Info: (20260720 - Tzuhan) 全程 Decimal 字串經 MoneyUtil,不經 number 中轉(ADR 003)
...
// Info: (20260720 - Tzuhan) 從零依賴 lib 取用(不 import calculation service:它連動 EmissionFactorRepo → prisma)
...
// Info: (20260720 - Tzuhan) 物料名稱決定性正規化:去空白 + 小寫(庫存紀錄 ↔ 活動數據的對齊鍵)
export const normalizeMaterialName = (name: string): string =>
  name.trim().toLowerCase();

// Info: (20260720 - Tzuhan) 單位是否屬可盤點物料(質量/體積)
...
export class CarbonArticulationService {
  /**
   * Info: (20260720 - Tzuhan) 全量勾稽(冪等:同輸入同輸出):
  ...
  check(
    ...
    if (violations.length > 0) {
      ...
      // Info: (20260720 - Tzuhan) 純電力/運輸盤查無庫存紀錄屬合法情境:不適用而非未通過
  ...
  /**
   * Info: (20260720 - Tzuhan) 單筆守恆檢核。回傳 null = 通過(含容差內損耗);非 null = violation。
  ...
  private checkConservation(
    ...
    // Info: (20260720 - Tzuhan) 消耗側加總:同名活動(正規化比對)換算至庫存單位;跨量綱 = 無法驗證 → 凍結
    ...
    for (const activity of matched) {
      ...
      if (!parsed) {
        // Info: (20260720 - Tzuhan) 消耗數值無法解析:計算引擎已列待補,守恆側視為無法驗證
        return {
          ...base,
          reason: ArticulationViolationReasonEnum.UNPARSABLE_QUANTITY,
          expectedConsumption: expected,
    ...
    // Info: (20260720 - Tzuhan) 缺口 ≤ 預期消耗 × 損耗容差 → 合理損耗放行;預期為 0 時實際也必須為 0
  ...
  // Info: (20260720 - Tzuhan) 非庫存類合理性區間:超出物理量級邊界僅警示(per unit 常數;無法解析者交由計算引擎待補)
