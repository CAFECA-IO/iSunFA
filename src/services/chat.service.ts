...
export class ChatService {
  ...
  /**
   * Info: (20260427 - Julian) 向 AI 提問，並使用串流返回答案與關聯標籤
   */
  async askAccountTalkStream(
    message: string,
    images: { data: string; mimeType: string }[] = [],
    onChunk: (partialAnswer: string) => void | Promise<void>,
  ): Promise<{ answer: string; tags: string[] }> {
    const prompt = AI_CONSULTATION_ROOM_PROMPT.replace("{{message}}", message);
    ...
    try {
      ...
      for await (const chunk of result.stream) {
        ...
        let partialAnswer = fullText;
        const answerMatch = fullText.match(/"answer"\s*:\s*"([\s\S]*?)"(?:\s*,|\s*\})/);
        if (answerMatch) {
          partialAnswer = answerMatch[1];
        } else {
          const openMatch = fullText.match(/"answer"\s*:\s*"([\s\S]*)/);
          if (openMatch) {
            partialAnswer = openMatch[1];
          } else {
            // Info: (20260427 - Julian) JSON 尚未產生到 answer 欄位，所以略過
            continue;
          }
        }
        
        // Info: (20260427 - Julian) 解碼 JSON 字串的跳脫字元
        partialAnswer = partialAnswer.replace(/\\n/g, '\n').replace(/\\"/g, '"');
        await onChunk(partialAnswer);
      }
      ...
      console.error("[ChatService] Error in askAccountTalkStream:", error);
      return { answer: "AI 暫時無法回答，請稍後再試。", tags: ["錯誤"] };
    }
  }
