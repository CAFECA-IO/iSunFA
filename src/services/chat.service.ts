...
// Info: (20260714 - Tzuhan) SchemaType 一併由此 re-export:所有 AI 串接(含 responseSchema 定義)統一經 chat.service,
// Info: (20260714 - Tzuhan) 其他服務不得直接 import @google/generative-ai(未來切換本地模型只需改本檔)
export { SchemaType };
...
export class ChatService {
  ...
  async generateContent(
    ...
    // Info: (20260716 - Tzuhan) 經防護執行器呼叫;未帶 timeoutMs/taskKey 時行為與原裸呼叫相同
    const result = await this.invokeGuarded(() => model.generateContent(parts), {
  ...
  /**
   * Info: (20260714 - Tzuhan) 碳會計師人設(單一來源):結構化回覆與招呼詞共用,避免 prompt 漂移
   */
...
${outlineCatalog}${langInstruction}`;
  ...
  /**
   * Info: (20260714 - Tzuhan) 碳會計師結構化回覆:對話內容 + 段落完成訊號(碳盤查對 Gemini 的唯一對話路徑)
   * Info: (20260714 - Tzuhan) 解決「無限訪談迴圈」:AI 判斷段落資訊已齊全時回報 readyParagraphId,
   * Info: (20260714 - Tzuhan) 由路由層觸發 ParagraphDraftService 寫入報告;id 經 enum 約束 + 本方法白名單裁決
   */
  async generateCarbonChatbotStructuredResponse(
    ...
    // Info: (20260716 - Tzuhan) 同步聊天路徑:45s 逾時 + 用量記錄(#6515)
   ...
   * Info: (20260714 - Tzuhan) 改走結構化回覆(移除重複的純文字對話方法,人設單一來源);招呼詞只取 reply
   */
