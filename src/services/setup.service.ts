...
// Info: (20260414 - Luphia) 共用常數與輔助函式 (Helper Functions)
const ROOT_PATH = process.cwd();
const ENV_PATH = path.join(ROOT_PATH, ".env");
const ENV_SETUP_PATH = path.join(ROOT_PATH, ".env.setup");
const ENV_EXAMPLE_PATH = path.join(ROOT_PATH, ".env.example");

// Info: (20260414 - Luphia) 更新或附加環境變數，統一處理正則表達式取代邏輯
function updateOrAppendEnv(content: string, key: string, value: string): string {
...
async function loadEnvConfig(targetPath: string): Promise<Record<string, string>> {
  if (!fs.existsSync(targetPath)) return {};
  const dotenv = await import("dotenv").then(m => (m as unknown as { default?: typeof m }).default || m);
  return dotenv.parse(fs.readFileSync(targetPath, "utf8"));
}

// Info: (20260414 - Luphia) 取得優先的環境變數設定 (先讀取 .env.setup，若無則讀取 .env)
async function getPriorityEnvConfig(): Promise<Record<string, string>> {
  const targetPath = fs.existsSync(ENV_SETUP_PATH) ? ENV_SETUP_PATH : (fs.existsSync(ENV_PATH) ? ENV_PATH : null);
  if (targetPath) {
    return await loadEnvConfig(targetPath);
  }
  return {} as Record<string, string>; // Info: (20260414 - Luphia) 明確轉型，安撫 TypeScript
}
...
export async function startDockerCompose() {
  ...
  envContent = updateOrAppendEnv(envContent, "POSTGRES_PASSWORD", `"${newPassword}"`);
  ...
  setupContent = updateOrAppendEnv(setupContent, "STORAGE_DOMAIN", "http://127.0.0.1:20022");
  setupContent = updateOrAppendEnv(setupContent, "NEXT_PUBLIC_RPC_URL", "https://mainnet.isuncoin.com");
...
export async function getAdminWalletInfo() {
  try {
    ...
    let isMining = false;
    try {
      const miningRes = await dockerService.execContainer("blockchain", `isuncoin attach --exec "eth.mining" http://127.0.0.1:20024`);
    ...
    return { success: true, address, balance: balanceEth, isfBalance, isMining: !!isMining };
  } catch (error: unknown) {
...
export async function getDatabaseStatus() {
  try {
    ...
    const content = fs.existsSync(schemaPath) ? fs.readFileSync(schemaPath, "utf8") : "";
    const tableCount = (content.match(/^model\s+/gm) || []).length;

    const dbUrlString = envConfig.DATABASE_URL ? envConfig.DATABASE_URL.replace(/^"(.*)"$/, '$1') : "postgresql://isunfa@127.0.0.1:20021/isunfa";
    ...
    return { success: true, tableCount, dbPassword: dbPassword.replace(/^"(.*)"$/, '$1'), dbHost, dbPort };
  } catch (e) {
    return { success: false, error: String(e), tableCount: 0, dbPassword: "", dbHost: "127.0.0.1", dbPort: "20021" };
  }
}
...
export async function checkSuperAdminExists(): Promise<{ exists: boolean, address?: string, needsAuth?: boolean, credId?: string }> {
  try {
    ...
    const adminUser = await localPrisma.user.findFirst({ where: { role: "SUPER_ADMIN" } });
...
export async function authorizeSuperAdmin(): Promise<{ success: boolean, address?: string, error?: string, pendingTask?: boolean }> {
  try {
    const localPrisma = await getDynamicPrisma();
    const user = await localPrisma.user.findFirst({ where: { role: "SUPER_ADMIN" } });

    if (user) {
      const res = await createSuperAdminRecord(user.credentialId!, user.pubKeyX!, user.pubKeyY!);
      await localPrisma.$disconnect();
      return res;
    }
...
export async function createSuperAdminRecord(credentialId: string, pubKeyX: string, pubKeyY: string, name: string = "ISUNFA SUPER ADMIN"): Promise<{ success: boolean, address?: string, error?: string, pendingTask?: boolean }> {
  try {
    ...
    if (!factoryAddress) return { success: false, error: "NEXT_PUBLIC_SCW_FACTORY_ADDRESS is not set." };
    ...
    const localPrisma = await getDynamicPrisma();
    const existingByCred = await localPrisma.user.findUnique({ where: { credentialId } });

    if (existingByCred) {
      if (existingByCred.address !== address.toLowerCase()) {
        await localPrisma.user.update({
          where: { credentialId },
          data: { address: address.toLowerCase(), pubKeyX, pubKeyY }
        });
      }
    } else {
      const existingByAddr = await localPrisma.user.findUnique({ where: { address: address.toLowerCase() } });
      if (!existingByAddr) {
        await localPrisma.user.updateMany({ where: { role: "SUPER_ADMIN" }, data: { role: "USER" } });
        await localPrisma.user.create({
          data: { address: address.toLowerCase(), pubKeyX, pubKeyY, credentialId, role: "SUPER_ADMIN", name },
        });
      }
    }
    await localPrisma.$disconnect();

    const globalAny = global as typeof globalThis & { superAdminTaskStatus?: { done: boolean, error: string | null, progress: string } };
    globalAny.superAdminTaskStatus = { done: false, error: null, progress: "Initializing admin wallet..." };

    const doBlockchainSetup = async () => {
      try {
        let adminAccount;
        try { adminAccount = await getAdminAccount(); } catch { }

        if (adminAccount) {
          globalAny.superAdminTaskStatus = { done: false, error: null, progress: "Connecting to Blockchain..." };
          const setupConfig = await loadEnvConfig(ENV_SETUP_PATH);
          ...
          if (setupConfig.NEXT_PUBLIC_KYC_REGISTRY_ADDRESS) {
            globalAny.superAdminTaskStatus = { done: false, error: null, progress: "Setting KYC Levels..." };
            const irAddress = setupConfig.NEXT_PUBLIC_KYC_REGISTRY_ADDRESS as `0x${string}`;
            const irAbi = parseAbi(["function updateKYC(address, uint8) external", "function getKYCLevel(address) view returns (uint8)"]);

            const kycLevel = await publicClient.readContract({ address: irAddress, abi: irAbi, functionName: "getKYCLevel", args: [address] });
            if (kycLevel === 0) {
              const tx = await mainWalletClient.writeContract({ address: irAddress, abi: irAbi, functionName: "updateKYC", args: [address, 1] });
              await publicClient.waitForTransactionReceipt({ hash: tx });
            }

            const adminKycLevel = await publicClient.readContract({ address: irAddress, abi: irAbi, functionName: "getKYCLevel", args: [adminAccount.address] });
            if (adminKycLevel === 0) {
              const adminTx = await mainWalletClient.writeContract({ address: irAddress, abi: irAbi, functionName: "updateKYC", args: [adminAccount.address, 1] });
          ...
          for (const contractAddr of contractsToTransfer) {
            globalAny.superAdminTaskStatus = { done: false, error: null, progress: `Granting Admin Role...` };
          ...
          if (setupConfig.NEXT_PUBLIC_CREDIT_POINT_ADDRESS) {
            globalAny.superAdminTaskStatus = { done: false, error: null, progress: "Minting collateral..." };
        ...
        globalAny.superAdminTaskStatus = { done: true, error: null, progress: "Completed" };
      } catch (e) {
        console.error("Error securing contracts:", e);
        globalAny.superAdminTaskStatus = { done: true, error: String(e), progress: "Failed" };
      }
    };
    ...
    if (fs.existsSync(ENV_SETUP_PATH)) {
      let setupContent = fs.readFileSync(ENV_SETUP_PATH, "utf-8");
      if (!setupContent.includes("# PART 4")) setupContent += "\n\n# PART 4: Server SUPER ADMIN\n";
      ...
      setupContent = updateOrAppendEnv(setupContent, "DEWT_PRIVATE_KEY_PEM", `"${privateKey.replace(/\n/g, "\\n")}"`);
      setupContent = updateOrAppendEnv(setupContent, "SUPER_ADMIN_CRED_ID", credentialId);
      setupContent = updateOrAppendEnv(setupContent, "SUPER_ADMIN_PUB_X", pubKeyX);
      setupContent = updateOrAppendEnv(setupContent, "SUPER_ADMIN_PUB_Y", pubKeyY);
...
export async function getSuperAdminTaskStatus() {
  return (global as typeof globalThis & { superAdminTaskStatus?: { done: boolean, error: string | null, progress: string } }).superAdminTaskStatus || { done: true, error: null, progress: "Idle" };
}
...
export async function getEnvHashChallenge(): Promise<{ success: boolean; challenge?: string; error?: string }> {
  try {
    if (!fs.existsSync(ENV_SETUP_PATH)) return { success: false, error: ".env.setup not found" };

    const baseConfig = await loadEnvConfig(ENV_PATH);
    const setupConfig = await loadEnvConfig(ENV_SETUP_PATH);
...
export async function verifyAndFinalizeConfig(authData: AuthenticationJSON): Promise<{ success: boolean; error?: string }> {
  try {
    if (!fs.existsSync(ENV_SETUP_PATH)) return { success: false, error: "Setup file not found" };

    const envConfig = await loadEnvConfig(ENV_SETUP_PATH);
    const pubX = envConfig.SUPER_ADMIN_PUB_X;
    const pubY = envConfig.SUPER_ADMIN_PUB_Y;
    const credId = envConfig.SUPER_ADMIN_CRED_ID;

    if (!pubX || !pubY || !credId) return { success: false, error: "Credentials not found in setup file." };

    // Info: (20260413 - Luphia) Normalize base64url strings to ignore optional padding
    const safeAuthId = authData.id.replace(/=/g, '');
    const safeCredId = credId.replace(/=/g, '');
    if (safeAuthId !== safeCredId) return { success: false, error: "Wrong FIDO credential used." };

    const challengeRes = await getEnvHashChallenge();
    if (!challengeRes.success || !challengeRes.challenge) return { success: false, error: "Challenge hashing failed." };
...
export async function saveExternalConfig(config: { appUrl: string, gaId: string, geminiKey: string, oenToken: string, oenMerchant: string }): Promise<{ success: boolean; error?: string }> {
  try {
    let content = fs.existsSync(ENV_SETUP_PATH) ? fs.readFileSync(ENV_SETUP_PATH, "utf-8") : "";

    if (!content.includes("# PART 5")) content += "\n\n# PART 5: External API Configuration\n";

    content = updateOrAppendEnv(content, "NEXT_PUBLIC_APP_URL", `"${config.appUrl}"`);
    if (config.gaId) content = updateOrAppendEnv(content, "NEXT_PUBLIC_GA_MEASUREMENT_ID", `"${config.gaId}"`);
    if (config.geminiKey) content = updateOrAppendEnv(content, "GEMINI_API_KEY", `"${config.geminiKey}"`);
    if (config.oenToken) content = updateOrAppendEnv(content, "OEN_ACCESS_TOKEN", `"${config.oenToken}"`);
    if (config.oenMerchant) content = updateOrAppendEnv(content, "OEN_MERCHANT_ID", `"${config.oenMerchant}"`);
...
export async function getAdminList() {
  try {
    const localPrisma = await getDynamicPrisma();
    const admins = await localPrisma.user.findMany({
      where: { role: { in: ["SUPER_ADMIN", "ADMIN"] } },
      select: { address: true, name: true, role: true, createdAt: true },
...
export async function createAdminRecord(credentialId: string, pubKeyX: string, pubKeyY: string, name: string = "SERVER ADMIN") {
  try {
    let factoryAddress = process.env.NEXT_PUBLIC_SCW_FACTORY_ADDRESS as `0x${string}`;
    const envConfig = await getPriorityEnvConfig();
    ...
    const abi = parseAbi(["function getAddress(bytes credentialId, uint256 pubKeyX, uint256 pubKeyY, uint256 salt) view returns (address)"]);
    const address = await publicClient.readContract({
      address: factoryAddress, abi, functionName: "getAddress",
      args: [stringToHex(credentialId), BigInt(pubKeyX), BigInt(pubKeyY), BigInt(0)],
    });
    ...
    if (!existing) {
      await localPrisma.user.create({ data: { address: address.toLowerCase(), pubKeyX, pubKeyY, credentialId, role: "ADMIN", name } });
    } else if (existing.role === "USER") {
      await localPrisma.user.update({ where: { address: address.toLowerCase() }, data: { role: "ADMIN", name } });
    }
    await localPrisma.$disconnect();
    ...
    try {
      ...
      if (adminAccount) {
        ...
        for (const contractAddr of contractsToTransfer) {
          const tx = await mainWalletClient.writeContract({
            address: contractAddr as `0x${string}`, abi: accessControlAbi, functionName: "grantRole",
            args: [DEFAULT_ADMIN_ROLE, address],
          });
          await publicClient.waitForTransactionReceipt({ hash: tx });
        }
