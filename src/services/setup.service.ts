...
export async function startDockerCompose() {
  ...
  setupContent = updateOrAppendEnv(setupContent, "NEXT_PUBLIC_RPC_URL", "http://127.0.0.1:20024");
...
): Promise<{ success: boolean; address?: string; error?: string; pendingTask?: boolean }> {
  try {
    ...
    const existingByCred = await setupRepo.findUserByCredentialId(dbUrl, credentialId);

    if (existingByCred) {
      if (existingByCred.address !== address.toLowerCase()) {
        await setupRepo.upsertSuperAdminByAddress(dbUrl, {
          address: address.toLowerCase(), credentialId, pubKeyX, pubKeyY, name: existingByCred.name || name,
        });
      }
    } else {
      const existingByAddr = await setupRepo.findUserByAddress(dbUrl, address.toLowerCase());
      if (!existingByAddr) {
        await setupRepo.downgradeAllSuperAdminsToUser(dbUrl);
        await setupRepo.createUser(dbUrl, {
          address: address.toLowerCase(), pubKeyX, pubKeyY, credentialId, role: "SUPER_ADMIN", name,
        });
      }
    }

    await setSuperAdminTaskStatus({ done: false, error: null, progress: "Initializing admin wallet..." });

    const doBlockchainSetup = async () => {
      try {
        await setSuperAdminTaskStatus({ done: false, error: null, progress: "Deploying SCW (Account Abstraction) Wallet..." });
        ...
        await setSuperAdminTaskStatus({ done: true, error: null, progress: "Completed" });
      } catch (e) {
        console.error("Error securing contracts:", e);
        await setSuperAdminTaskStatus({ done: true, error: String(e), progress: "Failed" });
      }
    };
...
export async function createAdminRecord(
  ...
  try {
    ...
    const dbUrl = await getDbUrl();
    const existing = await setupRepo.findUserByAddress(dbUrl, address.toLowerCase());

    if (!existing) {
      await setupRepo.createUser(dbUrl, {
        address: address.toLowerCase(), pubKeyX, pubKeyY, credentialId, role: "ADMIN", name,
      });
    } else if (existing.role === "USER") {
      await setupRepo.updateUser(dbUrl, address.toLowerCase(), { role: "ADMIN", name });
    }
