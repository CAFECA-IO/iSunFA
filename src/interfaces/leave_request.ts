...
export interface ILeavePolicySnapshot {
   ...
   *
   * 目前只用來**擋下送出、試算與核准這三個入口** —— 併計扣減尚未實作
   * （計畫書 §17 缺口 17），而放行會讓法定額度被繞過：請滿 7 日家庭照顧假
   * 之後事假仍是完整 14 日。
   *
   * 更正（review 第二輪 R2）：這裡先前寫的是「只用來擋下送出」，
   * 而那句話描述的正是那個缺陷本身 —— `approve()` 沒有閘，
   * HR 事後把併計目標設起來就繞得過去，不需要任何舊資料。
   *
