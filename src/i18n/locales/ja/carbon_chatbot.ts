export const carbonChatbot = {
  ...
  // Info: (20260904 - Emily) 失敗ではなく拒否:その一文を消すまで再試行しても変わらない
  ...
  // Info: (20260828 - Julian) zh_tw と同じ：この文言には既知の誤りが 2 つある（計画 §4、未修正）
  ...
  import_interrupted_chapters:
    "前回のインポートは完了しませんでした。次の章はまだ解析されていません：{{chapters}}。ここから続けられます。完了済みの部分は再実行されず、追加のポイントもかかりません。",
  // Info: (20260901 - Luphia) 「可能」不是贅字（review #6726 中-2）：BUSY 也可能是自己的分頁崩潰後租約未過期
  import_job_busy:
    "このインポートは別のタブまたは別の端末で実行中の可能性があります。あるいは前回が正常に終了しなかった場合もあり、その場合は最長 {{minutes}} 分後に再試行できます。二重に実行するとポイントも二重に消費されます。",
  // Info: (20260901 - Luphia) 三個終局判決各說各的話（review #6726 阻-1）
  import_job_cancelled:
    "このインポートはキャンセル済みです。再開されず、ポイントもこれ以上消費されません。やり直すには、ファイルをもう一度アップロードしてください。",
  import_job_completed_already:
    "このインポートはすでに完了しています。再開するものはありません。",
  import_job_forbidden:
    "このアカウントには、このインポートへのアクセス権がありません。",
