export const common = {
  ...
  error: {
    ...
    // Info: (20260903 - Emily) 内容のレッドラインはシステム障害ではない:再試行しても変わらない
    pdf_framework_claim:
      "報告書の紙面に「当社は {{name}} に準拠している」といった主体レベルの適合宣言があるため、書き出しを中止しました。PDF は一度出力するとシステムの外に出てしまいます。その一文を削除してから書き出してください（文書構成についての記述は対象外です）。",
    pdf_download_needs_preview:
      "ダウンロードの前に「PDF プレビュー」に切り替えてください —— PDF の内容はプレビューと同じです(目次とページ番号が付きます)",
