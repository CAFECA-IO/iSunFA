export const digitalProductPassport = {
  title: "디지털 상품 여권",
  description: "B2B SKU 관리 및 제조 배치 발행.",
  login_to_use: "디지털 제품 여권을 사용하려면 로그인해 주세요",
  define_sku: "새 SKU 정의",
  define_sku_desc: "AI 추출을 위한 기술 문서 업로드",
  create_sku: "SKU 생성",
  sku_name: "SKU 이름",
  company_name: "회사 이름",
  gtin: "GTIN",
  batch_production: "여권 생성",
  batch_production_desc: "새 제조 배치 발행",
  issue_batch: "배치 발행",
  view_passport: "여권 보기",
  recent_skus: "최근 SKU",
  no_recent_skus: "최근 SKU가 없습니다. 시작하려면 첫 번째 SKU를 정의하세요.",
  select_sku_for_batch:
    "작업 필요: 새 생산 배치를 발행하려면 기존 SKU를 선택하세요.",
  sku_creation: {
    title: "제품 SKU 정의",
    subtitle:
      "제품 매뉴얼 및 보고서를 업로드하세요. AI가 디지털 상품 여권 구조를 구축합니다.",
    upload_failed: "업로드 실패: ",
    upload_error: "업로드 중 오류가 발생했습니다.",
    ai_analyzing: "AI가 문서를 분석 중입니다...",
    ai_analyzing_desc: "데이터를 추출하고 DPP 사양에 맞추는 중",
    uploading: "업로드 중...",
    start_extraction: "AI 추출 시작",
    drag_drop: "문서 끌어다 놓기",
    upload_desc:
      "사용자 매뉴얼, BOM, LCA 보고서 및 규정 준수 인증서를 업로드하세요. 지원 형식: PDF, DOCX, JSON.",
    or: "또는",
    browse_files: "파일 찾아보기",
    ai_status: "AI 추출 상태",
    mapping_desc: "문서를 DPP 스키마에 매핑 중",
    modules: {
      m1: "1. 제품 및 제조업체 정보",
      m2: "2. 생산 배치",
      m3: "3. 환경적 영향",
      m4: "4. 순환성 및 효율성",
      m5: "5. 내구성 및 폐기",
      m6: "6. 기술 매뉴얼",
      m7: "7. 규정 준수 감사",
      m8: "8. 공급망 추적성",
      m9: "9. 재료 구성",
    },
  },

  batch_creation: {
    fill_required: "모든 필수 필드를 입력해주세요.",
    success: "배치가 성공적으로 생성되었습니다!",
    failed: "배치 생성 실패",
    error: "오류가 발생했습니다",
    title: "생산 배치 발행",
    subtitle: "SKU에서 유전자 데이터를 상속하여 새 배치 여권 생성: ",
    batch_number: "배치 번호",
    batch_number_ph: "예: BATCH-202605-001",
    manufacture_date: "제조일",
    facility_site: "제조 시설",
    facility_site_ph: "예: 타이베이 A 공장",
    serial_range: "일련 번호 범위 (선택 사항)",
    serial_range_ph: "예: SN1000 - SN1999",
    generate: "디지털 상품 여권 생성",
    ai_extracting: "배치 정보를 추출하는 중...",
    ai_extraction_complete:
      "AI 추출이 완료되었습니다. 양식이 자동 완성되었습니다.",
    upload_desc: "AI 인식 입력을 위해 생산 관련 문서 업로드",
  },
  sku_diagnostics: {
    title: "SKU 진단",
    id: "ID: ",
    issue_batch: "생산 배치 발행",
    ready_title: "여권 준비 완료",
    action_required_title: "조치 필요",
    ready_desc: "모든 필수 모듈이 성공적으로 추출 및 검증되었습니다.",
    action_required_desc:
      "일부 필수 모듈이 누락되었습니다. SKU를 완료하려면 필요한 문서를 업로드하세요.",
    readiness: "준비 상태",
    successfully_extracted: "성공적으로 추출됨",
    gap_analysis: "격차 분석 대시보드",
    gap_desc: "AI가 규정 준수에 필요한 누락된 정보를 식별했습니다.",
    issues_found: "개의 문제 발견됨",
    no_gaps: "격차 없음",
    no_gaps_desc: "이 SKU는 모든 요구 사항을 충족합니다.",
    upload_doc: "이 섹션에 대한 문서 업로드",
    status: {
      READY: "준비됨",
      AUDITING: "감사 중",
      INCOMPLETE: "데이터 누락",
    },
  },

  list: {
    simulator_title: "DPP 시뮬레이터 데이터 생성기",
    simulator_desc:
      "공개 재무 및 지속 가능성 보고서를 사용하여 데모 및 테스트용 디지털 제품 여권(DPP) 모의 데이터 소스를 자동으로 생성합니다.",
    login_to_use: "DPP 시뮬레이터 데이터 생성기를 사용하려면 로그인해 주세요",
    create_simulation: "새 시뮬레이션",
    enter_data_center: "데이터 센터로 이동",
    no_simulations:
      "시뮬레이션된 기업이 없습니다. '새 시뮬레이션'을 클릭하여 첫 데이터 세트를 생성하세요.",
    simulator_no_simulations_desc:
      "상장 기업을 검색하여 실감 나는 디지털 제품 여권(DPP) 시뮬레이션 데이터를 자동으로 생성할 수 있습니다.",
    year_unit: "년",
    demo_data_base: "2025 (데모 데이터 베이스)",
    last_year: "2024 (지난해)",
    previous_year: "2023 (이전해)",
    delete_confirm_title: "삭제 확인",
    delete_confirm_message:
      "'{{name}} ({{year}}년)'의 시뮬레이션 데이터를 삭제하시겠습니까? 이 작업은 실행 취소할 수 없습니다.",
    delete_failed: "삭제에 실패했습니다. 나중에 다시 시도해 주세요.",
    simulation_status: "시뮬레이션 상태",
    simulation_progress: "시뮬레이션 진행 상황",
    has_data: "데이터 있음",
    no_data: "데이터 없음",
    phase1: "1단계: 기초 데이터",
    esg_download: "ESG 다운로드",
    esg_extrapolate: "ESG 추정",
    phase23: "2 & 3단계: 카탈로그 및 제품 DPP",
    product_count: "제품 수: ",
    enter_workspace: "작업 공간 들어가기",
    company_info: "기업 정보",
    progress_status: "진행 상태",
    action_buttons: "작업 버튼",
  },
  start: {
    initialize_simulation: "기업 시뮬레이션 데이터베이스 초기화",
    search_company: "대상 기업 검색",
    search_company_placeholder: "회사명 또는 통합 사업자 번호 입력...",
    simulation_year: "시뮬레이션 연도",
    start_simulation: "초기화 시작",
    start_generation: "추정 및 분석 스크립트 실행 시작",
    unknown_error: "알 수 없는 오류가 발생했습니다",
    step1: "1. 재무 보고서 및 공개 데이터 추출",
    step2_extrapolate: "2. ESG 연도 간 추정 (타임머신)",
    step2: "2. ESG 지속가능성 보고서 및 지표 추출",
    step3: "3. AI 시각적 차트 추출 (ai_vision_extractor)",
    step4: "4. 기업 페르소나 구축 (persona_generator)",
    step5: "5. BOM 및 전구체 데이터 구축 (generate_bom_precursors)",
    step6: "6. 제품 사양 생성 (generate_product_specs)",
    step7: "7. 제품 설계도 생성 (generate_product_image)",
    step8: "8. LCA 및 탄소 발자국 데이터 계산 (generate_lca_ground_truth)",
    step9:
      "9. 제품 규정 준수 및 검증 데이터 생성 (generate_compliance_declaration)",
    download_failed: "다운로드에 실패했습니다. 나중에 다시 시도해 주세요!",
    simulator_title_phase1: "기업 시뮬레이션 데이터 센터",
    simulator_title_phase1_desc:
      "특정 기업을 위한 고품질 모의 데이터를 생성하여 전체 다중 제품 관리 프로세스를 경험해 보세요.",
    target_enterprise_label: "대상 기업: ",
    year_label: "연도: ",
    simulation_matrix: "시뮬레이션 매트릭스",
    next_step_verify: "다음 단계: 검증",
    next_step_verify_desc:
      "제품별로 시뮬레이션 데이터가 생성되었습니다. 패키지를 다운로드하고 실제 업로드 프로세스를 진행하여 검증하세요.",
    go_to_create: "여권 생성으로 이동",
    baseline_details: "베이스라인 세부 정보 (기술 컨설턴트용)",
  },
  simulator: {
    empty_product_data:
      "제품 데이터가 없습니다. 우측 상단의 '시뮬레이션 제품 SKU 추가'를 클릭하세요.",
    group_manufacturer: "제조사 (Manufacturer)",
    ai_extrapolation: "✨ AI 추정",
    ai_extrapolation_principle:
      "AI 연도별 추정 원칙: 시스템이 전년도 지속가능성 보고서 기준을 자동으로 캡처하고, 금년도 최신 재무 및 매출 데이터를 결합하여 대규모 언어 모델을 통해 금년도의 탄소 배출 및 에너지 소비 지표를 동적으로 추정합니다.",
    header_title: "기업 목업 데이터 생성 센터",
    header_subtitle:
      "대상 기업을 지정하고 크롤러와 AI 추출 스크립트를 자동으로 트리거하여 디지털 제품 여권(DPP)의 기업 페르소나 및 기본 파일을 구축합니다.",
    group_company_policy: "기업 운영 및 정책 (Company Policy & Persona)",
    group_traceability:
      "🔗 공급망 추적성 및 자재 라이브러리 (Traceability & Material)",
    group_circularity: "순환성 및 효율성 정책 (Circularity)",
    group_product_specs:
      "📦 제품 사양 및 엔지니어링 데이터 (Product Specs & Engineering)",
    group_dpp_core: "📊 LCA 발자국 및 컴플라이언스 보고서",
    matrix_title: "제품 포트폴리오 및 차이(Gap) 설정 (고유)",
    matrix_desc:
      "다양한 제품 SKU에 대해 서로 다른 결함 시나리오를 설정하여 판정을 경험해 보세요.",
    add_sku: "시뮬레이션 제품 SKU 추가",
    scenario: "시나리오 설정",
    gap_settings: "차이(Gap) 설정 (이 제품 고유 데이터에만 해당)",
    gap_bom: "제품 및 물류 정보 (BOM)",
    gap_lca: "환경 영향 보고서 (LCA)",
    download_sku: "SKU 파일 패키지 생성 및 다운로드",
    view_sku_details: "SKU 데이터 세부정보 보기",
    baseline_manufacturer: "제조업체 (Manufacturer)",
    baseline_traceability: "공급망 추적성 (Traceability)",
    baseline_circularity: "순환성 및 효율성 정책 (Circularity)",
    baseline_compliance: "기술 매뉴얼 & 규정 준수 감사 정책 (Compliance)",
    baseline_material: "재료 구성 정책 (Material Composition)",
    baseline_title: "회사 공용 베이스라인 데이터 세트",
    baseline_desc:
      "이 데이터 세트는 재무 및 ESG 보고서에서 역설계되었습니다. 모든 제품 시뮬레이션의 공통 기반 역할을 합니다.",
    baseline_tab: "기업 Baseline",
    cbam_extrapolation_tooltip:
      "데이터의 높은 합리성을 보장하고 CBAM 신고 요건 준수: 시스템은 전년도 지속 가능성 보고서 기준을 자동으로 추출하고 이를 올해의 최신 재무 및 수익 데이터와 결합하여 대규모 언어 모델을 통해 올해의 탄소 배출 지표를 동적으로 추정합니다.",
    view_baseline_details: "기준선 생성 세부정보 보기",
    regenerate_baseline: "회사 공용 데이터 다시 생성",
    continue_baseline: "회사 공용 데이터 생성 계속",
    data_complete: "데이터 완성 (다운로드 가능)",
    data_incomplete: "데이터 처리 중이거나 불완전함",
    generate_all: "원클릭으로 모든 데이터 생성",
    company_selector_placeholder: "생성된 기업을 선택하세요...",
    company_selector_search: "기업명 또는 세금 ID 검색...",
    company_selector_not_found: "일치하는 생성된 기업을 찾을 수 없습니다",
  },
  sidebar: {
    search_placeholder: "사업자 번호 검색...",
    select_company_first: "먼저 기업을 선택해 주세요",
    mode: "모드",
    mode_accounting: "재무 회계",
    mode_carbon: "탄소 계산",
    mode_business: "기업 PR 분석",
    mode_catalog: "기업 제품 카탈로그(BOM) 생성",
    product_catalog: "제품 카탈로그",
    mode_dpp: "이 제품 전용 DPP 생성",
    product_list: "제품 목록",
    no_products: "없음",
    select_product: "제품을 선택해 주세요",
  },
  preview: {
    no_file: "미리 볼 수 있는 파일이 없습니다",
    support_formats:
      "미리 보려면 왼쪽 단계를 선택하세요 (PDF, JSON, PNG, HTML 지원)",
    generating: "생성 중...",
  },

  sidebar_extra: {
    target_enterprise: "대상 기업 (사업자 번호 / 기업명)",
    year: "연도",
    year_prediction: "2025 (연도별 추정)",
    processing: "처리 중...",
    download_reports: "보고서 및 데이터 다운로드",
    regenerate_persona: "기업 페르소나 재생성",
    execute_ai: "AI 추출 및 페르소나 생성 실행",
    regenerate_bom: "제품 카탈로그 (BOM) 재생성",
    generate_bom: "기업 제품 카탈로그 (BOM) 생성",
    select_product_dpp: "단일 제품 DPP를 생성할 제품 선택",
    regenerate_dpp: "단일 제품 DPP 재생성",
    generate_dpp: "이 제품 전용 DPP 생성",
    pipeline_execution: "파이프라인 실행 (Pipeline Execution)",
    extrapolation_alert_title:
      "해당 연도의 ESG 보고서를 찾을 수 없습니다. AI 연도별 추정 (Time-Machine)이 자동으로 적용되었습니다.",
    extrapolation_alert_desc:
      "시스템이 자동으로 과거 기준을 소급 추적하고 다음 핵심 원칙에 따라 연도별 동적 시뮬레이션을 수행했습니다.",
    extrapolation_bullet1:
      "거시 경제 예측: 글로벌 인플레이션 동향 및 공급망 변동성 요인 적용",
    extrapolation_bullet2:
      "수익 모멘텀 추정: 발표된 재무 보고서의 수익 모멘텀을 기반으로 생산 능력 및 에너지 소비 변화 추정",
    extrapolation_bullet3:
      "녹색 에너지 전환 추정: CBAM 압력 하에서 기업의 재생 에너지 조달 비율 증가 및 탈탄소화 경로 시뮬레이션",
    click_to_view: "소스 파일을 보려면 클릭하세요",
  },

  preview_extra: {
    view_generated_output: "생성된 출력 보기",
    file_not_found: "파일을 찾을 수 없음",
    file_not_found_desc:
      "지정된 파일을 찾을 수 없습니다. 생성 단계에서 오류가 발생했을 수 있습니다. 이 단계의 데이터를 다시 생성해 보세요.",
    regenerate_file: "이 데이터 다시 생성",
    json_title: "DPP / AI 분석 데이터 표시",
    json_view_desc: "시스템 통합 또는 분석에 적합한 구조화된 데이터입니다.",
    json_desc:
      "시스템이 원시 JSON 데이터를 시각적 구성 요소로 변환하여 쉽게 볼 수 있도록 했습니다.",
    regenerating_title: "데이터 생성 중...",
    regenerating_desc:
      "AI가 데이터를 다시 생성하고 있습니다. 잠시만 기다려주세요.",
    waiting_title: "항목을 선택하세요",
    waiting_desc: "관련 파일 또는 데이터를 미리 보려면 왼쪽 단계를 클릭하세요.",
    loading: "로딩 중...",
  },
  report_downloader: {
    title: "공개 재무 및 ESG 보고서 다운로더",
    description:
      "티커 기호와 연도를 입력하면 공공 출처에서 재무 및 지속 가능성 보고서를 자동으로 다운로드합니다.",
    target_enterprise: "대상 기업",
    year: "연도",
    downloading: "다운로드 중...",
    start_download: "다운로드 시작",
    processing_status: "처리 상태",
    downloaded_files: "다운로드한 파일",
    open_new_tab: "새 탭에서 열기",
    save_file: "파일 저장",
    download_failed: "다운로드 실패, 다시 시도해 주세요",
    download_complete: "다운로드 완료",
    preparing: "다운로드 준비 중...",
  },
  public_passport: {
    not_found_title: "여권을 찾을 수 없음",
    not_found_desc:
      "이 제품 배치의 디지털 상품 여권을 검색할 수 없습니다. 아직 생성되지 않았거나 링크가 올바르지 않을 수 있습니다.",
    go_to_dashboard: "대시보드로 이동",
    public_passport_tag: "공개 여권",
    download_pdf: "PDF 다운로드",
    passport_id: "여권 ID",
    verified_by: "TÜV Rheinland 인증 완료",
    general_info: "기본 정보",
    product_name: "제품명",
    model_number: "모델 번호",
    gtin: "GTIN",
    cn_code: "CN 코드",
    category: "카테고리",
    weight: "중량",
    facility: "시설",
    manufactured_date: "제조일",
    product_design_blueprint: "제품 설계도",
    batch_details: "배치 상세 정보",
    batch_number: "배치 번호",
    serial_range: "일련 번호 범위",
    carbon_footprint: "탄소 발자국 요약",
    methodology: "산정 방법:",
    cradle_to_gate: "ISO 14067 (Cradle-to-Gate)",
    precursors: "전구체",
    scope1: "스코프 1",
    scope2: "스코프 2",
    precursors_emissions: "전구체 배출량",
    direct_emissions: "직접 배출 (스코프 1)",
    indirect_emissions: "간접 배출 (스코프 2)",
    circularity_composition: "순환성 및 재료 구성",
    recycled_content: "재생 원료 함량",
    no_recycled_data: "사용 가능한 재생 원료 데이터가 없습니다.",
    recycled_label: "재생 비율:",
    pre_consumer: "소비 전",
    post_consumer: "소비 후",
    primary: "신규 원료",
    chemical_composition: "화학적 구성",
    no_composition_data: "사용 가능한 화학적 구성 데이터가 없습니다.",
    critical_raw_materials: "핵심 원자재",
    durability_repairability: "내구성 및 수리 가능성",
    physical_lifespan: "물리적 수명",
    years: "년",
    repair_instructions: "수리 지침",
    no_repair_instructions: "특별한 수리 지침이 없습니다.",
    end_of_life: "제품 수명 종료 / 폐기",
    disposal_desc: "현지 전자 폐기물 규정에 따라 폐기하십시오.",
    compliance_certs: "규정 준수 및 인증",
    rohs_compliant: "RoHS 준수",
    pfas_free_label: "PFAS 프리",
    social_responsibility: "사회적 책임",
    ethical_sourcing: "윤리적 조달",
    labor_standards: "노동 기준",
    verified: "인증됨",
    not_audited: "감사 전",
    compliant: "준수함",
    not_certified: "미인증",
    non_compliant: "미준수",
    contains_pfas: "PFAS 함유",
    pfas_free: "PFAS 프리",
    attached_declaration: "첨부된 선언서 문서",
    importer_logistics: "EU 수입업체 및 물류",
    importer_company: "수입 업체",
    eori_number: "EORI 번호",
    importer_address: "수입 업체 주소",
    cradle: "Cradle",
    to_gate: "to Gate",
    pre_consumer_val: "소비 전 ({{val}}%)",
    post_consumer_val: "소비 후 ({{val}}%)",
    primary_val: "신규 원료 ({{val}}%)",
    footer_disclaimer:
      "* 탄소 발자국은 ISO 14067 / CBAM 이행 규정 (EU) 2023/1773에 따라 평가되었습니다. 시스템 경계: Cradle-to-Gate.",
    footer_cn_code: "* 관세 통계 통합 품목 분류(CN) 코드 대상:",
    footer_powered_by:
      "iSunFA 기업 탄소 회계 시스템 지원 • 탈중앙화 신뢰 엔진을 통해 검증됨",
    all_rights_reserved: "All rights reserved.",
  },
};
