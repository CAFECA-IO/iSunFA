  ...
  try {
    ...
    return {
      ...
      openGraph: {
        ...
        images: [
          {
            url: `${baseUrl}/api/v1/share/og?token=${token}&lang=${ogLang}`,
            width: 1200,
            height: 630,
          },
        ],
...
export default async function PublicReportPage({
  ...
  try {
    ...
    const status =
      (
        shareRecord.analysis.order as { status?: string }
      )?.status?.toLowerCase() || "unknown";
    if (
      ["processing", "pending", "doing", "uploading", "paying"].includes(status)
    ) {
      safeData.safeMarkdown =
        "> **⏳ 報告正在生成中，請稍後重新整理本頁面查看結果...**";
