...
import {
  WizardSidebar,
  IWizardStep,
} from "@/components/admin/setup/wizard_sidebar";
...
export default function SetupWizardPage() {
  ...
  const STEPS: IWizardStep[] = [
    {
      ...
      detail: t("admin_setup.step1.desc"),
      component: SetupVerifyEngine,
    },
    {
      id: 2,
      title: t("admin_setup.step2.title"),
      detail: t("admin_setup.step2.desc"),
      component: SetupStartVerifyNodes,
    },
    {
      id: 3,
      title: t("admin_setup.step3.title"),
      detail: t("admin_setup.step3.desc"),
      component: SetupFundWallet,
    },
    {
      id: 4,
      title: t("admin_setup.step4.title"),
      detail: t("admin_setup.step4.desc"),
      component: SetupDeployContracts,
    },
    {
      id: 5,
      title: t("admin_setup.step5.title"),
      detail: t("admin_setup.step5.desc"),
      component: SetupInitDatabase,
    },
    {
      id: 6,
      title: t("admin_setup.step6.title"),
      detail: t("admin_setup.step6.desc"),
      component: SetupSuperAdmin,
    },
    {
      id: 7,
      title: t("admin_setup.step7.title"),
      detail: t("admin_setup.step7.desc"),
      component: SetupDomainConfig,
    },
    {
      id: 8,
      title: t("admin_setup.step8.title"),
      detail: t("admin_setup.step8.desc"),
      component: SetupFinalizeEnv,
    },
  ];
        ...
        if (res.success) {
          ...
          if (res.status === "COMPLETE") {
            ...
            (res.status === "MISSING_KEYS" &&
              res.missingKeys?.length === 1 &&
              res.missingKeys[0] === "SUPER_ADMIN_SIGNATURE")
          ) {
            setStep(8);
            setActiveTab(8);
      ...
      <div className="flex h-screen items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-4">
          <div className="relative">
            <Settings className="h-16 w-16 text-orange-500 opacity-20" />
            <Loader2 className="absolute top-1/2 left-1/2 h-8 w-8 -translate-x-1/2 -translate-y-1/2 animate-spin text-orange-500" />
          </div>
          <span className="animate-pulse font-medium text-slate-500">
            {t("admin_setup.page.init")}
          </span>
        </div>
      </div>
    );
  ...
  if (error) {
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-4">
          <ServerCrash className="h-16 w-16 text-rose-500" />
          <h2 className="text-xl font-bold text-slate-800">
            {t("admin_setup.page.fatal")}
          </h2>
          <div className="max-w-sm rounded-xl bg-rose-50 p-4 text-sm text-rose-700">
            <p className="mb-2 font-semibold">
              {t("admin_setup.page.fatal_desc")}
            </p>
            <code className="block rounded border border-rose-100 bg-white/50 p-2 font-mono text-xs break-all">
              {error}
            </code>
          </div>
          <button
            onClick={() => window.location.reload()}
            className="mt-2 rounded-lg bg-slate-800 px-6 py-2.5 font-medium text-white transition hover:bg-slate-700"
          >
  ...
  return (
    <div className="relative flex h-screen flex-col items-center overflow-hidden bg-slate-50 px-4 py-4 font-sans sm:py-6">
      <div
        className="absolute inset-x-0 -top-40 -z-10 transform-gpu overflow-hidden opacity-30 blur-3xl"
        aria-hidden="true"
      >
      ...
      <div className="flex h-full min-h-0 w-full max-w-6xl flex-col gap-4 sm:gap-6">
        <WizardHeader progressPercentage={progressPercentage} />

        <div className="flex min-h-0 w-full flex-1 flex-col gap-4 pb-4 sm:gap-6 lg:grid lg:grid-cols-12 lg:items-start lg:pb-0">
          <WizardSidebar
            steps={STEPS}
            activeTab={activeTab}
            currentStep={step}
          ...
          <div className="custom-scrollbar relative z-10 flex w-full flex-1 flex-col overflow-y-auto rounded-2xl border border-slate-200 bg-white lg:col-span-8 lg:h-full">
            <div
              key={`steps-container-${resetKey}`}
              className="flex h-full min-h-[500px] flex-1 flex-col p-4 sm:p-6 lg:min-h-0"
            >
              {STEPS.map(({ id, component: Component }) => {
                ...
                return (
                  <div
                    key={id}
                    className={`h-full ${activeTab === id ? "block" : "hidden"}`}
                  >
                    <Component
                      step={id}
