    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          const { stdout: visionStdout } = await execPromise(
            `npx tsx src/scripts/e2e-seeder/ai_vision_extractor.ts ${stockId} ${year}`,
          );
          ...
          await execPromise(
            `npx tsx src/scripts/e2e-seeder/persona_generator.ts ${stockId} ${year} --products=${productCount}`,
          );

          sendEvent({ type: "log", message: `Rendering HTML persona...` });
          await execPromise(
            `npx tsx src/scripts/e2e-seeder/render_persona_html.ts ${stockId} ${year}`,
          );
