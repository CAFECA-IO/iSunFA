  ...
  try {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        const runScript = async (command: string, args: string[], captureOutput = false) => {
        ...
        try {
          if (mode === "all" || mode === "download_only") {
            ...
            while (!finCompleted || !esgCompleted) {
              ...
              sendEvent({ type: "log", message: `Waiting for download tasks...` });
          ...
          if (mode === "all" || mode === "generate_only" || mode === "extrapolate_only" || mode === "persona_only") {
            ...
            if (mode !== "all") {
               sendEvent({ type: "fin_complete", file: `data/${stockId}/${year}/inputs/raw_reports/${year}_FIN_REPORT.pdf` });
               sendEvent({ type: "esg_complete", file: `data/${stockId}/${year}/inputs/raw_reports/${year}_ESG_REPORT.pdf` });
               sendEvent({ type: "preview", file: `data/${stockId}/${year}/inputs/raw_reports/${year}_ESG_REPORT.pdf` });
            }

            if (mode === "all" || mode === "generate_only" || mode === "extrapolate_only") {
              ...
              const { stdout: visionStdout } = await runScript(
                "npx",
                ["tsx", "src/scripts/e2e-seeder/ai_vision_extractor.ts", stockId, year],
                true
              );
            ...
            if (mode === "all" || mode === "generate_only" || mode === "persona_only") {
              ...
              await runScript(
                "npx",
                ["tsx", "src/scripts/e2e-seeder/persona_generator.ts", stockId, year, `--products=${productCount}`]
              );

              sendEvent({ type: "log", message: `Rendering HTML persona...` });
              await runScript(
                "npx",
                ["tsx", "src/scripts/e2e-seeder/render_persona_html.ts", stockId, year]
              );
            }
          }
          ...
          if (mode === "all" || mode === "generate_only" || mode === "persona_only") {
            ...
            sendEvent({ type: "log", message: "Extrapolation and vision extraction completed successfully." });
            ...
            sendEvent({ type: "log", message: "Downloads completed successfully." });
