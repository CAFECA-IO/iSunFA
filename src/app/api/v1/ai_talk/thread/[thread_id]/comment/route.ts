  ...
  try {
   ...
   if (!user) {
    ...
    const formattedComments: IComment[] = comments.map(comment => ({
      ...
      userReaction: comment.reactions.find(r => r.userId === loginUserId)?.type ?? null,
    }));
    ...
    console.error(`[API] /api/v1/ai_talk/thread/${(await params).thread_id}/comment GET error:`, error);
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ thread_id: string }> }) {
  try {
   ...
   if (!user) {
    ...
    const replyToUser = replyTo ? await prisma.user.findFirst({
      where: { name: replyTo },
    }) : null;
