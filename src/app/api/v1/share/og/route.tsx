...
export async function GET(request: Request) {
    try {
        ...
        // Info: (20260414 - Tzuhan) 計算動態字體尺寸，避免因字串過長(如："建議持有 (註：60分為及格線)")導致 Flexbox 跑版
        const getScoreFontSize = (score: string) => {
            if (score.length <= 3) return '100px';
            if (score.length <= 8) return '64px';
            if (score.length <= 15) return '48px';
            return '36px'; // 對於極長的敘述
        };
        ...
        return new ImageResponse(
            (
                <div style={{ height: '100%', width: '100%', display: 'flex', flexDirection: 'column', fontFamily: 'sans-serif', position: 'relative', overflow: 'hidden' }}>
                    ...
                    {/* Background Layer: Premium Light Gradient */}
                    ...
                    <div style={{ 
                        ...
                        padding: '50px 80px', flex: 1, justifyContent: 'space-between',
                        zIndex: 10
                    }}>
                        {/* Header */}
                        ...
                        <div style={{ display: 'flex', width: '100%', justifyContent: 'space-between', alignItems: 'center' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', flex: 1, gap: '16px', maxWidth: displayScore ? '55%' : '100%', minWidth: 0 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                    <div style={{ display: 'flex', width: '40px', height: '4px', background: '#EA580C', borderRadius: '2px' }} />
                                    <span style={{ fontSize: '22px', color: '#6B7280', letterSpacing: '0.15em', fontWeight: 700 }}>ANALYSIS TARGET</span>
                                </div>
                                    ...
                                    <div style={{ display: 'flex', gap: '12px', marginTop: '16px', flexWrap: 'wrap' }}>
                                        {displayTags.map((tag, idx) => (
                                            <div key={idx} style={{ display: 'flex', padding: '8px 24px', background: '#FFF7ED', borderRadius: '999px', border: '1px solid #FED7AA', maxWidth: '100%' }}>
                                                <span style={{ fontSize: '20px', color: '#C2410C', fontWeight: 700, whiteSpace: 'nowrap', textOverflow: 'ellipsis', overflow: 'hidden' }}>#{tag}</span>
                                            </div>
                                        ))}
                                    </div>
                                )}
                            ...
                            {displayScore && (
                                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', background: '#FFFFFF', padding: '40px 48px', borderRadius: '32px', border: '2px solid #FFEDD5', marginLeft: '40px', maxWidth: '40%', flexShrink: 0, boxShadow: '0 20px 25px -5px rgba(234, 88, 12, 0.1), 0 8px 10px -6px rgba(234, 88, 12, 0.1)' }}>
                                    <span style={{ fontSize: '20px', color: '#C2410C', fontWeight: 800, marginBottom: '8px', letterSpacing: '0.1em', textAlign: 'center' }}>{isScoreMode ? 'ANALYSIS SCORE' : 'RATING'}</span>
                                    <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px', flexWrap: 'wrap', justifyContent: 'center' }}>
                                        <span style={{ fontSize: scoreFontSize, fontWeight: 900, color: '#EA580C', lineHeight: 1.2, textAlign: 'center', wordBreak: 'break-word' }}>{displayScore}</span>
                        ...
                        <div style={{ display: 'flex', width: '100%', justifyContent: 'space-between', borderTop: '2px solid #FFEDD5', paddingTop: '28px', alignItems: 'center' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                                <span style={{ fontSize: '24px', color: '#1A2E50', fontWeight: 700 }}>運用 AI 驅動的頂尖財務與碳管理平台</span>
                                <div style={{ display: 'flex', width: '6px', height: '6px', borderRadius: '50%', background: '#F97316' }} />
                                <span style={{ fontSize: '24px', color: '#6B7280', fontWeight: 600 }}>免費產生專屬企業洞察</span>
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', padding: '12px 32px', background: 'linear-gradient(90deg, #EA580C 0%, #F97316 100%)', borderRadius: '999px', boxShadow: '0 8px 16px -4px rgba(234, 88, 12, 0.4)' }}>
                                <span style={{ fontSize: '22px', color: '#FFFFFF', fontWeight: 800, letterSpacing: '0.05em' }}>立即前往 isunfa.com</span>
                            </div>
                        </div>
                    </div>
                </div>
