...
export async function GET(request: NextRequest) {
  try {
    ...
    // Info: (20260716 - Emily) #52 帳本閱覽動線:帶 accountBookId 時列出該帳本全部會話;
    // Info: (20260716 - Emily) 需為團隊成員(VIEWER 含),非成員不得枚舉;逐會話內容授權由 report/inventory GET 再裁決
    ...
    if (accountBookId) {
      ...
      const sessions = bookRooms.map((room) => ({
        ...
        // Info: (20260716 - Emily) 是否為本人會話(前端據此決定聊天面板可用性)
...
/**
 * Info: (20260716 - Emily) #52 建立/綁定帳本會話:
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Info: (20260716 - Emily) 綁定 = 寫入行為,需 EDITOR 以上
    ...
    // Info: (20260716 - Emily) 不可改綁:既有綁定與請求不符即拒
