...
export const POST = async (req: NextRequest) => {
  try {
    ...
    // Info: (20260630 - Julian) 步驟 3：在呼叫 LLM 之前，先取得關聯的 PostgreSQL Report 記錄，以便注入 metadata 作為 Context
    ...
    // Info: (20260630 - Julian) 將 SQL 中繼資料 (Metadata) 格式化為結構化背景知識，確保 LLM 能夠直接回答「資本額」等基本資料
...
- 是否經第三方確信：${r.isVerifiedByThirdParty ? "是" : "否"}`,
    ...
    // 將搜尋到的 Markdown 文字片段拼接在一起
    const textContext = matchedDocs.map((doc) => doc.text).join("\n\n---\n\n");

    // 合併中繼資料與向量內文段落，並加入防溢位保護
    const context = [reportMetadataContext, textContext]
      .filter(Boolean)
      .join("\n\n=== 報告書內文段落 ===\n\n")
      .slice(0, 3500);

    // Info: (20260612 - Julian) 步驟 4：撰寫嚴謹的 RAG Prompt
    // ToDo: (20260612 - Julian) 之後要移動 to constants/prompts
