...
/**
 * Info: (20260213 - Julian) 掃描檔案並產生唯一的根雜湊 (Root Hash)
 * @param fileBuffer 原始檔案的 Buffer
 * @returns 十六進制的雜湊字串
 */
export function generateFileHash(fileBuffer: Buffer): string {
  // Info: (20260213 - Julian) 使用 sha256 演算法
  const hashSum = createHash("sha256");

  // Info: (20260213 - Julian) 開始掃描內容
  hashSum.update(fileBuffer);

  // Info: (20260213 - Julian) 產出最終雜湊值 (身份證)
  return hashSum.digest("hex");
}

/**
 * Info: (20260213 - Julian) 上傳檔案
...
export async function POST(request: Request) {
  try {
    ...
    // Info: (20260213 - Julian) --- Step 1: 掃描檔案 (Scanning) ---
    const fileHash = generateFileHash(buffer);
    console.log(`掃描完成，檔案身份證為: ${fileHash}`);

    // Info: (20260213 - Julian) 檢查資料庫是否已有相同 Hash 的檔案 (重複刪除檢查)
    ...
    /**
     * Info: (20260213 - Julian) --- Step 2 & 3: Laria 切割檔案 (Laria Encoding) & IPFS 存儲 (IPFS Storage) ---
    ...
    // Info: (20260213 - Julian) --- Step 4: 儲存算出的 Hash (Store Hash) ---
    const newFile = await prisma.file.create({
      data: {
        fileName: file.name,
        hash: fileHash, // Info: (20260213 - Julian) SHA256 用於重複檢查
        mimeType: file.type,
        fileSize: file.size,
        url: fileUrl,
      },
