...
export async function POST(request: Request) {
  try {
    ...
    if (!text) {
      return NextResponse.json({ error: 'Please provide transportation text.' }, { status: 400 });
    }
    ...
    const segmentsWithEmissions = parsedData.segments.map((seg: { mode: string, from: string, to: string, estimatedKm: number, bbox?: string }) => {
      let coef = 0;
      ...
      return {
        ...
        coefficientSource: 'IPCC Guidelines for National Greenhouse Gas Inventories',
