...
export default function TeamManagementPage() {
  ...
  const handleCreateTeam = async (e: React.FormEvent) => {
    ...
    } catch { alert("Error creating team"); } finally { setCreating(false); }
  };
  ...
  const handleUpdateName = async () => {
    ...
    } catch { alert("Error updating team name"); }
  };
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/v1/user/team/${selectedTeamId}/invitations`, {
        method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ address: inviteAddress.trim(), role: "MEMBER", authentication }),
      });
      const json = await res.json();
      if (json.success) { setInviteAddress(""); setIsInviteModalOpen(false); fetchSentInvitations(selectedTeamId); alert("Invitation sent successfully!"); } 
      else alert(json.message);
    } catch { alert("Error inviting member"); } finally { setInviting(false); }
  };
  ...
  const handleAcceptInvite = async (inviteId: string) => {
    ...
    try {
      ...
      const json = await res.json();
      if (json.success) { fetchPendingInvitations(); fetchTeams(); } else alert(json.message);
    } catch { alert("Error accepting invitation"); } finally { setAcceptingId(null); }
  };
  ...
  const handleChangeRole = async (memberId: string, newRole: string) => {
    ...
    try {
      ...
      const json = await res.json();
      if (json.success) fetchMembers(selectedTeamId); else alert(json.message);
    } catch {}
  };
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">Manage your teams and members here.</p>
          </div>
                ...
                <div key={inv.id} className="bg-white rounded-xl shadow-sm border border-orange-100 p-4 flex flex-col justify-between">
                  <div className="flex items-center space-x-3 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">From {inv.inviter.name || "Unknown"} as {inv.role}</p>
                    </div>
                  </div>
                  <button onClick={() => handleAcceptInvite(inv.id)} disabled={acceptingId === inv.id} className="w-full py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors text-sm font-medium disabled:opacity-50">
                    {acceptingId === inv.id ? "Accepting..." : "Accept via FIDO2"}
                  </button>
                </div>
              ))}
            </div>
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <div className="w-full md:w-64 shrink-0 space-y-2">
            ...
            {teams.length === 0 && <div className="text-sm text-gray-500 p-4 text-center border rounded-xl bg-white shadow-sm">No teams available.</div>}
          </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex items-center space-x-3">
                            <div className="p-2 bg-gray-50 rounded-lg group-hover:bg-orange-50 transition-colors">
                              {member.user?.imageUrl ? (
                                ...
                                <img src={member.user.imageUrl} alt="Avatar" className="w-6 h-6 rounded-full" />
                              ) : <UserCircle2 className="h-6 w-6 text-gray-400 group-hover:text-orange-500" />}
                            </div>
                            <div>
                              <h3 className="text-sm font-medium text-gray-900 group-hover:text-orange-600 transition-colors">
                                {member.user?.name || "Anonymous"}
                                {member.user?.address === user?.address && <span className="text-[10px] text-orange-500 bg-orange-50 px-1.5 py-0.5 rounded ml-2">You</span>}
                              </h3>
                          ...
                          <div className="flex flex-col items-end space-y-2">
                            {isOwner && member.user?.address !== user?.address && member.role !== "OWNER" ? (
                              ...
                              <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-gray-100 text-gray-600">{member.role}</span>
                            )}
                      ...
                      <div key={inv.id} className="bg-orange-50/50 rounded-xl border border-dashed border-orange-200 p-4 opacity-75">
                         <div className="flex items-start justify-between">
                           <div className="flex items-center space-x-3">
                             ...
                             <div>
                               <h3 className="text-sm font-medium text-gray-500">Pending Invite</h3>
                ...
                {currentTeam.accountBooks && currentTeam.accountBooks.length > 0 && (
                  <div className="mt-8 border-t border-gray-100 pt-6">
                    <h3 className="text-lg font-medium text-gray-900 mb-4">Account Books (Companies)</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4">
            ...
            <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl">
              ...
              <form onSubmit={handleCreateTeam} className="space-y-4">
                <div>
                  ...
                  <input id="team-name" type="text" required value={newTeamName} onChange={(e) => setNewTeamName(e.target.value)} disabled={creating} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-orange-500 focus:border-orange-500 sm:text-sm text-gray-900 bg-white" placeholder="Enter team name" />
                </div>
                <div className="mt-6 flex justify-end gap-3">
                  <button type="button" onClick={() => setIsCreateModalOpen(false)} disabled={creating} className="px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 border border-gray-300 rounded-lg">Cancel</button>
                  <button type="submit" disabled={creating || !newTeamName.trim()} className="px-4 py-2 text-sm font-medium text-white bg-orange-600 hover:bg-orange-700 rounded-lg disabled:opacity-50">{creating ? "Creating..." : "Create Team"}</button>
                </div>
              </form>
            </div>
          </div>
      ...
      <Dialog open={isInviteModalOpen} onClose={() => !inviting && setIsInviteModalOpen(false)} className="relative z-50">
        ...
        <div className="fixed inset-0 overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-medium leading-6 text-gray-900">Invite Member</h3>
              ...
              <form onSubmit={handleInvite} className="space-y-4">
                <div>
                  <label htmlFor="invite-address" className="block text-sm font-medium text-gray-700 mb-1">Web3 Address</label>
                ...
                <div className="bg-orange-50 p-3 rounded-lg border border-orange-100 flex items-start mt-2">
                  <div className="text-xs text-orange-800"><span className="font-semibold block mb-1">FIDO2 Requirement:</span>You will be asked to authenticate via Passkey to sign this invitation on-chain.</div>
                </div>
                <div className="mt-6 flex justify-end gap-3">
                  <button type="button" onClick={() => setIsInviteModalOpen(false)} disabled={inviting} className="px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 border border-gray-300 rounded-lg">Cancel</button>
                  <button type="submit" disabled={inviting || !inviteAddress.trim()} className="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-orange-600 hover:bg-orange-700 rounded-lg disabled:opacity-50">{inviting ? "Signing..." : "Invite via FIDO2"}</button>
                </div>
              </form>
            </div>
          </div>
