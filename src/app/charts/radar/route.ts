...
export async function GET(request: NextRequest) {
  ...
  if (!configStr) {
    return NextResponse.json(
      { error: 'Missing config parameter "c"' },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Failed to generate chart", details: String(error) },
      { status: 500 },
    );
  }
}
...
function generateRadarSVG(data: IRadarData): string {
  ...
  data.datasets.forEach((dataset, idx) => {
    ...
    const strokeColor =
      dataset.borderColor || defaultStrokes[idx % defaultStrokes.length];
