...
export interface ISalarySendTarget {
   ...
   *
   * 回 key 而不是回布林：停用的按鈕一定要說得出為什麼，而「為什麼」有四種，
   * 四種的下一步都不一樣（等一下／重新選員工／回員工列表補信箱／這個人已經不在了）。
   * 少一種就會有兩種狀況塌成同一句話，而其中一句一定是錯的
   *（review #6776 應修-B 就是這樣來的）。
   */
...
/**
 * Info: (20260905 - Luphia) 判斷順序是刻意的，四道各自擋不同的東西。
 *
 * 0. **沒有連到員工 → 請重新選一位。** 連「要問誰」都還不知道，
 *    後面三道都無從問起（見下方那一段的案例）。
...
export const resolveSendTarget = (
  ...
  /**
   * Info: (20260906 - Luphia) 這張試算沒有連到員工 —— **它有自己的理由**
   *（review #6776 應修-B）。
   *
   * 初版回的是「沒有電子郵件」，而那句話會把使用者送上一條修不好的路：
   *
   * 1. 會計選了王小明、算完八月薪水、按儲存
   * 2. 發現名字打成「王小名」，就在上面改回來 —— `changeEmployeeName` 會
   *    順手解除員工連結（刻意的：免得畫面寫甲、資料存到乙身上），
   *    而 `savedRecord` **不會**跟著被清
   * 3. 按寄出 → 灰的，寫著「這位員工的資料裡沒有電子郵件，請先到員工列表補上」
   * 4. 他去員工列表，看到王小明的信箱明明就填著。重存一次、回來 —— 一字不差
   *
   * 初版的註解寫「呼叫端另有『請先儲存薪資紀錄』之類的提示在講它」，
   * 而那條提示的條件是「還沒存過」—— 這個情境剛好是**存過了**，所以它不會出現。
   * 註解宣稱了一個不存在的行為。
   *
   * 另一個入口是 Step 1 的「解除連結」按鈕，症狀相同。
   */
  if (employeeId === null) {
    return { blockedReason: "calculator.button.send_disabled_unlinked" };
  }
