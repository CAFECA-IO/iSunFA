...
const ConversionRates: Record<PhysicalDimension, Record<string, number>> = {
  ...
  [PhysicalDimension.VOLUME]: {
    ...
    [MeasurementUnit.GALLON]: 3.78541, // US Gallon to Liter
    [MeasurementUnit.M3]: 1000, // 1 Cubic Meter = 1000 Liters
  },
  [PhysicalDimension.ENERGY]: {
    [MeasurementUnit.KWH]: 1,
    [MeasurementUnit.MWH]: 1000,
    [MeasurementUnit.GJ]: 277.778, // 1 GJ = 277.778 kWh
  },
  [PhysicalDimension.TRANSPORT]: {
    [MeasurementUnit.KM]: 1,
    [MeasurementUnit.TKM]: 1, // Tonne-kilometer can be complex, assuming 1:1 base for distance part for now unless specified
    [MeasurementUnit.PKM]: 1, // Passenger-kilometer
  },
...
export class UnitConverter {
  ...
  public static convert(
    ...
    if (!fromDimension || !toDimension || fromDimension !== toDimension) {
      throw new Error(`Cannot convert between different dimensions: ${fromUnit} and ${toUnit}`);
    }

    const dimensionRates = ConversionRates[fromDimension];
    
    // 找不到匯率表
    if (!dimensionRates || !dimensionRates[fromUnit] || !dimensionRates[toUnit]) {
