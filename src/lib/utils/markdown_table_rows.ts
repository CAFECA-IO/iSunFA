 ...
 *
 * ## 加了第 6、7 條之後的界（08-21 更新；舊的「最多 32 列」已不成立）
 *
 * 團隊 checklist §2.5 要求「最多能發生多壞」要**從護欄的界推**、而且要能被下一個人檢查。
 * 上面那句「從 4 放大到 32」是**加第 6、7 條之前**的界，留著會誤導，所以在這裡更新。
 *
 * 設被折斷那列的寬度 `w_open`、被吞的列寬 `w_r`、同段最寬完整列 `ceiling`：
 *
 * - 合併後的寬度約為 `w_open + w_r - 1`（開頭那半截的末格與被吞列的首格併成一格）。
 * - 第 6 條要求 `w_open + w_r - 1 <= ceiling`，也就是 **`w_r <= ceiling - w_open + 1`**。
 * - 因此當 `w_open == ceiling`（折斷的是一個表寬的資料列 —— 最常見的情況），
 *   `w_r <= 1`：只能吸收一個單格碎片，而單格碎片不是一列。
 *
 * **界：能被吸收的格數 <= `ceiling - w_open`。** `MAX_CONTINUATION_LINES = 32` 現在只
 * 限制「往下掃幾行」，不再限制「能吞掉多少真實資料」——**放大倍數已經不在續行數上**。
 *
 * ### 兩條護欄各自看不到什麼（用輸入空間寫）
 *
 * - **第 7 條只檢查本身就閉合的續行**（缺前導管線、但有尾管線，如 `CH4 | 2000 |`）。
 *   **首尾管線都缺**的列它看不到 —— 那種只有第 6 條擋。
 * - **第 6 條的餘裕實測是 0**（表4.4 / 表4.8 都是 ceiling 恰好等於接回後的寬度），
 *   所以它在「整張表最寬的那一列正是被折斷的那一列」時會拒絕接回。
 *   那個情況由測試「第 6 條護欄的已知脆弱點」釘住，並由 log 的 `refusedTooWide` 觀測。
 *
 * 也就是說：**要讓一列真實資料被無聲吞掉，必須同時滿足**「該列首尾管線都缺（或寬度不在
 * 本表列寬集合裡）」與「`w_open` 明顯小於 `ceiling`」。兩者同時成立的形狀還沒有在任何一趟
 * 匯入裡出現過 —— 沒出現不等於不會出現，所以兩條的計數都進 log。
 *
 * ### 為什麼判準是「不得比最寬的完整列更寬」
 *
