	...
	private sinkDown(n: number) {
		...
		while (true) {
			...
			if (child2Index < length) {
				...
				if (child2.priority < (swapIndex === null ? element.priority : child1Priority)) {
...
class SeaGraph {
		...
		for (const feature of data.features) {
			...
			if (feature.geometry && feature.geometry.type === 'LineString') {
				...
				for (let i = 0; i < coords.length - 1; i++) {
					...
					if (!this.nodes.has(k1)) this.nodes.set(k1, { lng: c1[0], lat: c1[1], key: k1 });
					if (!this.nodes.has(k2)) this.nodes.set(k2, { lng: c2[0], lat: c2[1], key: k2 });
					...
					if (!edges1) { edges1 = []; this.edges.set(k1, edges1); }
					edges1.push({ target: k2, weight: dist });

					let edges2 = this.edges.get(k2);
					if (!edges2) { edges2 = []; this.edges.set(k2, edges2); }
	...
	public snapToMainNetwork(lat: number, lng: number): INode {
		...
		const targetKeys = searchKeys.size > 0 ? searchKeys : this.mainComponentKeys;
		...
		while (!openSet.isEmpty()) {
			...
			for (const neighbor of (this.edges.get(current) || [])) {
				...
				if (tentativeG < neighborG) {
					...
					openSet.enqueue(neighbor.target, tentativeG + heuristic(neighbor.target));
				}
			}
		}
		return [];
 ...
 */
export function calculateSeaPath(start: { lat: number, lng: number }, end: { lat: number, lng: number }): ITransportSegment {
	try {
		if (!graphInstance) {
		...
		let totalDistance = 0;
		totalDistance += calculateDistanceKm(start.lat, start.lng, startNode.lat, startNode.lng);

		for (let i = 0; i < pathNodes.length; i++) {
			coordinates.push([pathNodes[i].lng, pathNodes[i].lat]);
			if (i > 0) {
				totalDistance += calculateDistanceKm(pathNodes[i - 1].lat, pathNodes[i - 1].lng, pathNodes[i].lat, pathNodes[i].lng);
			}
		}

		totalDistance += calculateDistanceKm(endNode.lat, endNode.lng, end.lat, end.lng);
		...
		const distKm = calculateDistanceKm(start.lat, start.lng, end.lat, end.lng) * 1.5;
		const geometry: GeoJSON.LineString = {
			type: "LineString",
			coordinates: [[start.lng, start.lat], [end.lng, end.lat]] as Coordinate[]
		};
		return {
			success: true,
			distanceKm: distKm,
