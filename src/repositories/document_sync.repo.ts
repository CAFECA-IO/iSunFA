      ...
      if (failureReason && !voucherBase && !voucherLines) {
        ...
        try {
          ...
          if (docType === DocumentType.PAYMENT_RECEIPT) {
            ...
            const searchAmountStr = MoneyUtil.toDecimal(totalAmtStr).toFixed(0);
          ...
          if (vd.startDate && vd.endDate && finalVoucherId) {
            ...
            const originalPrepaidIndex =
              vd.lines?.findIndex((l) => !!l.amortizationTargetCategory) ?? -1;
                ...
                if (existingSchedule) {
                  await tx.amortizationSchedule.update({
                    ...
                    data: {
                      ...
                      assetAccountCode: prepaidLineDb.accountingCode || "1251",
                      expenseAccountCode,
                      accountBookId,
                    },
                  });
