...
export class ChatroomRepository {
  ...
  /**
   * Info: (20260716 - Emily) #52 綁定會話至帳本(upsert:會話可能尚未建立);
  ...
  // Info: (20260716 - Emily) #52 依 channel 查綁定帳本(null = 個人會話或不存在)
  ...
  // Info: (20260716 - Emily) #52 列出帳本的碳盤查會話(帳本成員閱覽動線)
  ...
  async listChatroomsByChannelPrefix(channelPrefix: string, purpose?: string) {
    return prisma.chatroom.findMany({
      ...
      select: {
        ...
        createdAt: true,
        updatedAt: true,
        // Info: (20260716 - Emily) #52 前端據此切換保存模式(帳本=明文/個人=E2EE)
        accountBookId: true,
      },
    });
  }
