...
export const runAiBlindTester = async (stockId: string) => {
  ...
  if (!accountBook) {
    console.error(`[ERROR] AccountBook ${accountBookId} not found. Please run phase2_runner first.`);
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(`✅ [JSON 產出成功] Balance Sheet - 總資產: ${bsReport.assets.total}`);
    console.log(`✅ [JSON 產出成功] Cash Flow - 淨現金流: ${cfReport.summary.netIncreaseDecrease}`);
    console.log(`✅ [JSON 產出成功] Income Statement - 淨利: ${isReport.sections.netIncome.total}`);
    console.log(`✅ [JSON 產出成功] ESG Report - 總碳排: ${esgReport.metrics.totalEmissions} kgCO2e`);

    // 基本防呆檢查 (有沒有產生 NaN 或非邏輯的 Undefined)
    if (isNaN(bsReport.assets.total) || isNaN(cfReport.summary.netIncreaseDecrease)) {
      console.error(`❌ [FAILED] JSON 報表結算出現 NaN，運算邏輯可能遭遇浮點數崩潰或除以零！`);
    } else {
  ...
  for (const { category, isExternal } of categoriesToTest) {
    ...
    try {
      ...
      if (report && report.data && typeof report.data === "object" && 'missionName' in report.data) {
        const missionName = report.data.missionName as string;
        const resultPath = path.join(process.cwd(), "missions", missionName, "result.md");
        if (fs.existsSync(resultPath)) {
          ...
          if (category === ANALYSIS_CATEGORY.FINANCIAL_HEALTH) {
            ...
            if (!resultMd.includes("ROE") && !resultMd.includes("現金")) {
              console.warn(`[WARNING] Financial Health report seems to lack concrete metrics.`);
            }
          }

          if (category === ANALYSIS_CATEGORY.CARBON_HEALTH_CHECK) {
            if (!resultMd.includes("碳")) {
              console.warn(`[WARNING] Carbon Health report seems disconnected from actual ESG records.`);
            }
          }
