...
export async function generatePersona(stockId: string, year: string = "2024") {
  ...
  if (!fs.existsSync(contextCacheFile)) {
    throw new Error(`[Error] 找不到 context cache。請先執行 ai_vision_extractor.ts! (路徑: ${contextCacheFile})`);
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!stockId || stockId.startsWith("--")) {
    console.error("Usage: npx tsx persona_generator.ts <stockId> [--year=2025] [--force]");
    process.exit(1);
  }
