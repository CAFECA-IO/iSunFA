...
export async function renderDppPdf(stockId: string, year: string = "2024") {
  const dataDir = path.resolve(
    process.cwd(),
    `data/${stockId}/${year}/outputs/cbam_mocks`,
  );
  const jsonPath = path.join(dataDir, "dpp_ground_truth.json");
  const pdfOutFile = path.join(dataDir, "dpp_ground_truth_dashboard.pdf");

  if (!fs.existsSync(jsonPath)) {
    throw new Error(`Missing dpp_ground_truth.json at ${jsonPath}`);
  }

  const dppData = JSON.parse(fs.readFileSync(jsonPath, "utf-8"));

  const blueprintPath = path.join(dataDir, "fastener_blueprint.png");
  let blueprintBase64 = "";
  ...
  // Create an HTML string mimicking a gorgeous dashboard (Battery Pass style)
  const htmlContent = `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
  ...
  <div class="grid">
    <div class="card">
      ...
      <div class="kv-row" style="flex-direction: column; gap: 8px; margin-top: 15px;">
        ...
        <span class="kv-val" style="color: #cbd5e1; font-weight: normal; line-height: 1.5;">${dppData.durabilityAndRepair.repairability}</span>
      </div>
      <div class="kv-row" style="flex-direction: column; gap: 8px; margin-top: 15px;">
        <span class="kv-key">Disposal Instructions</span>
        <span class="kv-val" style="color: #cbd5e1; font-weight: normal; line-height: 1.5;">${dppData.durabilityAndRepair.disposal}</span>
      </div>
    </div>
    <div class="card">
      ...
      <div style="margin-top: 30px; padding: 15px; background: #0f172a; border-radius: 8px; border: 1px solid #334155;">
        ...
        <div style="color: #3b82f6; font-weight: 500;">📄 ${dppData.compliance.declarationDocument}</div>
      </div>
    </div>
  </div>
  ...
  console.log("⏳ Rendering DPP Dashboard PDF...");
  await mdToPdf(
  ...
  console.log(`🎉 [SUCCESS] DPP Dashboard PDF 已成功匯出：${pdfOutFile}`);
}
