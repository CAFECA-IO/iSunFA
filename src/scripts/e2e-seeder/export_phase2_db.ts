...
export const exportPhase2Db = async (stockId: string) => {
  ...
  console.log(`✅ Successfully exported ${vouchers.length} Vouchers and ${esgRecords.length} ESG Records.`);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!targetStock) {
    console.error("Please provide a stock ID or 'all'. Usage: tsx export_phase2_db.ts 1538");
    process.exit(1);
  }
