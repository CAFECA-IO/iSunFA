...
export async function renderPersonaHtml(
  stockId: string,
  year: string = "2024",
) {
  ...
  const personaData: IPersonaData = JSON.parse(
    fs.readFileSync(personaPath, "utf-8"),
  );

  console.log(
    `🚀 [Persona HTML Renderer] 開始為 ${stockId} 生成企業畫像 HTML...`,
  );
  ...
  <div class="content-wrapper">
    ...
    <div class="card">
      ...
      <table>
        ...
        <tbody>
          ${(personaData.manufacturingProcess || [])
            .map(
              (p) => `
            <tr>
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  renderPersonaHtml(stockId, year).catch((e) => {
    console.error(e);
    process.exit(1);
  });
}
