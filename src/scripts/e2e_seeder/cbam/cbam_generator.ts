...
export const runCbamGenerator = (stockId: string, year: string = "2024") => {
  ...
  if (!fs.existsSync(vouchersPath)) {
    console.error(
      `[ERROR] Missing simulated_vouchers.json for ${stockId} at ${vouchersPath}`,
    );
    process.exit(1);
  }

  const vouchers: ISimulatedVoucher[] = JSON.parse(
  ...
  if (fs.existsSync(bomsPath)) {
    ...
    console.warn(
      `[WARN] Missing BOMs data at ${bomsPath}. Precursor carbon emissions will use fallback values.`,
    );
  }
      ...
      if (product.bom) {
        for (const pre of product.bom) {
          if (pre.supplierName && pre.embeddedEmissionsKgCO2ePerKg) {
            supplierFactors.set(
              pre.supplierName,
              pre.embeddedEmissionsKgCO2ePerKg,
            );
          }
        }
      }
  ...
  console.log(
    `[INFO] Reverse-Engineering CBAM Data from ${vouchers.length} Vouchers...`,
  );

  // Info: (20260605 - Tzuhan) 2. Aggregate Vouchers
  const COGS_ACCOUNT = SemanticAccountMatcher.match(
  ...
  console.log(`\n🎉 [CBAM Generator] 反推完成！`);
  console.log(
    ` - 總用電度數 (推估): ${inferredElectricityKwh.toLocaleString()} kWh`,
  );
  console.log(
    ` - Scope 2 碳排: ${scope2EmissionsKgCo2e.toLocaleString()} kgCO2e`,
  );
  console.log(
    ` - 前驅物碳排: ${totalPrecursorEmissionsKgCo2e.toLocaleString()} kgCO2e`,
  );
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!targetStock) {
    console.error("Usage: tsx cbam_generator.ts <stockId> [year]");
    process.exit(1);
  }
