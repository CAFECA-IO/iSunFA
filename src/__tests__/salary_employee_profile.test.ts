...
describe("34 個表單欄位的分類", () => {
  ...
  it("常態欄位與員工檔的對照沒有漏接（16 → 15）", () => {
    ...
    const uncovered = EMPLOYEE_PROFILE_KEYS.filter((key) => !covered.has(key));
...
describe("diffEmployeeProfile", () => {
  ...
  it("逐欄列出 before / after，不是只回一個布林", () => {
    ...
    expect(diff).toEqual([{ field: "dependentsCount", before: 0, after: 1 }]);
  });
  ...
  it("浮點誤差不算差異", () => {
    expect(
      diffEmployeeProfile({ ...PROFILE, baseSalary: 36000.0000001 }, PROFILE),
    ).toEqual([]);
  });
...
describe("DEFAULT_EMPLOYEE_PROFILE 與 schema 的 @default 一致", () => {
  ...
  it.each([
    ...
    const value =
      DEFAULT_EMPLOYEE_PROFILE[field as keyof typeof DEFAULT_EMPLOYEE_PROFILE];
    const inSchema = declaration.slice(
      declaration.indexOf("(") + 1,
      declaration.lastIndexOf(")"),
    );

    expect(`${typeof value === "string" ? `"${value}"` : value}`).toBe(
      inSchema,
    );
  });
