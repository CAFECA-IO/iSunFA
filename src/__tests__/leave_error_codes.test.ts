...
const LEAVE_MODULE_ERRORS: Readonly<Record<string, ApiCode>> = {
  ...
  VA_LEAVE_NOT_RECALLABLE: ApiCode.VALIDATION_ERROR,
  VA_LEAVE_APPROVAL_RULE_INVALID: ApiCode.VALIDATION_ERROR,
  VA_LEAVE_GENERAL_RULE_REQUIRED: ApiCode.VALIDATION_ERROR,
  VA_LEAVE_NO_SHIFT_FOR_ACCRUAL: ApiCode.VALIDATION_ERROR,
  ...
  // Info: (20260818 - Julian) L24–L30（加班）新增
  VA_OVERTIME_ALREADY_REVIEWED: ApiCode.VALIDATION_ERROR,
  VA_OVERTIME_COMP_EXPIRY_UNSET: ApiCode.VALIDATION_ERROR,
  VA_OVERTIME_DAY_NOT_SCHEDULED: ApiCode.VALIDATION_ERROR,
  VA_OVERTIME_PREMIUM_UNDEFINED: ApiCode.VALIDATION_ERROR,
  // Info: (20260818 - Julian) L2–L6（假別設定）新增
  VA_LEAVE_POLICY_LOCKED_FIELD: ApiCode.VALIDATION_ERROR,
  VA_LEAVE_POLICY_MERGE_CYCLE: ApiCode.VALIDATION_ERROR,
  VA_LEAVE_TIER_TABLE_INVALID: ApiCode.VALIDATION_ERROR,
  VA_LEAVE_TIER_NOT_APPLICABLE: ApiCode.VALIDATION_ERROR,
  /**
   * Info: (20260818 - Julian) 沿用出勤模組既有的碼。
   * 登記在這裡不是重複 —— 這張表問的是「假勤模組引用了哪些碼」，
   * 而漏登的那一個正是「以為它存在」的下一個受害者。
   */
  VA_ATTENDANCE_RANGE_TOO_LARGE: ApiCode.VALIDATION_ERROR,
  FO_HR_FUNCTION_REQUIRED: ApiCode.FORBIDDEN,
  FO_NO_PERMISSION_TO_VIEW_THIS: ApiCode.FORBIDDEN,
  CF_LEAVE_POLICY_CODE_TAKEN: ApiCode.CONFLICT,
  ...
  FO_LEAVE_RECALL_SCOPE: ApiCode.FORBIDDEN,
...
const dictionary = API_ERRORS as unknown as Record<
  string,
  IErrorDef | undefined
>;
