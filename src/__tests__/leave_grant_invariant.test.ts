...
describe("assertGrantSource — grantedMinutes 必須可驗算", () => {
  ...
  /**
   * Info: (20260819 - Julian) 比例給假會產生小數分鐘（1.1 日 × 465 分鐘 = 511.5），
   * 進位後為 512。
   *
   * 這條原本的說明是「用與 `deriveGrantSchedule` 完全相同的式子驗算」——
   * 那正是 review B6 打掉的東西：判準重算實作，就驗不出實作本身算錯
   * （`1.1 × 420` 的浮點乘積是 `462.00000000000006`，進位多給一分鐘而
   * 這條檢查照樣放行）。現在驗的是**定義**的兩側邊界，與乘法怎麼實作無關。
   * 多給／少給一分鐘各自的紅燈在 `leave_granted_minutes_precision.test.ts`。
   */
  it("比例給假的進位結果通過", () => {
    expect(() =>
      assertGrantSource({
