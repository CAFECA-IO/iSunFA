// Info: (20260716 - Emily) #52 碳盤查存取裁決測試:個人/帳本雙軌 × 角色 × 讀寫層級矩陣
// Info: (20260717 - Emily) 重要:next/jest(SWC)只 hoist「全域 jest」的 jest.mock 呼叫;
// Info: (20260717 - Emily) 若 jest 是 @jest/globals 的 import 綁定,mock 不會被 hoist → 真實 prisma
// Info: (20260717 - Emily) 先被載入(pg Pool 開啟導致 worker 無法退出、mockReset 不存在)。
// Info: (20260717 - Emily) 故比照 allocation_engine.test.ts:declare 全域 jest,只 import 型別。
...
// Info: (20260716 - Emily) mock 取用(型別化,不使用 any)
...
describe("resolveCarbonAccess", () => {
  ...
  it("book session: VIEWER can view but not edit; EDITOR+ can edit; non-member denied", async () => {
    ...
    // Info: (20260716 - Emily) EDITOR/ADMIN/OWNER 皆可編輯
