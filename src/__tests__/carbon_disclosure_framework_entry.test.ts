...
describe("完成判準:選了 IFRS,重載之後仍然是 IFRS", () => {
  ...
  it("勾稽阻擋紀錄撐過重載(#6760 把 open/73 做完:界量過、截斷放在寫入端)", () => {
    /**
     * Info: (20260904 - Emily) 這一條原本方向是反的,而那是刻意的:本檔一度宣告過
     * `ledgerImportBlocks`(`reason: max(500)`),而寫入端 `blockedReason` 值域無上界
     * (筆數隨廠址 × 類別成長、subject 是客戶原文)。猜一個上界比不宣告更糟:
     * 超界那次存得進去,下一次載入 `safeParse` 失敗 → **整份 state 被丟棄**。
     * 所以當時釘住「它還沒被持久化」這個已知缺口,並指向那張票。
     *
     * Info: (20260909 - Emily) 票做完,改成正向。量測與截斷的不變式在
     * `carbon_ledger_import_blocks.test.ts`;這裡只釘「它現在會撐過重載」。
     */
    const state = {
      ...baseState,
      ledgerImportBlocks: [
    ...
    expect(roundTrip(state)).toHaveProperty("ledgerImportBlocks");
    expect((roundTrip(state) as typeof state).ledgerImportBlocks).toEqual(
      state.ledgerImportBlocks,
    );
  });
});
