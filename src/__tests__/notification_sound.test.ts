...
describe("arrivalKeyOf", () => {
  ...
  /**
   * Info: (20260826 - Julian) 這一條驗的是**同一次抵達的前後兩態**（review 1.1）。
   *
   * 它原本的名字是「沒有入庫通知時仍靠數量區分」，註解寫著活算的待辦
   * 「只有數量會動，鍵仍然要跟著變」—— 那讓人以為活算路徑已被涵蓋，
   * 而它其實只證明了 `0 → 1` 這一步鍵有變。真正會出事的是
   * **兩次不同的抵達**：`1 → 0 → 1`，第二個 1 與第一個 1 字面相同。
   * 那一條純函式測不到（它看不見時間從哪來），
   * 在 `notification_service.test.ts` 的「提示音的抵達識別值」以真模組驗。
   */
  it("數量從 0 變 1 時鍵會變", () => {
  ...
  /**
   * Info: (20260826 - Julian) 而數量繞一圈回到原值時，鍵**不會**變。
   *
   * 這是上面那條測不到的另一半，寫成一條會通過的測試而不是註解：
   * 它把「光靠數量不夠」變成一個看得見的事實 —— 也就是
   * `latestUnreadAt` 為什麼非得涵蓋活算的邀請不可。
   */
  it("數量繞回原值時鍵相同 —— 所以識別值不能只有數量", () => {
    expect(arrivalKeyOf(null, 1, 0)).toBe(arrivalKeyOf(null, 1, 0));
  });
});
