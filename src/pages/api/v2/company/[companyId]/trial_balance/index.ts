...
> = async ({ query }) => {
   ...
   * 0.2. 1h)撰寫 Utils 工具，輸入會計分錄清單，將重複的分錄合併並加總其借貸金額，輸出合併後的會計分錄清單
   ...
   * 2. 10min)獲取該公司所有會計科目，使用 account.repo
   * 3. 10min)取得 periodEnd 時間點以前的所有會計傳票並整理傳票 id 清單 voucherIds，使用 voucher.repo
   * 4. 10min)根據 voucherIds 取得所有會計傳票的所有會計分錄，使用 line_item.repo
   * 5. 10min)依照 periodBegin 為區分點，將所有會計分錄分為期初到期中 begining、期中到期末 midterm 二個部分
   * 6. 10min)使用 Utils 工具合併 begining 並計算其借方貸方金額，統計總額 totalDebit、totalCredit
   * 7. 10min)使用 Utils 工具合併 midterm 並計算其借方貸方金額，統計總額 totalDebit、totalCredit
   * 8. 10min)使用 Utils 工具將 begining 與 midterm 的科目合併加總為期末部份 ending，並計算其借方貸方金額，統計總額 totalDebit、totalCredit
   * 9. 10min)篩除借方與貸方金額皆為 0 的科目
   * 10. 10min)使用 Utils 0.3. 工具增列虛擬科目
   * 11. 10min)整理資料為 API 回傳格式
   * 12. 2h)整理可能發生的例外情形
   * 13. 10min)回傳 API 回應
   */
