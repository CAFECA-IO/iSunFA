...
/**
 * Info: (20250520 - Shirley) Process a single PDF file to generate and associate a thumbnail
...
async function processPdfFile(
  ...
  try {
    ...
    // Info: (20250520 - Shirley) 如果檔案是加密的，先解密
    if (isEncrypted) {
      ...
      try {
        ...
        // Info: (20250520 - Shirley) 將解密後的檔案寫入臨時檔案
        ...
        // Info: (20250702 - Shirley) 確認臨時檔案已成功寫入
        ...
        // Info: (20250702 - Shirley) 如果解密失敗，重試
    ...
    try {
      ...
      // Info: (20250702 - Shirley) 確認縮略圖檔案已成功產生
      ...
      // Info: (20250520 - Shirley) 如果原檔案是加密的，則對縮略圖進行加密
      if (isEncrypted) {
        ...
        try {
          ...
          // Info: (20250520 - Shirley) 使用新的 IV 參數
          const newIv = crypto.getRandomValues(new Uint8Array(16));

          // Info: (20250520 - Shirley) 執行加密
          ...
          // Info: (20250520 - Shirley) 將新的 IV 轉換為 Buffer 以存儲在數據庫中
          const newIvBuffer = uint8ArrayToBuffer(newIv);

          // Info: (20250520 - Shirley) 將縮略圖資訊添加到數據庫
          ...
          if (thumbnailInDB) {
            // Info: (20250520 - Shirley) 更新原始檔案記錄，添加縮略圖 ID
          ...
          // Info: (20250520 - Shirley) 創建未加密的縮略圖記錄作為後備方案
          const thumbnailInDB = await createFile({
            name: thumbnailFileName,
        ...
        // Info: (20250520 - Shirley) 如果原檔案不是加密的，則直接存儲縮略圖資訊
        const thumbnailInDB = await createFile({
          name: thumbnailFileName,
          size: thumbnailInfo.size,
          mimeType: 'image/png',
      ...
      // Info: (20250702 - Shirley) 如果縮略圖處理失敗，重試
      if (retryCount < maxRetries) {
        loggerBack.info(
    ...
    // Info: (20250520 - Shirley) 移除臨時解密檔案
...
/**
 * Info: (20250702 - Shirley) 連續處理檔案，確保每個檔案都完成處理後再處理下一個
...
async function processFilesSequentially(fileDataList: any[]) {
  ...
  for (const fileData of fileDataList) {
    ...
    // Info: (20250702 - Shirley) 獲取實際文件路徑
    const pdfPath = url.replace('/baseurl/', '');

    try {
      // Info: (20250702 - Shirley) 檢查檔案是否存在
      await fs.access(pdfPath);

      // Info: (20250702 - Shirley) 處理此PDF檔案
    ...
    // Info: (20250702 - Shirley) 在每個檔案處理完後短暫暫停，避免資源爭用
...
export default async function handler(
  ...
  try {
    // Info: (20250702 - Shirley) 獲取 invoice 資料夾路徑並讀取所有 PDF 檔案
    ...
    // Info: (20250701 - Shirley) 獲取檔案資訊和對應的公司ID的SQL邏輯實現
    ...
    // Info: (20250702 - Shirley) 連續處理檔案，確保每個檔案處理完成後再處理下一個
    const results = await processFilesSequentially(fileDataWithCompanyId);

    // Info: (20250701 - Shirley) 找出未處理的檔案
