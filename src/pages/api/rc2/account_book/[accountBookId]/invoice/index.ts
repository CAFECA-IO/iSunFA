...
import { listInvoiceRC2 } from '@/lib/utils/repo/invoice_rc2.repo';
...
const handleGetRequest = async (req: NextApiRequest) => {
  ...
  if (!query) throw new Error(STATUS_MESSAGE.INVALID_INPUT_DATA);

  const invoices = await listInvoiceRC2(session.userId, query);

  const { isOutputDataValid, outputData } = validateOutputData(APIName.LIST_INVOICE_RC2, invoices);
