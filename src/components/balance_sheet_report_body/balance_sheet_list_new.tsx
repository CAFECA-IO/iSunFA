...
// 定義 ReportObject 和 ReportReturnType 用於類型推斷
...
// Info: (20241015 - Anna) 先拿掉{ reportId }: IBalanceSheetReportBodyAllProps
const BalanceSheetList = () => {
  // Info: (20241016 - Anna) 新代碼
  const [reportData, setReportData] = useState<IAccountReadyForFrontend[] | null>(null);
  const { selectedCompany } = useUserCtx();
  // 不知道為什麼串不上的api...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await APIHandler(APIName.REPORT_GET_V2, {
          params: { companyId: selectedCompany?.id },
          query: { reportType: 'balance_sheet', startDate: 20220101, endDate: 20221231 },
        });
        ...
        // 確保 response 是物件，且有正確的 data 結構
        if (response && typeof response === 'object' && 'payload' in response) {
          const payload = response.payload as ReportReturnType;

          if (payload.general) {
            setReportData(payload.general);
            // eslint-disable-next-line no-console
            console.log('General Report Data:', payload.general);
          } else {
            // eslint-disable-next-line no-console
            console.error('No "general" field in payload:', payload);
          }
        } else {
          // eslint-disable-next-line no-console
  ...
  }, [selectedCompany]); // 加上 selectedCompany 來作為依賴
  // 加上 selectedCompany 來作為依賴
  ...
  const rowsForPage1 = () => {
    const rows = [
      <tr key="1">
        <td className="border border-stroke-brand-secondary-soft p-10px text-sm">1001</td>
        <td className="border border-stroke-brand-secondary-soft p-10px text-sm">現金及約當現金</td>
        <td className="border border-stroke-brand-secondary-soft p-10px text-end text-sm">
          1,000,000
        </td>
      ...
      <tr key="2">
        <td className="border border-stroke-brand-secondary-soft p-10px text-sm">1002</td>
        <td className="border border-stroke-brand-secondary-soft p-10px text-sm">應收帳款</td>
        <td className="border border-stroke-brand-secondary-soft p-10px text-end text-sm">
          500,000
        </td>
        <td className="border border-stroke-brand-secondary-soft p-10px text-center text-sm">
        ...
        <td className="border border-stroke-brand-secondary-soft p-10px text-center text-sm">9%</td>
      </tr>,
      <tr key="3">
        <td className="border border-stroke-brand-secondary-soft p-10px text-sm">1003</td>
        <td className="border border-stroke-brand-secondary-soft p-10px text-sm">存貨</td>
        <td className="border border-stroke-brand-secondary-soft p-10px text-end text-sm">
          300,000
        </td>
        <td className="border border-stroke-brand-secondary-soft p-10px text-center text-sm">6%</td>
        <td className="border border-stroke-brand-secondary-soft p-10px text-end text-sm">
          280,000
        </td>
        <td className="border border-stroke-brand-secondary-soft p-10px text-center text-sm">
    ...
    const staticItems = [
      {
        code: '2100',
        name: '應付帳款',
        curPeriodAmountString: '1,000,000',
        curPeriodPercentage: '25%',
        prePeriodAmountString: '900,000',
        prePeriodPercentage: '22%',
      },
      {
        code: '2200',
        name: '其他負債',
        curPeriodAmountString: '500,000',
        curPeriodPercentage: '12%',
        prePeriodAmountString: '450,000',
        prePeriodPercentage: '11%',
      },
      {
        code: '2300',
        name: '短期借款',
        curPeriodAmountString: '1,200,000',
        curPeriodPercentage: '30%',
        prePeriodAmountString: '1,000,000',
        prePeriodPercentage: '25%',
      },
      // 其他靜態項目數據
    ];

    // 使用靜態數據生成表格行
    const rows = staticItems.slice(0, 9).map((item) => {
      if (!item.code) {
        return (
          <tr key={item.name}>
            <td
    ...
    const staticItems = [
      {
        ...
        name: '應收帳款',
        curPeriodAmountString: '500,000',
        curPeriodPercentage: '15%',
        prePeriodAmountString: '450,000',
        prePeriodPercentage: '14%',
      },
      {
        code: '1300',
        name: '存貨',
        curPeriodAmountString: '300,000',
        curPeriodPercentage: '9%',
        prePeriodAmountString: '280,000',
        prePeriodPercentage: '8%',
      },
    ];

    // 直接使用靜態數據生成表格行
    const rows = staticItems.map((item) => {
      if (!item.code) {
        return (
          <tr key={item.name}>
            <td
  ...
  const page1 = (
    <div id="1" className="relative h-a4-height overflow-y-hidden bg-white">
      ...
      <header className="mb-12 flex justify-between pl-0 text-white">
        <div className="w-3/10 bg-surface-brand-secondary pb-14px pl-10px pr-14px pt-40px font-bold">
          <div className="">
            {/* Info: (20241016 - Anna) reportFinancial && reportFinancial.company && */}
            {
              <>
  ...
  const page2 = (
    <div id="2" className="relative h-a4-height overflow-y-hidden bg-white">
  ...
  const page3 = (
    <div id="3" className="relative h-a4-height overflow-y-hidden bg-white">
      <header className="flex justify-between text-white">
