...
interface IChatSidebarProps {
  // Info: (20260722 - Emily) UAT:boundBookName 有值 = 帳本會話(列表以圖示+帳本名 chip 區隔個人會話)
  ...
  // Info: (20260716 - Emily) UAT 帳本報告入口:展開帳本時載入該帳本全部會話(含他人;成員即可見)
  onFetchBookSessions?: (accountBookId: string) => Promise<IBookSessionEntry[]>;
  // Info: (20260716 - Emily) 開啟他人會話的報告(帳本檢視器;本人會話走 onSelectSession)
  onOpenBookReport?: (channel: string) => void;
}

// Info: (20260716 - Emily) 帳本會話列表項(標題衍生自密文首訊 server 不可讀,故以建立日期呈現)
...
export function ChatSidebar({
  ...
  // Info: (20260716 - Emily) 帳本報告區:展開狀態與 lazy 載入的會話列表(每帳本一次)
  ...
  return (
    <div className="relative hidden w-[280px] shrink-0 flex-col border-r border-gray-200 bg-white lg:flex">
        ...
        <div>
            ...
            {sessionsList.map((s) => (
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  >
                    {/* Info: (20260722 - Emily) UAT:帳本會話與個人會話的視覺區隔(帳本=書本圖示) */}
                    {s.boundBookName ? (
          ...
          ref={(node) => node?.focus()}
                    ...
                    <div className="mt-1.5 flex items-center justify-between">
                      ...
                      {/* Info: (20260722 - Emily) UAT:帳本會話 chip(帳本名;個人會話不顯示) */}
                      {s.boundBookName && (
                        <span
      ...
      {/* Info: (20260716 - Emily) UAT 帳本報告:成員查看團隊碳盤查報告的入口(聊天記錄個人加密,僅共享報告) */}
