...
// Info: (20260716 - Tzuhan) #52 帳本報告檢視器:帳本成員開啟「他人會話」的報告(UAT:組織其他人在哪裡查看)
// Info: (20260716 - Tzuhan) 隱私邊界:本檢視器只載入報告(帳本明文模式),絕不觸碰聊天訊息 —
// Info: (20260716 - Tzuhan) 聊天為個人 E2EE,他人在密碼學上不可讀,結構上也不請求
// Info: (20260716 - Tzuhan) 權限:server 依 TeamRole 裁決 — VIEWER 唯讀;EDITOR 以上可編輯(需解鎖取得本人金鑰簽存)
...
export interface IBookReportViewerProps {
  channel: string;
  // Info: (20260716 - Tzuhan) 編輯保存需本人金鑰(recipientPublicKey);未解鎖 → 一律唯讀
...
export function BookReportViewer({
  ...
  // Info: (20260716 - Tzuhan) 載入報告(帳本明文模式免金鑰;access 由 server 裁決回傳)
  useEffect(() => {
    let cancelled = false;
    loadReportDraft(channel, masterKey)
      .then((loaded) => {
        ...
        // Info: (20260716 - Tzuhan) 編輯需 server 授權 + 本人金鑰(保存 payload 需 recipientPublicKey)
  ...
  // Info: (20260716 - Tzuhan) Editor 編輯:直接改 rawMarkdown(權威來源)+ debounce 保存(樂觀鎖)
    ...
    <div className="fixed inset-0 z-[85] flex items-center justify-center bg-black/30 p-4">
      <div className="flex h-[90vh] w-full max-w-4xl flex-col rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center gap-2 border-b border-gray-100 px-5 py-3">
          ...
          {/* Info: (20260716 - Tzuhan) 隱私聲明:聊天記錄為個人加密,帳本成員僅共享報告 */}
        ...
        <div className="min-h-0 flex-1 overflow-hidden">
          ...
          {!isLoading && reportData && canEdit && (
            // Info: (20260716 - Tzuhan) Editor:左編輯右預覽(簡化雙欄;完整工具列屬本人會話動線)
