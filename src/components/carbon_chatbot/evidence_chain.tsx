...
// Info: (20260720 - Tzuhan) 第三章證據鏈元件(#54):報告內的層層下鑽 — Scope(並聯加總)→ 排放源
// Info: (20260720 - Tzuhan) (並聯加總)→ 單筆 EsgRecord(串聯:數量 × 係數 = 排放)→ 點開單一憑證
// Info: (20260720 - Tzuhan) (RecordTabModal:journal/voucher/esg/原始檔四分頁,重用財報線檢視器)
// Info: (20260720 - Tzuhan) 數據實時問 /chat/carbon/esg-records(帳本閱覽權限由 server 裁決),
// Info: (20260720 - Tzuhan) 加總全程 MoneyUtil 字串累加(ADR 003);accordion 樣式沿用 emission_sources_item
...
// Info: (20260720 - Tzuhan) 匯入端點回傳的活動附帶驗證旗標(顯示用;server 端已裁決)
...
// Info: (20260720 - Tzuhan) 決定性聚合:Scope → 排放源 → 紀錄(插入序穩定;字串累加零誤差)
...
export function EvidenceChain({ accountBookId }: IEvidenceChainProps) {
  ...
  // Info: (20260720 - Tzuhan) 實時載入(開啟報告即最新認列結果;權限不符由 server 回 403)
  useEffect(() => {
    let cancelled = false;
  ...
  return (
    <div className="my-4 overflow-hidden rounded-xl border border-orange-100 bg-white text-sm">
              ...
              group.sources.map((source) => {
                ...
                return (
                  <div key={sourceKey} className="ml-6 border-l border-gray-100">
                    ...
                    {/* Info: (20260720 - Tzuhan) 第 3 層:單筆憑證(串聯:數量 × 係數 = 排放);點列開憑證 */}
                    {isSourceOpen &&
      ...
      {/* Info: (20260720 - Tzuhan) 最細顆粒:單一憑證四分頁檢視(voucher 優先;無傳票落 esg) */}
      {evidenceTarget && (
        <RecordTabModal
          isOpen
