...
export interface IEmployeeListFiltersProps {
  ...
  /**
   * Info: (20260907 - Julian) 三個勾選 filter（`EmployeeListIssueFilters`）。
   *
   * 20260907 早一版它們是獨立的一塊橫幅，刻意不放進這張卡片；改成勾選之後
   * 它們就是條件，收進來才對 —— 一個「只看缺信箱」的勾選放在篩選卡片外面，
   * 會讓它看起來與搜尋是兩種不同的東西。
   *
   * 一位員工都沒有時是 `null`（沒有名單就沒有問題可以篩）。
   */
  issueFilters?: ReactNode;
}
