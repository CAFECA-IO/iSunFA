...
export default function BlockchainTreasury({
  ...
  return (
    <div className="flex h-full flex-col items-center rounded-3xl border border-gray-200 bg-white p-8 shadow-sm">
      <div className="w-full">
        ...
        <div className="group relative mx-auto flex aspect-square w-full max-w-[240px] items-center justify-center rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50/50 p-4 transition hover:border-orange-300 hover:bg-orange-50/30">
          {data ? (
            <div className="transform overflow-hidden rounded-xl bg-white p-2 shadow-sm transition group-hover:scale-105">
              <QRCodeSVG
                value={data.address}
                size={200}
      ...
      <div className="mt-8 w-full text-center">
        ...
        {data ? (
          ...
          >
            <span className="max-w-[85%] truncate font-mono text-sm font-medium text-gray-600 transition group-hover:text-orange-700">
              {data.address}
            </span>
            <Copy className="h-4 w-4 shrink-0 text-gray-400 transition group-hover:text-orange-500" />
          </button>
        ) : (
