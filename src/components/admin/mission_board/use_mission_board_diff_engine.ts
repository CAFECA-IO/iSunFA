  ...
  useEffect(() => {
    ...
    if (isInitialSyncRef.current) {
      ...
      for (let i = 0; i < total; i += chunkSize) {
        ...
        robotRef.current?.queueAction({ type: "PICKUP", targetZone: "SPAWN", blockId: chunk[0].taskId, scale });
        robotRef.current?.queueAction({
          type: "DROP", targetZone: "OPEN", blockId: chunk[0].taskId, scale,
          onComplete: () => {
            setDisplayTasks(prev => {
              const updated = [...prev];
              chunk.forEach(apiTask => {
                if (!updated.some(t => t.taskId === apiTask.taskId)) {
                  updated.push({ ...apiTask, status: TaskStatus.Open, _trueStatus: apiTask.status });
                }
              });
      ...
      const nonOpen = tasksToProcess.filter(t => t.status !== TaskStatus.Open);
      for (let i = 0; i < nonOpen.length; i += chunkSize) {
        const chunk = nonOpen.slice(i, i + chunkSize);
        const scale = 1 + Math.log10(chunk.length);
        robotRef.current?.queueAction({
          type: "PICKUP", targetZone: "OPEN", blockId: chunk[0].taskId, scale,
          onComplete: () => {
            setDisplayTasks(prev => {
              const updated = prev.filter(t => !chunk.some(c => c.taskId === t.taskId));
              displayTasksRef.current = updated;
              return updated;
            });
          }
        ...
        robotRef.current?.queueAction({
          type: "DROP", targetZone: "PENDING_REVIEW", blockId: chunk[0].taskId, scale,
          onComplete: () => {
            setDisplayTasks(prev => {
              const updated = [...prev.filter(t => !chunk.some(c => c.taskId === t.taskId))];
              chunk.forEach(apiTask => {
                updated.push({ ...apiTask, status: TaskStatus.PendingReview, _trueStatus: apiTask.status });
              });
              displayTasksRef.current = updated;
              return updated;
            });
      ...
      const closedTasks = tasksToProcess.filter(t => t.status === TaskStatus.Closed);
      for (let i = 0; i < closedTasks.length; i += chunkSize) {
        const chunk = closedTasks.slice(i, i + chunkSize);
        const scale = 1 + Math.log10(chunk.length);
        robotRef.current?.queueAction({
          type: "PICKUP", targetZone: "PENDING_REVIEW", blockId: chunk[0].taskId, scale,
          onComplete: () => {
            setDisplayTasks(prev => {
              const updated = prev.filter(t => !chunk.some(c => c.taskId === t.taskId));
              displayTasksRef.current = updated;
        ...
        robotRef.current?.queueAction({
          type: "DROP", targetZone: "CLOSED", blockId: chunk[0].taskId, scale,
          onComplete: () => {
            setDisplayTasks(prev => {
              const updated = [...prev.filter(t => !chunk.some(c => c.taskId === t.taskId))];
              chunk.forEach(apiTask => {
                updated.push({ ...apiTask, status: TaskStatus.Closed, _trueStatus: apiTask.status });
              });
              displayTasksRef.current = updated;
              return updated;
            });
    ...
    const getZone = (apiTask: ITask) => {
      return apiTask.status === TaskStatus.Closed ? "CLOSED" : apiTask.status === TaskStatus.PendingReview ? "PENDING_REVIEW" : "OPEN";
    };
    ...
    tasksToProcess.forEach(apiTask => {
      ...
      if (!displayTask) {
        ...
        robotRef.current?.queueAction({
          type: "DROP", targetZone: getZone(apiTask), blockId: apiTask.taskId,
          onComplete: () => {
            setDisplayTasks(prev => {
        ...
        if (oldZone !== newZone) {
          robotRef.current?.queueAction({
            type: "PICKUP", targetZone: oldZone, blockId: apiTask.taskId,
            onComplete: () => {
              setDisplayTasks(prev => {
          ...
          robotRef.current?.queueAction({
            type: "DROP", targetZone: newZone, blockId: apiTask.taskId,
            onComplete: () => {
              setDisplayTasks(prev => {
