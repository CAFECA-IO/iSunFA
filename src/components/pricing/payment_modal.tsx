...
export default function PaymentModal({
  ...
  const checkPaymentStatus = useCallback(
    async (orderId: string, retries = 0) => {
      try {
        const statusRes = await request<{
          payload?: { status: string; transactionHash?: string };
        }>(`/api/v1/payment/oen/status?orderId=${orderId}`);
        const status = statusRes.payload?.status;

        if (status === "COMPLETED") {
          await refreshAuth();
          setTxHash(statusRes.payload?.transactionHash || "");
          setStep("success");
          onSuccess(statusRes.payload?.transactionHash || "");
          setLoading(false);
          ...
          if (retries < 30) {
            setTimeout(() => checkPaymentStatus(orderId, retries + 1), 2000);
          } else {
  ...
  }, [checkPaymentStatus, isOpen, user?.credits]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await request<{
        message?: string;
        payload?: { orderId: string; oenResponse: unknown };
      ...
      const { oenResponse } = response.payload;

      if (
        oenResponse &&
        typeof oenResponse === "object" &&
        "action" in oenResponse &&
        oenResponse.action === "CHARGED"
      ) {
        checkPaymentStatus(response.payload.orderId);
      } else if (
        oenResponse &&
        typeof oenResponse === "object" &&
        "paymentUrl" in oenResponse &&
        typeof oenResponse.paymentUrl === "string"
      ) {
        // Save state to keep modal open with correct props when returning
        ...
        // Open fallback checkout in the same tab to avoid popup blockers and navigate to Oen
        window.location.href = oenResponse.paymentUrl;
      } else if (typeof oenResponse === "string") {
        sessionStorage.setItem(
          "pendingPayment",
          JSON.stringify({
            amount,
        ...
        window.location.href = oenResponse;
      } else {
        console.error("No valid payment URL found in API response.");
        setError("Payment gateway offline. Please contact test administrator.");
        setStep("error");
        setLoading(false);
      }
    } catch (err) {
                    ...
                    <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left w-full">
                      {step === "confirm" && (
                        <>
                          ...
                          >
                            <div className="mt-6 sm:flex sm:flex-row-reverse border-t border-gray-100 pt-6">
                              <button
                                type="submit"
                                disabled={loading}
