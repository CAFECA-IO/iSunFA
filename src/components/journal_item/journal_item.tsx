...
const Operations = ({
  ...
  return (
    <td className="absolute right-0 top-1/2 flex -translate-y-1/2 items-center justify-center gap-4px md:justify-end">
      <button
        type="button"
        className="rounded-xs p-5px text-icon-surface-primary hover:text-icon-surface-accent md:p-10px"
...
const JournalItem = ({
  ...
  const displayedType =
    ...
    eventType === EventType.TRANSFER ? (
      <div className="flex w-fit items-center gap-5px rounded-full bg-badge-surface-soft-secondary px-10px py-6px text-sm font-medium text-badge-text-secondary-solid">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="16"
          height="16"
  ...
  const displayedAmount = (
    <div className="flex flex-col gap-8px py-8px text-left font-medium">
      <div className="flex items-center gap-6px">
        ...
        <p className="w-200px whitespace-nowrap text-text-neutral-tertiary">
  ...
  return (
    <tr
      key={voucherId}
      className="relative border-b border-stroke-neutral-quaternary text-center align-middle"
    >
      {/* Info: (20240808 - Anna) Alpha版先隱藏(日記帳頁面的選取方塊) */}
      ...
      <td className="px-16px text-left font-medium text-text-neutral-primary">
        {truncateString(description ?? '', 10)}
      </td>
      {/* Info: (20240418 - Julian) From / To */}
      <td className="px-16px text-left font-medium text-text-neutral-primary">{fromTo}</td>
...
export const JournalItemMobile = ({
  ...
  const displayedTypeMobile =
    ...
    eventType === EventType.TRANSFER ? (
      <div className="flex w-fit items-center rounded-full bg-badge-surface-soft-secondary px-10px py-6px text-sm font-medium text-badge-text-secondary-solid">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="16"
          height="16"
    ...
    >
      ...
      <td className="align-middle md:w-50px">
        <div className="flex justify-center px-10px">
          <input
            ...
            // checked={isChecked}
            // onChange={checkHandler}
            className="relative h-4 w-4 border border-checkbox-stroke-unselected bg-checkbox-surface-unselected accent-checkbox-stroke-unselected"
          />
        </div>
      </td>
      {/* Info: (20240418 - Julian) 日期 */}
      ...
      <td className="align-middle">
        <div className="flex w-180px items-center justify-center gap-4px p-8px">
          ...
          {displayedTypeMobile}
          <div className="flex flex-1 flex-col text-xs text-text-neutral-primary">
