...
const CounterpartyInput: React.FC<ICounterpartyInputProps> = ({
  counterparty,
  setCounterparty,
  flagOfSubmit,
  className,
  ...
  const counterpartyInput = isCounterpartyEditing ? (
    ...
    >
      <p className="w-100px border-r px-12px py-10px">{counterparty?.taxId ?? 'N/A'}</p>
      <p className="px-12px py-10px">{counterparty?.name ?? 'N/A'}</p>
    </div>
  );
  ...
  return (
    <div className={`relative flex w-full flex-1 flex-col items-start gap-8px ${className}`}>
