...
const NewVoucherForm: React.FC<NewVoucherFormProps> = ({ selectedData }) => {
  ...
  const handleOpenSelectorModal = useCallback(() => {
    setSelectedIds(selectedInvoices.map((item) => item.id));
    ...
    (resData: IPaginatedData<IInvoiceRC2[]>) => {
      ...
      const invoicesData = data.reduce(
        (acc, item) => {
          acc[item.id] = {
            ...item,
            isSelected: selectedInvoices.some((selectedItem) => selectedItem.id === item.id),
            actions: [],
          };
          return acc;
        },
  ...
  useEffect(() => {
    setSelectedInvoices(Object.values(selectedData));
