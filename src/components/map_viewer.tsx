...
export default function MapViewer({ routeGeojson = null, focusGeojson = null, className = "w-full h-full min-h-[600px]", interactive = true, hideLabel = false, fitBoundsPadding = 80 }: IMapViewerProps) {
    ...
    useEffect(() => {
        if (targetGeojson && mapRef.current) {
            ...
            if (bbox) {
                mapRef.current.fitBounds(bbox, { padding: fitBoundsPadding, duration: 2500, maxZoom: 12, essential: true });
            }
        }
    ...
    return (
        <div className={`${className} rounded-xl overflow-hidden shadow-2xl relative`}>
            ...
            {!hideLabel && (
                <div className="absolute bottom-4 left-4 bg-black/70 text-white px-4 py-2 rounded-lg backdrop-blur-sm text-sm border border-gray-700 flex flex-col gap-1">
