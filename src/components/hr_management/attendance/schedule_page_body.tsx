 ...
 * ## 這一頁是判定的輸入
 *
 * 出勤總覽（A9）是系統算出來的，這一頁是人排出來的 —— 而排班是判定的輸入之一。
 * 因此它必須能在判定之外獨立存在：下個月的班表現在就排得出來，
 * 那時還沒有任何打卡可判。
 *
 * ## 部門篩選在前端做
 *
 * A7 支援 `departmentId` 參數，但這一頁不用它 —— 因為下拉選單的選項來自
 * 回傳的資料，一旦把篩選推到伺服器，**選了某個部門之後其他選項就消失了**，
 * 使用者再也切不回去。整個月 × 全體在 demo 規模是十來列，前端篩零成本。
 *
 * ToDo: (20260813 - Julian) Demo 沒有權限控制，任何員工都改得了任何人的班。
 * 正式版排班是 HR／主管的動作（計畫書 §8.3 的唯讀模式），
 * 而**這一頁刻意不做假的唯讀開關** —— 一個擋不住任何請求的鎖頭圖示，
 * 在被問到「權限怎麼控」的時候，比誠實地說沒有更糟。
 */
...
const SchedulePageBody: FC = () => {
  ...
  const applyUpdate = async (update: IAttendanceScheduleUpdate) => {
    ...
    try {
      ...
      /**
       * Info: (20260813 - Julian) 只換那一格，不重抓整個月。
       *
       * A8 回的就是那一格的最新狀態，重抓一次只是把同一件事再問一遍 ——
       * 而中間那半秒畫面上的值是舊的，看起來像沒存進去。
       */
      setCalendar((current) =>
        current
          ? {
