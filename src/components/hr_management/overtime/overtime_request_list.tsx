 ...
 *
 * 申請與認列並排顯示：認列 = `min(核准, 打卡事實)`，申請 3 小時、認列 1 小時
 * 是**正常結果**不是故障，但只印一個數字，員工看到的是沒有解釋的落差
 * （ADR 024 §2）。兩個都印，落差就自己說明了自己。
 *
 * 簽核區是可輸入的欄位而不是一顆「同意」鈕，因為核 2 小時而申請 3 小時是
 * 常態。預設帶申請的分鐘數 —— 多數照准，少數才要改。
 */
const OvertimeRequestList: FC<{
  ...
  decidable?: boolean;
  /**
   * Info: (20260819 - Julian) 具 `HR_ADMIN` 職能才看得到 §32 IV 的認定區。
   * 藏不藏都不影響安全（service 端自己會擋），但把一個按下去必定被拒的表單
   * 放在主管面前，只會讓他以為系統壞了。
   */
  ...
  /**
   * Info: (20260818 - Julian) 被法定上限擋下的三個碼。單獨認出它們，是因為
   * 下一步不同：其他的是「這張單填錯了」，這三個是「這段工時已經發生，
   * 但超過法定上限」—— 那不是改一改欄位就能解決的事。
   */
  ...
  const decide = async (
    ...
    try {
      ...
      /**
       * Info: (20260818 - Julian) 核准的回應要說出三件事：認列多少、超出核准的有多少
       * （已列入未核准時段）、換出幾批補休。前兩者是同一個決定的兩面，只說一半
       * 會讓主管以為超出的部分消失了。
       *
       * 只有核准會回這三個數字 —— 駁回與撤回回的是單子本身，
       * 不檢查就會印出「認列 undefined 分」。
       */
    ...
    <div className="flex flex-col gap-2">
      {error && (
        <div className="rounded-xl bg-rose-50 px-3 py-2 text-sm text-rose-700">
          ...
          {/**
           * Info: (20260818 - Julian) 超限的下一步入口，**目前停用**。
           *
           * 「不擋」與「合法」是兩件事，而它們的資料模型不同：報告書若是 §32 IV
           * 的法定報備，它是**合法性依據**；若只是主管的例外核准說明，它是一筆
           * **違規紀錄**。把後者當成前者，等於讓系統對勞動檢查宣稱一件不成立的合法性。
           *
           * §32 IV 那一側已經有落腳處（上面的認定區），但這顆鈕處理的是**超限**
           * 那一種 —— 「超限但有報備」該落成哪一種紀錄還沒有決定，因此只放出入口、
           * 不改變阻擋行為。見計畫書 §8.3。
           */}
          ...
          >
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2 text-sm text-gray-800">
                  ...
                  {/**
                   * Info: (20260819 - Julian) 標記要連得到報備紀錄。一個寫著
                   * 「天災事變·加倍發給」卻點不進去的標記，看的人判斷不出它是不是真的
                   * 報備過 —— 而那正是這一欄從自填布林值改成強制記載的理由。
                   */}
                  {item.isEmergency &&
                    (() => {
                      /**
                       * Info: (20260820 - Julian) 只有 http(s) 才畫成連結。
                       *
                       * 服務端已擋在 validator 與不變式兩層；這裡守的是**既有資料** ——
                       * 補上驗證之前寫進去的列還在庫裡，而它們會直接進 `href`。
                       *
                       * 顯示 host 是因為這一欄刻意**不擋內網位址**（公文放在內部文件伺服器
                       * 是正當的），所以「連到哪」要讓讀的人自己看得到：
                       * `intranet.local` 與主管機關的網域不是同一回事。
                       */
                      const host =
            ...
            {decidable && item.status === OvertimeRequestStatus.PENDING && (
              <div className="mt-3 flex flex-wrap items-end gap-2 border-t border-gray-100 pt-3">
                ...
                {/**
                 * Info: (20260819 - Julian) §32 IV 的認定 —— **不是核准的一部分，是另一個人
                 * 做的另一件事**：自己送到 `.../emergency`，送完仍停在待簽核，
                 * 主管接著看到標記再決定核不核。
                 *
                 * 做成核准的一個欄位會撞上一個空集合：核准要求「管得到他的主管」、
                 * 認定要求 `HR_ADMIN`，一般組織裡沒有人同時是兩者。
                 *
                 * 兩格都必填：後果是整段工資跳到加倍發給，而「已報備」是一件對外發生的事。
                 * repository 端會再擋一次，這裡只是讓人不必按下去才知道。
                 */}
                          ...
                          onClick={() =>
                            decide(
                              ...
                              overtimeRequestEmergencyApi(item.id),
                              {
                                 ...
                                 *
                                 * `datetime-local` 給的是牆上時鐘。用 `new Date(值).toISOString()` 的話，
                                 * 人資筆電的時區就決定了 §32 IV 的「二十四小時內」怎麼算 ——
                                 * 政策時區由伺服器決定。
                                 */
             ...
             * 事後補單要填理由 —— 那是收回一句對已發生事實的陳述，方向對雇主有利，
             * 沒有理由的話事後判斷不出它是自願的還是被要求的。事前不必填。
             */}
      ...
      {/**
       * Info: (20260818 - Julian) 規則說明擺在清單末端、每種各一次。逐列重印的話
       * 五張待簽單就是五段一模一樣的灰字，而真正需要它的人被淹沒在重複裡。
       * 條件是「有沒有那個動作可做」而不是「清單空不空」。
       */}
