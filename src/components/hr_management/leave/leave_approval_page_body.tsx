 ...
 *
 * 駁回必須填理由、核准可以不填 —— 這個不對稱是刻意的：核准是照著規則走，
 * 駁回是**否定一個人的請求**，而他有權知道為什麼。一句「已駁回」會讓他
 * 重送一模一樣的單，然後再被駁一次。
 *
 * 這一頁不顯示事由：那是密文入庫的 Tier 2 個資。簽核者確實有權看，但那該
 * 發生在明細頁的一次明確點擊，而不是在一張會被投影出來的清單上一次攤開
 * 全部門的病名。
 */
        ...
        <div className="flex flex-col gap-2">
          {pending.map((item) => (
            ...
            >
              ...
              {/**
               * Info: (20260817 - Julian) 駁回理由就地展開，不用 modal。主管一次要處理一疊單子，
               * modal 會讓他每一張都多按兩次；就地展開也讓「哪一張正在被駁回」不會弄錯。
               */}
              {rejectingId === item.id && (
