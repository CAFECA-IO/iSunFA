...
export default function ReceiptPdfDownloader({
  ...
  const handleDownload = async (e: MouseEvent) => {
    ...
    try {
      ...
      const deterministicHashFallback =
        Math.abs(
          officialReceiptId
            .split("")
            .reduce(
      ...
      const officialRandomCode =
        payload?.data?.data?.randomCode || deterministicHashFallback.toString();
      ...
      const targetSelector = isB2B
        ? ".invoice-content-a4"
        : ".invoice-content-thermal";
      ...
      alert(
        t("billing.orders.download_failed", {
          defaultValue: "下載失敗，請稍後再試。",
        }),
      );
    } finally {
  ...
  const numberToChinese = (num: number) => {
    ...
    for (let i = 0; i < fraction.length; i++) {
      s += (
        digit[Math.floor(numValue * 10 * Math.pow(10, i)) % 10] + fraction[i]
      ).replace(/零./, "");
    }
    s = s || "整";
  ...
  return (
    <>
      <button
        ...
        className={
          className ||
          "inline-flex items-center justify-center gap-1.5 rounded-lg bg-orange-50 px-3 py-1.5 text-xs font-semibold whitespace-nowrap text-orange-600 transition-colors hover:bg-orange-100 disabled:opacity-50"
        }
      ...
      <div
        ref={containerRef}
        className="pointer-events-none fixed top-0 -left-[9999px] -z-10 opacity-0"
      >
        ...
        >
          ...
          >
            ...
            <div style={{ textAlign: "center", marginBottom: "8px" }}>
              ...
              >
                <span>隨機碼：{rcode}</span>
                <span>總計：{amount.toLocaleString()}</span>
              </div>
              <div
                style={{
                  display: "flex",
          ...
          >
            ...
            <div style={{ minHeight: "100px" }}>
              {invoiceItems.map((item, index) => (
                ...
                >
                  <div style={{ flex: "0 0 35%", fontWeight: "500" }}>
                    {item.name}
                  </div>
