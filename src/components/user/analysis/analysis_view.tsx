...
export default function AnalysisView() {
  ...
  const [accountBooks, setAccountBooks] = useState<
    Array<{ id: string; name: string; enterpriseId?: string }>
  >([]);
  ...
  const [pendingAccountBook, setPendingAccountBook] = useState<{
    id: string;
    name: string;
  } | null>(null);
  ...
  const currentCategories =
    activeTab === "internal" ? INTERNAL_CATEGORIES : EXTERNAL_CATEGORIES;
  const isInternalCompanyAnalysis = activeTab === "internal";
  const isCarbonAnalysis = [
    "CARBON_HEALTH_CHECK",
    "NET_ZERO_EMISSIONS",
  ].includes(category);
  const isExternalCarbonAnalysis = activeTab === "external" && isCarbonAnalysis;
  const needsCompanyInput =
    isInternalCompanyAnalysis || isExternalCarbonAnalysis;
  ...
  useEffect(() => {
    ...
    let isMounted = true;
    request<{
      payload: Array<{ id: string; name: string; enterpriseId?: string }>;
  ...
  const handleAnalysisWorkflow = async () => {
    ...
    if (isInternalCompanyAnalysis) {
      if (addons.bookkeeper)
        payloadItems.push({
          name: "Addon: Bookkeeper Visa",
          unitPrice: ANALYSIS_ADDON_COSTS.BOOKKEEPER,
          quantity: 1,
        });
      if (addons.cpa)
        payloadItems.push({
          name: "Addon: CPA Visa",
          unitPrice: ANALYSIS_ADDON_COSTS.CPA,
          quantity: 1,
        });
      if (addons.thirdParty)
        payloadItems.push({
          name: "Addon: Third Party Visa",
          unitPrice: ANALYSIS_ADDON_COSTS.THIRD_PARTY,
          quantity: 1,
        });
    }
  ...
  const handleUpdateTaxId = async () => {
    ...
    try {
      const res = await request(
        `/api/v1/user/account_book/${pendingAccountBook.id}`,
        {
          method: "PUT",
      ...
      if (res) {
        setAccountBooks((prev) =>
          prev.map((ab) =>
            ab.id === pendingAccountBook.id
              ? { ...ab, enterpriseId: taxIdInput }
              : ab,
          ),
        );
        setSelectedCompany({
  ...
  const renderPeriodOptions = useCallback(() => {
    ...
    switch (periodType) {
          ...
          <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
            {SEASONS.map((season) => (
              <button
                ...
                className={`py-3 ${buttonClass(selectedPeriodValue === season)}`}
              >
                {season}
              </button>
            ))}
          ...
          <div className="grid grid-cols-3 gap-2 sm:grid-cols-4 md:grid-cols-6">
            {MONTHS.map((month) => (
              <button
                ...
                className={`h-10 ${buttonClass(selectedPeriodValue === month)}`}
              >
                {month}
              </button>
            ))}
      ...
      case ANALYSIS_PERIOD.WEEKLY:
        return (
          <div className="grid max-h-48 grid-cols-4 gap-2 overflow-y-auto p-1 sm:grid-cols-8 md:grid-cols-10">
            {WEEKS.map((week) => (
              <button
                key={week}
                onClick={() => setSelectedPeriodValue(week)}
                className={`h-9 text-xs ${buttonClass(selectedPeriodValue === week)}`}
              >
                {week}
              </button>
            ))}
          ...
          <div className="flex flex-wrap gap-2">
            {last7Days.map((dateItem) => (
              <button
                ...
                className={`px-4 py-2 ${buttonClass(selectedPeriodValue === dateItem.value).replace("border-orange-200 bg-orange-50 text-orange-700 ring-1 ring-orange-200", "border-orange-600 bg-orange-600 text-white shadow-sm")}`}
              >
                {dateItem.label}
              </button>
            ))}
  ...
  const renderAddonsCheckbox = (
    ...
    return (
      <label
        className={`flex cursor-pointer items-center justify-between rounded-lg border p-3 transition-all ${isChecked ? "border-orange-500 bg-orange-50 ring-1 ring-orange-200" : "border-gray-200 hover:bg-gray-50"}`}
      >
        <div className="flex items-center gap-3">
          ...
          />
          <span
            className={`text-sm font-medium ${isChecked ? "text-orange-900" : "text-gray-700"}`}
          >
            {t(labelKey)}
          </span>
        </div>
    ...
    switch (category) {
        ...
        return (
          <>
            <p className="mb-2 text-sm font-bold text-orange-600">
              {t("analysis.tooltips.industry_development.title")}
            </p>
            <p className="mb-3 text-gray-600">
              {t("analysis.tooltips.industry_development.desc")}
            </p>
            <div className="space-y-4 text-gray-700">
              <div>
                <span className="block font-semibold text-orange-700">
                  {t("analysis.tooltips.industry_development.sectors_title")}
                </span>
                <span className="whitespace-pre-line text-gray-600">
                  {t("analysis.tooltips.industry_development.sectors_desc")}
                </span>
              </div>
              <div>
                <span className="block font-semibold text-orange-700">
                  {t("analysis.tooltips.industry_development.sub_title")}
                </span>
                <span className="whitespace-pre-line text-gray-600">
                  {t("analysis.tooltips.industry_development.sub_desc")}
                </span>
              </div>
              <div>
                <span className="block font-semibold text-orange-700">
                  {t("analysis.tooltips.industry_development.trends_title")}
                </span>
                <span className="whitespace-pre-line text-gray-600">
                  {t("analysis.tooltips.industry_development.trends_desc")}
                </span>
              </div>
            </div>
          </>
      ...
      case ANALYSIS_CATEGORY.IRSC:
        return (
          <>
            <p className="mb-2 text-sm font-bold text-orange-600">
              {t("analysis.tooltips.smart_enterprise_rating.title")}
            </p>
            <p className="mb-3 text-gray-600">
              {t("analysis.tooltips.smart_enterprise_rating.desc")}
            </p>
            <div className="space-y-4 text-gray-700">
              <div>
                <span className="block font-semibold text-orange-700">
                  {t(
                    "analysis.tooltips.smart_enterprise_rating.us_tickers_title",
                  )}
                </span>
                <span className="whitespace-pre-line text-gray-600">
                  {t(
                    "analysis.tooltips.smart_enterprise_rating.us_tickers_desc",
                  )}
                </span>
              </div>
              <div>
                <span className="block font-semibold text-orange-700">
                  {t(
                    "analysis.tooltips.smart_enterprise_rating.tw_tickers_title",
                  )}
                </span>
                <span className="whitespace-pre-line text-gray-600">
                  {t(
                    "analysis.tooltips.smart_enterprise_rating.tw_tickers_desc",
                  )}
                </span>
              </div>
              <div>
                <span className="block font-semibold text-orange-700">
                  {t("analysis.tooltips.smart_enterprise_rating.fuzzy_title")}
                </span>
                <span className="whitespace-pre-line text-gray-600">
                  {t("analysis.tooltips.smart_enterprise_rating.fuzzy_desc")}
                </span>
              </div>
              <div className="mt-3 rounded bg-orange-50 p-3">
                <span className="font-semibold text-orange-800">
                  {t(
                    "analysis.tooltips.smart_enterprise_rating.analyst_view_title",
                  )}
                </span>
                <br />
                <span className="whitespace-pre-line text-orange-700">
                  {t(
                    "analysis.tooltips.smart_enterprise_rating.analyst_view_desc",
                  )}
                </span>
              </div>
            </div>
      ...
      case ANALYSIS_CATEGORY.FINANCIAL_PRODUCT_RATING:
        return (
          <>
            <p className="mb-2 text-sm font-bold text-orange-600">
              {t("analysis.tooltips.financial_product_rating.title")}
            </p>
            <p className="mb-3 text-gray-600">
              {t("analysis.tooltips.financial_product_rating.desc")}
            </p>
            <div className="space-y-4 text-gray-700">
              <div>
                <span className="block font-semibold text-orange-700">
                  {t("analysis.tooltips.financial_product_rating.etf_title")}
                </span>
                <span className="whitespace-pre-line text-gray-600">
                  {t("analysis.tooltips.financial_product_rating.etf_desc")}
                </span>
              </div>
              <div>
                <span className="block font-semibold text-orange-700">
                  {t(
                    "analysis.tooltips.financial_product_rating.mutual_funds_title",
                  )}
                </span>
                <span className="whitespace-pre-line text-gray-600">
                  {t(
                    "analysis.tooltips.financial_product_rating.mutual_funds_desc",
                  )}
                </span>
              </div>
              <div>
                <span className="block font-semibold text-orange-700">
                  {t("analysis.tooltips.financial_product_rating.bonds_title")}
                </span>
                <span className="whitespace-pre-line text-gray-600">
                  {t("analysis.tooltips.financial_product_rating.bonds_desc")}
                </span>
              </div>
              <div>
                <span className="block font-semibold text-orange-700">
                  {t(
                    "analysis.tooltips.financial_product_rating.derivatives_title",
                  )}
                </span>
                <span className="whitespace-pre-line text-gray-600">
                  {t(
                    "analysis.tooltips.financial_product_rating.derivatives_desc",
                  )}
                </span>
              </div>
              <div className="mt-3 rounded bg-orange-50 p-3">
                <span className="font-semibold text-orange-800">
                  {t(
                    "analysis.tooltips.financial_product_rating.analyst_view_title",
                  )}
                </span>
                <br />
                <span className="whitespace-pre-line text-orange-700">
                  {t(
                    "analysis.tooltips.financial_product_rating.analyst_view_desc",
                  )}
                </span>
              </div>
            </div>
