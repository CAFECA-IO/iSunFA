...
export default function AiConfidence({
  ...
  const formattedNote =
    note
      ...
      ?.replaceAll(
        /-\s(\S+：)/g,
        "<li class='text-slate-800 font-bold'>$1</li>",
      )
      ...
      <Popover className="relative">
        {({ open, close }) => (
          <>
            ...
            <PopoverButton
              disabled={!hasNote}
              className={`flex items-center gap-1.5 rounded-full border px-2 py-1 text-xs font-bold shadow-sm transition-colors outline-none disabled:cursor-not-allowed disabled:border-gray-300 disabled:bg-gray-200 disabled:text-gray-500 ${
                open
