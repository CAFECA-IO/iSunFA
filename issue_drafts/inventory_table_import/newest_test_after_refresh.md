...
| (3) 屏東分公司 1.1 | 直接溫室氣體排放 (範疇一) | 2,591.862 TONNE | —(表3.8) | 2,591,861.5 | 系統計算 |
| (3) 屏東分公司 1.2 | 直接溫室氣體排放 (範疇一) | 13.221 TONNE | —(表3.8) | 13,220.6 | 系統計算 |
| (3) 屏東分公司 1.3 | 直接溫室氣體排放 (範疇一) | 189.036 TONNE | —(表3.8) | 189,036.3 | 系統計算 |
| (3) 屏東分公司 1.4 | 直接溫室氣體排放 (範疇一) | 19.959 TONNE | —(表3.8) | 19,958.9 | 系統計算 |
| (3) 屏東分公司 1.5 | 直接溫室氣體排放 (範疇一) | 0 TONNE | —(表3.8) | 0 | 系統計算 |
| (3) 屏東分公司 2.1 | 能源間接溫室氣體排放 (範疇二) | 3,325.015 TONNE | —(表3.8) | 3,325,015.2 | 系統計算 |
| (3) 屏東分公司 2.2 | 能源間接溫室氣體排放 (範疇二) | 0 TONNE | —(表3.8) | 0 | 系統計算 |
| (3) 屏東分公司 3.1 | 上游運輸與配送 | 176.821 TONNE | —(表3.8) | 176,821.1 | 系統計算 |
| (3) 屏東分公司 3.2 | 下游運輸與配送 | 927.458 TONNE | —(表3.8) | 927,457.5 | 系統計算 |
| (3) 屏東分公司 3.3 | 員工通勤 | 118.856 TONNE | —(表3.8) | 118,855.8 | 系統計算 |
| (3) 屏東分公司 3.5 | 商務差旅 | 3.1 TONNE | —(表3.8) | 3,100.2 | 系統計算 |
| (3) 屏東分公司 4.1 | 購買的商品與服務 | 654.907 TONNE | —(表3.8) | 654,906.8 | 系統計算 |
| (3) 屏東分公司 4.3 | 營運產生的廢棄物 | 101.685 TONNE | —(表3.8) | 101,684.5 | 系統計算 |
...
> _本報告已匯入，但帳本沒有任何可用數據，因此畫不出排放流向圖。桑基圖與系統數據表格的唯一來源是表3.8（各公司溫室氣體排放量），本次未取得該表。請確認第三章是否解析成功；若該章列為解析失敗，請以預覽卡的「重試失敗章節」重新匯入，並在伺服端日誌查看該表是否被丟棄及其原因。_

<!-- carbon-chart:IMPORTED_TOP_ITEMS_SANKEY:end -->

<!-- carbon-chart:IMPORTED_EMISSION_SANKEY:start -->

> _（資料不足，補齊活動數據後由系統自動生成圖表）_
